...
public class TransferManager {
  ...
  public static File getTransferFolder(PlayerState playerState) {
    File transferFolder =
        new File(DataUtils.getWorldDataPath() + "/transfer_data/" + playerState.getName());
    if (!transferFolder.exists()) transferFolder.mkdirs();
  ...
  public static boolean canTransfer(PlayerState playerState) {
    return playerState.getFirstControlledTransformableWOExc() != null
  ...
  public static boolean isValidTransfer(PlayerState playerState) {
    return playerState.getFirstControlledTransformableWOExc() != null
            ...
            && (playerState
                    ...
                    .getType()
                    .equals(SimpleTransformableSendableObject.EntityType.SHIP)
                && !(((Ship) playerState.getFirstControlledTransformableWOExc())
                    .getSpawner()
        ...
        || (playerState
                ...
                .getType()
                .equals(SimpleTransformableSendableObject.EntityType.SPACE_STATION)
            && !(((SpaceStation) playerState.getFirstControlledTransformableWOExc())
                .getSpawner()
                .toLowerCase(Locale.ENGLISH)
                .equals("<system>")));
  }

  public static void saveEntity(PlayerState playerState, SegmentController entity)
      throws Exception {
    ...
    if (transferFolder.isDirectory()) {
      ...
      File entityFile =
          new File(transferFolder.getPath() + "/" + entity.getUniqueIdentifierFull() + ".zip");
  ...
  public static void loadEntity(PlayerState playerState, String entityName) throws Exception {
    ...
    if (transferFolder != null && transferFolder.isDirectory()) {
      if (transferFolder.listFiles() != null
          && Objects.requireNonNull(transferFolder.listFiles()).length > 0) {
        for (File file : Objects.requireNonNull(transferFolder.listFiles())) {
          if (file.getName().endsWith(".zip")) {
            ...
            if (headerData.entityName.toLowerCase().contains(entityName.toLowerCase())) {
              ...
              File tempFile =
                  new File(
                      transferFolder.getPath() + "/entity_temp_" + headerData.saveDate + ".dat");
                  ...
                  SegmentController entity =
                      outline.spawn(
                          ...
                          new SegmentControllerSpawnCallbackDirect(
                              GameServer.getServerState(), playerState.getCurrentSector()) {
                            @Override
              ...
              throw new IllegalStateException(
                  "Failed to find blueprint \""
                      + entityName
                      + "\" even though it was just imported!");
            }
          }
        }
        throw new NullPointerException(
            "There is no entity by the name \""
                + entityName
                + "\" saved in the world transfer folder.");
      }
    }
  }
