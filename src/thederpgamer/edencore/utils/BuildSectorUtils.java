...
public class BuildSectorUtils {
    ...
    for (CatalogPermission permission : sender.getCatalog().getAvailableCatalog()) {
      try {
        BlueprintEntry blueprintEntry =
            BluePrintController.active.getBlueprint(permission.getUid());
        if (blueprintEntry.getName().toLowerCase().contains(entryName.toLowerCase()))
  ...
  public static void spawnEntry(PlayerState sender, BlueprintEntry entry, boolean aiEnabled) {
    ...
    } catch (PlayerNotFountException
        | PlayerControlledTransformableNotFound
        | IOException ignored) {
    }
    ...
    try {
      SegmentControllerOutline<?> outline =
      ...
      SegmentController entity =
          outline.spawn(
              ...
              false,
              new ChildStats(false),
              new SegmentControllerSpawnCallbackDirect(
                  GameServer.getServerState(), sender.getCurrentSector()) {
                @Override
  ...
  public static SegmentController getEntityByName(PlayerState sender, String entityName) {
    Map<String, SegmentController> entityMap =
        GameServer.getServerState().getSegmentControllersByName();
  ...
  public static SegmentControllerAIEntity<?> getAIEntity(SegmentController entity) {
    try {
      switch (entity.getType()) {
        ...
        default:
          throw new IllegalArgumentException("Entity must either be a Ship or Station!");
      }
    } catch (IllegalArgumentException exception) {
      LogManager.logCritical(
          "A critical exception occurred while trying to get an AIEntityState from an invalid or"
              + " corrupted entity!",
          exception);
    }
    return null;
  ...
  public static void toggleAI(SegmentController entity, final boolean toggle) {
    ...
    if (aiEntity[0] != null) {
      if (!toggle && aiEntity[0].getCurrentProgram() != null)
        aiEntity[0].getCurrentProgram().suspend(true);
      else if (aiEntity[0].getCurrentProgram() != null)
        aiEntity[0].getCurrentProgram().suspend(false);
    }
        ...
        new Runnable() {
          ...
          public void run() {
            for (SegmentController docked : dockedList) {
              ...
              if (diff
                  >= BuildSectorCommand
                      .MAX_WAIT_TIME) { // Shitty method to make sure game thread doesn't stall
                LogManager.logWarning(
                    "Toggling ai on entity \""
                        + docked.getRealName()
                        + "\" took more than "
                        + BuildSectorCommand.MAX_WAIT_TIME
                        + " ("
                        + diff
                        + "ms)",
                    null);
              ...
              try {
                ...
                LogManager.logException(
                    "Encountered an exception while toggling ai for entity \""
                        + docked.getRealName()
                        + "\".",
                    exception);
              }
  ...
  public static void setPeace(BuildSectorData sectorData, boolean protect) {
    if (GameCommon.isDedicatedServer() || GameCommon.isOnSinglePlayer()) {
      try {
        ...
        LogManager.logException(
            "Failed to protect "
                + sectorData.ownerName
                + "'s build sector from enemy spawns due to an unexpected error",
            exception);
        PlayerUtils.sendMessage(
            GameCommon.getPlayerFromName(sectorData.ownerName),
            "An unexpected error occurred while trying to protect your sector from pirate spawns."
                + " Let an admin know ASAP!");
      }
    }
  }
}
