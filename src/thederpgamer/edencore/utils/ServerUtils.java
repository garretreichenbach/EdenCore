...
public class ServerUtils {
  ...
  /**
   * Searches for a player by name on server. If the player isn't online, searches the server
    ...
    if (playerState == null) {
      try {
        ...
        LogManager.logException(
            "Encountered an exception while trying to fetch a player from database", exception);
      }
    }
    return playerState;
  }

  public static SegmentPiece getBlockLookingAt(GameServerState state, PlayerState player)
      throws IOException, PlayerNotFountException, PlayerControlledTransformableNotFound {
    Vector3f pos;
    Vector3f to;
    SimpleTransformableSendableObject firstControlledTransformable =
        player.getFirstControlledTransformable();
    if (player.getNetworkObject().isInBuildMode.getBoolean()) {
      ...
      Vector3f forw =
          GlUtil.getForwardVector(
              new Vector3f(), player.getBuildModePosition().getWorldTransform());
      forw.scale(5000);
      to = new Vector3f(pos);
      to.add(forw);
    } else {
      if (firstControlledTransformable instanceof AbstractCharacter<?>)
        pos =
            new Vector3f(
                ((AbstractCharacter<?>) firstControlledTransformable)
                    .getHeadWorldTransform()
                    .origin);
    ...
    CollisionWorld.ClosestRayResultCallback testRayCollisionPoint =
        ((PhysicsExt) sector.getPhysics())
            .testRayCollisionPoint(pos, to, false, null, null, false, true, false);
    if (testRayCollisionPoint.hasHit()
        && testRayCollisionPoint instanceof CubeRayCastResult
        && ((CubeRayCastResult) testRayCollisionPoint).getSegment() != null) {
  ...
  public static ArrayList<PlayerState> getAllPlayers() {
    assert GameCommon.isOnSinglePlayer() || GameCommon.isDedicatedServer();
    ArrayList<PlayerState> allPlayers =
        new ArrayList<>(GameServer.getServerState().getPlayerStatesByName().values());
    try {
      HashMap<String, Tag> entries = getEntriesOfType(EntryType.PLAYER_STATE);
      ...
      LogManager.logException(
          "Encountered an exception while trying to fetch players from database", exception);
    }
    return allPlayers;
  }
  ...
  public static HashMap<String, Tag> getEntriesOfType(final EntryType entryType) {
    ...
    if (databaseFolder.exists()) {
      try {
          ...
          for (File listFile : listFiles) {
            String name =
                listFile
                    .getName()
                    .substring(
                        entryType.databasePrefix.length(), listFile.getName().lastIndexOf("."));
            Tag tag =
                Tag.readFrom(new BufferedInputStream(new FileInputStream(listFile)), true, false);
        ...
        LogManager.logException(
            "Encountered an exception while trying to fetch server database entries", exception);
      }
    }
    return new HashMap<>();
  }
  ...
  public static String[] getPlayerDatabaseFields(String playerName) {
    ...
    try {
      String maxNIOSize =
          ";hsqldb.nio_max_size=" + ServerConfig.SQL_NIO_FILE_SIZE.getCurrentState() + ";";
      Connection c =
          DriverManager.getConnection(
              "jdbc:hsqldb:file:" + DatabaseIndex.getDbPath() + ";shutdown=true" + maxNIOSize,
              "SA",
              "");
