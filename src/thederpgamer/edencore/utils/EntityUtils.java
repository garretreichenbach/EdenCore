...
public class EntityUtils {
  ...
  public static void warpPlayerIntoEntity(PlayerState player, SegmentController entity) {
    ...
    if (entity.getType().equals(SimpleTransformableSendableObject.EntityType.SHIP))
      toEnter = entity.getSegmentBuffer().getPointUnsave(Ship.core);
    else if (entity.getType().equals(SimpleTransformableSendableObject.EntityType.SPACE_STATION))
      toEnter = getAvailableBuildBlock(entity);
    if (toEnter != null) {
      if (GameCommon.isOnSinglePlayer() || GameCommon.isClientConnectedToServer()) {
        ...
        GameClient.getClientState()
            ...
            .requestControlChange(
                GameClient.getClientPlayerState().getAssingedPlayerCharacter(),
                (PlayerControllable) entity,
                new Vector3i(),
                toEnter.getAbsolutePos(new Vector3i()),
                true);
  ...
  public static SegmentPiece getAvailableBuildBlock(SegmentController segmentController) {
    ...
    if (managerContainer != null && managerContainer.getBuildBlocks().size() > 0)
      return segmentController
          .getSegmentBuffer()
          .getPointUnsave(managerContainer.getBuildBlocks().toLongArray()[0]);
    else return null;
  }

  public static float getDistanceFromPlayer(
      PlayerState player, SegmentController segmentController) {
    Transform entityTransform = segmentController.getWorldTransform();
    Transform playerTransform = new Transform();
    player.getWordTransform(playerTransform);
    return Math.abs(
        Vector3fTools.distance(
            entityTransform.origin.x,
            entityTransform.origin.y,
            entityTransform.origin.z,
            playerTransform.origin.x,
            playerTransform.origin.y,
            playerTransform.origin.z));
  }

  public static void spawnEntry(
      PlayerState owner, BlueprintEntry entry, String spawnName, int factionId) {
      ...
      SegmentController entity =
          outline.spawn(
              ...
              new SegmentControllerSpawnCallbackDirect(
                  GameServer.getServerState(), owner.getCurrentSector()) {
                @Override
  ...
  public static void spawnEntryOnDock(
      PlayerState owner, BlueprintEntry entry, String spawnName, int factionId) {
