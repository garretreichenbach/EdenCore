...
/**
 * STARMADE MOD CREATOR: Max1M DATE: 20.09.2021 TIME: 20:02 helper class to access the server
 * database and player .ent files
 */
public class PlayerDataUtil {
  ...
  /**
   * stolen from EntitiyFileTools.write, modified will write the tag of given player to the
   * players.ent file (overwriting its persistent save data)
   *
   * @param tagStructure
   * @throws IOException
   */
  public static void writeTagForPlayer(final Tag tagStructure, final String playerName)
      throws IOException {
    ...
    synchronized (lock) {
      ...
      BufferedOutputStream bufferedOutputStream =
          new BufferedOutputStream(new FileOutputStream(tmpFile), 4096);
      DataOutputStream os = new DataOutputStream(bufferedOutputStream);

      try {
        if (tagStructure.getType() != Tag.Type.STRUCT
            || tagStructure.getValue() == null
            || ((Tag[]) tagStructure.getValue()).length < 1) {
          os.close();
          throw new IllegalArgumentException(
              "serialization of "
                  + playerName
                  + " failed, and will not be written because it could lead to corruption. Please"
                  + " send in a report!");
        }
      ...
      if (over.exists()) {
        if (old.exists()) {
          System.err.println(
              "Exception: tried parallel write off OLD: "
                  + old.getName()); // is this some legacy shit?
          old.delete();
        }
        ...
        System.err.println(
            "[SERVER] DELETING ORIGINALFILE TO REPLACE WITH NEW ONE: "
                + deleted
                + "; "
  ...
  /**
   * will load a playerstate from the names tag structure(savefile). only tag values will be set.
   * pure debug.
   *
   * @param playername
   * @return
  ...
  public static Tag getTagFromFile(String playername) throws IOException, EntityNotFountException {
    ...
    return GameServerState.instance
        .getController()
        .readEntity(pUID); // reads the .ent file with that UID
  }
}
