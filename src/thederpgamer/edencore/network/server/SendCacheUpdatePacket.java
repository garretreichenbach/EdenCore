...
public class SendCacheUpdatePacket extends Packet {
      ...
      for (int i = 0; i < sectorSize; i++) {
        try {
          ...
          LogManager.logException(
              "Encountered an exception while trying to deserialize Build Sector Data", exception);
        }
      }
    }
  ...
  public void writePacketData(PacketWriteBuffer packetWriteBuffer) throws IOException {
    ...
    if (blueprintExchangeItems.size() > 0)
    ...
    if (resourceExchangeItems.size() > 0)
      for (ResourceExchangeItem exchangeItem : resourceExchangeItems)
        exchangeItem.serialize(packetWriteBuffer);

    getItems();
    packetWriteBuffer.writeInt(itemExchangeItems.size());
    if (itemExchangeItems.size() > 0)
      for (ItemExchangeItem exchangeItem : itemExchangeItems)
        exchangeItem.serialize(packetWriteBuffer);
  ...
  public void processPacketOnClient() {
    ...
    try { // Exchange
      ...
      LogManager.logException(
          "Encountered an exception while trying to update client cache", exception);
    }
    ...
    try {
      ...
      for (BuildSectorData data : sectorData) {
        if (data != null
            && GameClient.getClientState() != null
            && GameClient.getClientPlayerState() != null
            && data.hasPermission(GameClient.getClientPlayerState().getName(), "ENTER"))
          ClientCacheManager.accessibleSectors.add(data);
      }

      ClientCacheManager.sectorEntities.clear();
      // if(DataUtils.isPlayerInAnyBuildSector(GameClient.getClientPlayerState()))
      // ClientCacheManager.sectorEntities.addAll(sectorEntities);
  ...
  private void getBlueprintItems() {
    ...
    for (Object obj :
        PersistentObjectUtil.getObjects(
            EdenCore.getInstance().getSkeleton(), BlueprintExchangeItem.class)) {
  ...
  private void getResourceItems() {
    ...
    for (Object obj :
        PersistentObjectUtil.getObjects(
            EdenCore.getInstance().getSkeleton(), ResourceExchangeItem.class)) {
  ...
  private void getItems() {
    ...
    for (Object obj :
        PersistentObjectUtil.getObjects(
            EdenCore.getInstance().getSkeleton(), ItemExchangeItem.class)) {
  ...
  private void getBuildSectors() {
    ...
    for (Object obj :
        PersistentObjectUtil.getObjects(
            EdenCore.getInstance().getSkeleton(), BuildSectorData.class)) {
  ...
  private void getSectorEntities() {
    if (DataUtils.isPlayerInAnyBuildSector(playerState))
      sectorEntities.addAll(
          DataUtils.getEntitiesInBuildSector(DataUtils.getPlayerCurrentBuildSector(playerState)));
    /*
    if(DataUtils.isPlayerInAnyBuildSector(playerState)) {
