...
/**
 * Requests that a player be kicked from a build sector. [CLIENT] -> [SERVER]
 *
 * @author TheDerpGamer
...
public class RequestBuildSectorKickPacket extends Packet {
  ...
  public void processPacketOnServer(PlayerState playerState) {
    ...
    try {
      if (DataUtils.isPlayerInAnyBuildSector(targetPlayer)
          && Objects.requireNonNull(DataUtils.getPlayerCurrentBuildSector(targetPlayer))
              .ownerName
              .equals(ownerName)) {
        if (targetPlayer.isOnServer()) {
          ...
          if (reason.isEmpty())
            PlayerUtils.sendMessage(
                targetPlayer, "You were kicked from " + ownerName + "'s build sector.");
          else
            PlayerUtils.sendMessage(
                targetPlayer,
                "You were kicked from " + ownerName + "'s build sector for \"" + reason + "\".");
        }
      }
    } catch (Exception exception) {
      LogManager.logException(
          "Encountered an exception while trying to kick player \""
              + playerState
              + "\" from a build sector.",
          exception);
    }
