...
/**
 * Requests that a player be banned from a build sector. [CLIENT] -> [SERVER]
 *
 * @author TheDerpGamer
        ...
        if (targetPlayer.isOnServer()) {
          ...
          if (reason.isEmpty())
            PlayerUtils.sendMessage(
                targetPlayer, "You were banned from " + ownerName + "'s build sector.");
          else
            PlayerUtils.sendMessage(
                targetPlayer,
                "You were banned from " + ownerName + "'s build sector for \"" + reason + "\".");
        }
      }
    } catch (Exception exception) {
      LogManager.logException(
          "Encountered an exception while trying to ban player \""
              + playerState
              + "\" from a build sector.",
          exception);
    }
