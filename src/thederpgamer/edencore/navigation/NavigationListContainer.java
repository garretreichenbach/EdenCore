...
/** STARMADE MOD CREATOR: Max1M DATE: 18.09.2021 TIME: 14:12 */
  ...
  public static NavigationListContainer getContainer(boolean autoAdd) {
    ArrayList<Object> objs =
        PersistentObjectUtil.getObjects(
            EdenCore.getInstance().getSkeleton(), NavigationListContainer.class);
    if (objs.size() == 0) {
    ...
    assert PersistentObjectUtil.getObjects(
                EdenCore.getInstance().getSkeleton(), NavigationListContainer.class)
            .size()
        == 1;
