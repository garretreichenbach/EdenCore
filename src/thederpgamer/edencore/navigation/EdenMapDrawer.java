        ...
        new Listener<MousePressEvent>() {
          ...
          public void onEvent(MousePressEvent event) {
            if (GameClientState.instance != null
                && GameClientState.instance.getWorldDrawer().getGameMapDrawer().isMapActive()
                && event.getRawEvent().pressedLeftMouse()
                && selected != null) {
              centerOn(selected.getSector());
            }
            // TODO make sure mouse is actually over the marker.

            // turned of auto-nav until event is more reliable. to much accidental navigation to
            // somewhere i dont wanna go.
