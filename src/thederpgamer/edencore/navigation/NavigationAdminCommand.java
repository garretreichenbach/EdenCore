...
public class NavigationAdminCommand implements CommandInterface {
  ...
  public String getDescription() {
    return "Adds, removes or lists coordinates to be displayed in each players saved coordinates.\n"
               ...
               + "%COMMMAND% gate \"name of gate\": creates a gate with name, gatesymbol and orange"
               + " lines for existing FTL connections. autosynch%COMMAND% synch: synchs server"
               + " change to clients.%COMMAND% list : Lists all public navigation points.\n";
  }
    ...
    switch (strings[0]) {
      case "add":
        {
          try {
            ...
            if (strings.length != 6 && strings.length != 3) return false;
      ...
      case "gate":
        { // gate "Space KFC"
          ...
          FTLConnection connection =
              GameServerState.instance
                  .getUniverse()
                  .getGalaxyManager()
                  .getFtlData()
                  .get(gateSector); // houses list of outgoing connections and their types.
          if (connection == null) {
          ...
          if (gateLines.isEmpty()) {
            PlayerUtils.sendMessage(
                admin, "zero outgoing warpgate connections exist from this sector.");
            return false;
          }

          // get existing gate
          MapMarker marker =
              NavigationUtilManager.instance.getPublicMarkers().get(gateSector.code());
          GateMarker gMarker;
      ...
      case "blue":
        {
          for (MapMarker marker : NavigationUtilManager.instance.getPublicMarkers().values()) {
            if (marker.getIcon().subSpriteIndex == 3) continue; // magges
