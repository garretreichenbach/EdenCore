...
public class BuildSectorCatalogScrollableList extends ScrollableTableList<CatalogPermission> {
  ...
  public BuildSectorCatalogScrollableList(
  ...
  private GUIHorizontalButtonTablePane redrawButtonPane(
      final CatalogPermission catalogPermission, GUIAncor anchor) {
        ...
        new GUICallback() {
            ...
            if (mouseEvent.pressedLeftMouse()) {
              if (sectorData.hasPermission(GameClient.getClientPlayerState().getName(), "SPAWN")) {
                ...
                PacketUtil.sendPacketToServer(
                    new RequestSpawnEntryPacket(catalogPermission.getUid(), false, false));
          ...
          public boolean isOccluded() {
            return !getState().getController().getPlayerInputs().isEmpty()
                || !sectorData.hasPermission(GameClient.getClientPlayerState().getName(), "SPAWN");
          }
        },
          ...
          public void callback(GUIElement guiElement, MouseEvent mouseEvent) {
            if (mouseEvent.pressedLeftMouse()) {
              if (sectorData.hasPermission(GameClient.getClientPlayerState().getName(), "SPAWN")) {
                ...
                PacketUtil.sendPacketToServer(
                    new RequestSpawnEntryPacket(catalogPermission.getUid(), true, false));
              } else getState().getController().queueUIAudio("0022_menu_ui - error 1");
            }
          }
          ...
          public void callback(GUIElement guiElement, MouseEvent mouseEvent) {
            if (mouseEvent.pressedLeftMouse()) {
              if (sectorData.hasPermission(
                  GameClient.getClientPlayerState().getName(), "SPAWN_ENEMIES")) {
                getState().getController().queueUIAudio("0022_menu_ui - select 3");
                PacketUtil.sendPacketToServer(
                    new RequestSpawnEntryPacket(catalogPermission.getUid(), false, true));
              } else getState().getController().queueUIAudio("0022_menu_ui - error 1");
            }
          }
  ...
  public void initColumns() {
    addColumn(
        Lng.str("Name"),
        15.0f,
        new Comparator<CatalogPermission>() {
          @Override
    ...
    addTextFilter(
        new GUIListFilterText<CatalogPermission>() {
          @Override
        ...
        Lng.str("SEARCH BY NAME"),
        ControllerElement.FilterRowStyle.LEFT);

    addDropdownFilter(
        new GUIListFilterDropdown<CatalogPermission, BlueprintType>(
            BlueprintType.SHIP, BlueprintType.SPACE_STATION) {
  ...
  public void updateListEntries(GUIElementList guiElementList, Set<CatalogPermission> set) {
    ...
    if (DataUtils.isPlayerInAnyBuildSector(GameClient.getClientPlayerState())) {
      for (CatalogPermission catalogPermission : set) {
        ...
        (classTextElement = new GUITextOverlayTable(10, 10, this.getState()))
            .setTextSimple(catalogPermission.getClassification().getName());
        ...
        (massTextElement = new GUITextOverlayTable(10, 10, this.getState()))
            .setTextSimple(StringTools.massFormat(catalogPermission.mass));
        GUIClippedRow massRowElement;
        (massRowElement = new GUIClippedRow(this.getState())).attach(massTextElement);

        BuildSectorCatalogListRow listRow =
            new BuildSectorCatalogListRow(
                getState(),
                catalogPermission,
                nameRowElement,
                typeRowElement,
  ...
  public class BuildSectorCatalogListRow extends ScrollableTableList<CatalogPermission>.Row {
    ...
    public BuildSectorCatalogListRow(
        InputState state, CatalogPermission catalogPermission, GUIElement... elements) {
