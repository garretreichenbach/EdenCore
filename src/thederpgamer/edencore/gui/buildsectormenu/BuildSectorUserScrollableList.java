...
public class BuildSectorUserScrollableList extends ScrollableTableList<String> {
        ...
        new GUICallback() {
          ...
          public boolean isOccluded() {
            return (playerName.equals(sectorData.ownerName)
                    && !ConfigManager.getMainConfig().getBoolean("debug-mode"))
                      ...
                      public boolean onInput(String s) {
                        if (!s.equals(sectorData.ownerName)) {
                          PacketUtil.sendPacketToServer(
                              new RequestBuildSectorKickPacket(
                                  sectorData.ownerName, playerName, s));
                          panel.recreateTabs();
                          return true;
                        } else {
              ...
              if (GameClient.getClientPlayerState().getName().equals(sectorData.ownerName)) {
                ...
                (new SimplePlayerTextInput("Enter Ban Reason", "") {
                      ...
                      public boolean onInput(String s) {
                        if (!s.equals(sectorData.ownerName)) {
                          PacketUtil.sendPacketToServer(
                              new RequestBuildSectorBanPacket(sectorData.ownerName, playerName, s));
                          panel.recreateTabs();
                          return true;
                        } else {
                          getState().getController().queueUIAudio("0022_menu_ui - error 2");
  ...
  public void updateListEntries(GUIElementList guiElementList, Set<String> set) {
    ...
    for (String playerName : set) {
      if (sectorData.hasPermission(playerName, "ENTER")) {
        String name =
            (playerName.equals(sectorData.ownerName)) ? playerName + " (Owner)" : playerName;
        GUITextOverlayTable nameTextElement;
        (nameTextElement = new GUITextOverlayTable(10, 10, this.getState())).setTextSimple(name);
        GUIClippedRow nameRowElement;
        (nameRowElement = new GUIClippedRow(this.getState())).attach(nameTextElement);

        BuildSectorUserScrollableListRow listRow =
            new BuildSectorUserScrollableListRow(getState(), playerName, nameRowElement);
  ...
  public class BuildSectorUserScrollableListRow extends ScrollableTableList<String>.Row {
    ...
    public BuildSectorUserScrollableListRow(
        InputState state, String playerName, GUIElement... elements) {
