...
public class ExchangeMenuPanel extends GUIMenuPanel {
	...
	private void createBlueprintsTab(GUIContentPane contentPane) {
		// GUIAncor tabAnchor = new GUIAncor(getState(), guiWindow.getInnerWidth() - 20,
		// guiWindow.getInnerHeigth() - 13);
	...
	public void givePlayerItem(ExchangeItem item) {
		...
		if (item instanceof BlueprintExchangeItem) {
			PacketUtil.sendPacketToServer(new RequestSpawnEntryPacket(item.name, false, GameClient.getClientPlayerState().getFactionId()));
