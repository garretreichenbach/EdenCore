...
public class AddBlueprintExchangeDialog extends GUIInputDialog {
    ...
    private BlueprintExchangeItem createItem() {
        if(NumberUtils.isNumber(getInputPanel().currentBarText) && getInputPanel().blueprintEntry != null && getInputPanel().barId > 0) {
            String iconPath = "";
            String description = "";
            CatalogPermission permission = getCatalogEntry(getInputPanel().blueprintEntry);
            ...
            BlueprintExchangeItem item = new BlueprintExchangeItem(getInputPanel().blueprintEntry, getInputPanel().barId, Math.abs(Integer.parseInt(getInputPanel().currentBarText)), description, iconPath);
            PacketUtil.sendPacketToServer(new ExchangeItemCreatePacket(0, item));
            return item;
        } else return null;
    }

    private CatalogPermission getCatalogEntry(BlueprintEntry blueprintEntry) {
        for(CatalogPermission permission : GameClient.getClientPlayerState().getCatalog().getAllCatalog()) {
            if(permission.getUid().equals(blueprintEntry.getName())) return permission;
        }
        return null;
    }
}
