...
public class AwardBarsCommand implements CommandInterface {
    ...
    public String getDescription() {
        return "Awards the specified player prize bars which can be used at the exchange.\n" +
               "- /%COMMAND% <player|all/*> <bronze|silver|gold> <count> [online_only] : Awards a player bars of the specified type.";
    }
        ...
        if(args[0].toLowerCase().equals("all") || args[0].toLowerCase().equals("*")) {
            ...
            if(targetPlayer == null) PlayerUtils.sendMessage(sender, "Player \"" + args[0] + "\" doesn't exist.");
                ...
                if(NumberUtils.isNumber(args[2].trim()) && bar != null) {
                    ...
                    if(onlineOnly && (!targetPlayer.isOnServer() || GameCommon.getPlayerFromName(targetPlayer.getName()) == null)) {
                    ...
                    String name = (amount > 1) ? bar.getItemInfo().getName() + "s" : bar.getItemInfo().getName();
                    if(targetPlayer.isOnServer() && GameCommon.getPlayerFromName(targetPlayer.getName()) != null) { //Player is online
                        PlayerUtils.sendMessage(targetPlayer, "You have been awarded " + amount + " " + name + ".");
