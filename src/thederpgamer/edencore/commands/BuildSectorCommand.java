...
public class BuildSectorCommand implements CommandInterface {
  ...
  public String[] getAliases() {
    return new String[] {"build_sector"};
  }
    ...
    if (args.length >= 1) {
      try {
        ...
        switch (subCommand) {
            ...
            if (args.length == 2) {
              ...
              if (target != null) {
                if (target.equals(sender))
                  PlayerUtils.sendMessage(sender, "You can't invite yourself!");
                else {
                  BuildSectorData sectorData = DataUtils.getBuildSector(sender.getName());
                  sectorData.addPlayer(target.getName());
                  PlayerUtils.sendMessage(
                      sender,
                      "Successfully added \"" + target.getName() + "\" to your build sector.");
                }
              } else PlayerUtils.sendMessage(sender, "Player \"" + args[1] + "\" doesn't exist.");
            } else return false;
            return true;
          case "remove":
            if (args.length == 2) {
              ...
              if (target != null) {
                if (target.equals(sender))
                  PlayerUtils.sendMessage(
                      sender, "You can't remove yourself from your own build sector!");
                else {
                  BuildSectorData sectorData = DataUtils.getBuildSector(sender.getName());
                  sectorData.removePlayer(target.getName());
                  if (DataUtils.getPlayerCurrentBuildSector(target) == sectorData)
                    DataUtils.movePlayerFromBuildSector(target);
                  PlayerUtils.sendMessage(
                      sender,
                      "Successfully removed \"" + target.getName() + "\" from your build sector.");
                }
              } else PlayerUtils.sendMessage(sender, "Player \"" + args[1] + "\" doesn't exist.");
            } else return false;
            return true;
          ...
          case "allow":
            if (args.length == 3) {
              ...
              if (target != null) {
                if (target.equals(sender))
                  PlayerUtils.sendMessage(sender, "You can't edit your own permissions!");
                else {
                  BuildSectorData sectorData = DataUtils.getBuildSector(sender.getName());
                  sectorData.allowPermission(target.getName(), args[2]);
                  PlayerUtils.sendMessage(
                      sender,
                      "Successfully set permission \""
                          + args[2]
                          + "\" for \""
                          + target.getName()
                          + "\" to ALLOW.");
          ...
          case "permissions":
            if (args.length == 2) {
              ...
              if (target != null) {
                ...
                StringBuilder builder = new StringBuilder();
                builder
                    .append("Current Permissions for \"")
                    .append(target.getName())
                    .append("\":\n");
                for (Map.Entry<String, Boolean> entry :
                    sectorData.getPermissions(target.getName()).entrySet())
                  builder
                      .append(entry.getKey())
                      .append(" : ")
          ...
          case "spawn":
            if (args.length == 2) {
              if (!DataUtils.isPlayerInAnyBuildSector(sender))
                PlayerUtils.sendMessage(
                    sender, "You must be in a build sector to perform this command.");
              else {
                ...
                if (sectorData.hasPermission(sender.getName(), "SPAWN")) {
                  ...
                  if (entry != null) {
                    ...
                    PlayerUtils.sendMessage(
                        sender, "Successfully spawned entity \"" + entry.getName() + "\".");
                  } else
                    PlayerUtils.sendMessage(
                        sender,
                        "Either you don't have catalog access to \""
                            + args[1]
              ...
              if (!DataUtils.isPlayerInAnyBuildSector(sender))
                ...
                if (sectorData.hasPermission(sender.getName(), "SPAWN_ENEMIES")) {
                  ...
                  if (entry != null) {
                    ...
                    PlayerUtils.sendMessage(
                        sender,
                        "Successfully spawned entity \"" + entry.getName() + "\" as an enemy.");
                  } else
                    PlayerUtils.sendMessage(
                        sender,
                        "Either you don't have catalog access to \""
          ...
          case "delete":
            ...
            if (!DataUtils.isPlayerInAnyBuildSector(sender))
              ...
              if (sectorData.hasPermission(sender.getName(), "DELETE")) {
                if (args.length == 1) {
                  if (GameCommon.getGameObject(sender.getSelectedEntityId()) != null
                      && GameCommon.getGameObject(sender.getSelectedEntityId())
                          instanceof SegmentController) {
                    SegmentController entity =
                        (SegmentController) GameCommon.getGameObject(sender.getSelectedEntityId());
                    if (entity.getSector(new Vector3i()).equals(sectorData.sector)) {
                      PlayerUtils.sendMessage(
                          sender, "Successfully deleted entity \"" + entity.getRealName() + "\".");
                      DataUtils.destroyEntity(entity);
                    } else
                      PlayerUtils.sendMessage(
                          sender,
                          "You must either specify a valid entity by name or have one selected to"
                  ...
                  if (entity != null
                      && entity.getSector(new Vector3i()).equals(sectorData.sector)) {
                    PlayerUtils.sendMessage(
                        sender, "Successfully deleted entity \"" + entity.getRealName() + "\".");
                    DataUtils.destroyEntity(entity);
                  } else
                ...
                if (sectorData.hasPermission(sender.getName(), "TOGGLE_AI")) {
                  ...
                  if (state.equals("on")
                    ...
                    if (args[1].toLowerCase().equals("*") || args[1].toLowerCase().equals("all")) {
                      ...
                      PlayerUtils.sendMessage(
                          sender, "Successfully activated all AI in build sector.");
                    } else {
                      SegmentController entity = BuildSectorUtils.getEntityByName(sender, args[1]);
                      if (entity != null) {
                        BuildSectorUtils.toggleAI(entity, true);
                        PlayerUtils.sendMessage(
                            sender,
                            "Successfully activated AI for entity \"" + entity.getName() + "\".");
                      } else
                        PlayerUtils.sendMessage(
                            sender,
                            "There is no entity by the name of \""
                                + args[1]
                                + "\" in the current build sector.");
                    }
                    ...
                    if (args[1].toLowerCase().equals("*") || args[1].toLowerCase().equals("all")) {
                      ArrayList<SegmentController> entityList =
                          DataUtils.getEntitiesInBuildSector(sectorData);
                      for (SegmentController entity : entityList) {
                        if (!entity.getDockingController().isTurretDocking()
                            || entity.getFactionId() == FactionManager.PIRATES_ID)
                          BuildSectorUtils.toggleAI(entity, false);
                      }
                      PlayerUtils.sendMessage(
                          sender, "Successfully deactivated all AI in build sector.");
                    } else {
                      SegmentController entity = BuildSectorUtils.getEntityByName(sender, args[1]);
                      if (entity != null) {
                        BuildSectorUtils.toggleAI(entity, false);
                        PlayerUtils.sendMessage(
                            sender,
                            "Successfully deactivated AI for entity \"" + entity.getName() + "\".");
                      } else
                        PlayerUtils.sendMessage(
                            sender,
                            "There is no entity by the name of \""
          ...
          case "warp":
            if (!DataUtils.isPlayerInAnyBuildSector(sender))
              ...
              if (sectorData != null && sectorData.hasPermission(sender.getName(), "EDIT")) {
                if (PlayerUtils.getCurrentControl(sender) instanceof SegmentController)
                  sender.getControllerState().forcePlayerOutOfSegmentControllers();
                if (args.length == 1) {
                  Sendable object = GameCommon.getGameObject(sender.getSelectedEntityId());
                  if (object instanceof SegmentController)
                    EntityUtils.warpPlayerIntoEntity(sender, (SegmentController) object);
                } else if (args.length == 2) {
                  SegmentController entity = BuildSectorUtils.getEntityByName(sender, args[1]);
                  if (entity != null && entity.getSector(new Vector3i()).equals(sectorData.sector))
                    EntityUtils.warpPlayerIntoEntity(sender, entity);
                } else return false;
                // if(PlayerUtils.getCurrentControl(sender) instanceof SegmentController &&
                // !PlayerUtils.getCurrentControl(sender).equals(object))
              } else PlayerUtils.sendMessage(sender, "You don't have permission to do this.");
            }
            return true;
        }
        ...
        LogManager.logException(
            "An exception occurred while trying to parse build_sector command", exception);
      }
    }
    return false;
  }
