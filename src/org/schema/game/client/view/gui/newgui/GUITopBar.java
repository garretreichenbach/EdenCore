...
public class GUITopBar extends GUIElement implements DialogInterface{
    ...
    private void addShipExpander(int index) {
            ...
            public String toString() {
                ...
                if (currentPlayerObject != null) {
                    if (currentPlayerObject instanceof Ship) {
                        return Lng.str("SHIP");
                    } else if (currentPlayerObject instanceof Planet) {
        ...
        shipExpander.addExpandedButton(Lng.str("THRUST"), new GUICallback() {
                ...
                if (event.pressedLeftMouse()) {
                    if (getState().getCurrentPlayerObject() != null && getState().getCurrentPlayerObject() instanceof Ship ) {
                        getState().getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager()
                                .activateThrustManager(getState().getShip());
                    }
                }
            }
        }, new GUIActivationHighlightCallback() {
            ...
            public boolean isHighlighted(InputState state) {
                return getState().getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager()
                        .getThrustManager().isTreeActiveAndNotSuspended();
            }
        });
        ...
        shipExpander.addExpandedButton(new Object(){
            ...
            public boolean isHighlighted(InputState state) {
                return
                        !getState().getController().getPlayerInputs().isEmpty() && getState().getController().getPlayerInputs().get(0) instanceof FactionBlockDialog;
            }
        });
    ...
    private void addMessageExpander(int index) {
        final ExpandedButton messageExpander = new ExpandedButton(getState(), 5);
...
//		});
        ...
        //INSERTED CODE
        messageExpander.addExpandedButton("BUILD TOOLS", new GUICallback() {
            @Override
            public void callback(GUIElement guiElement, MouseEvent mouseEvent) {
                if(mouseEvent.pressedLeftMouse()) EdenCore.getInstance().activateBuildToolsMenu();
            }
        ...
        if(getState().getPlayer().isAdmin()) {
            messageExpander.addExpandedButton("ADMIN TOOLS", new GUICallback() {
                @Override
                public void callback(GUIElement guiElement, MouseEvent mouseEvent) {
                    if(mouseEvent.pressedLeftMouse()) EdenCore.getInstance().activateAdminToolsMenu();
                }
    ...
    private void addExpander(ExpandedButton expandedButton) {
        getExpandableButtons().add(expandedButton);
    }
    int exp = 0;
    ...
    public class ExpandedButton extends GUIHorizontalButtonTablePane {
        ...
        private class CB implements GUICallbackBlocking{
            ...
            @Override
            public void onBlockedCallbackNotExecuted(
                    boolean anyOtherBlockedCallbacksExecuted) {
