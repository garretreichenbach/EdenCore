...
public class ShipyardOptionsChestButtonPanel extends GUIAncor implements InventoryToolInterface {
    ...
    public void onInit() {
        ...
        GUIInventoryOtherDropDown switchInvDrop = new GUIInventoryOtherDropDown(getState(), this, segmentController, new DropDownCallback() {
            ...
            public void onSelectionChanged(GUIListElement element) {
                if (element.getContent().getUserPointer() != null && element.getContent().getUserPointer() instanceof StashInventory) {
                    ...
                    getState().getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().getInventoryControlManager()
            ...
            public void callback(GUIElement callingGuiElement, MouseEvent event) {
                if (event.pressedLeftMouse()) {
                    ...
                    if(currentDocked != null && currentDocked.railController.isDockedAndExecuted() && !currentDocked.isVirtualBlueprint()){
                        PlayerGameOkCancelInput c = new PlayerGameOkCancelInput("CONFIRM", getState(), Lng.str("Undock"),
                                                                                Lng.str("Do you really want to undock\nthe current ship '%s' from shipyard?",  currentDocked.getRealName())) {
        ...
        ac.addButton(1, 1, Lng.str("Cancel current Order"), HButtonType.BUTTON_RED_MEDIUM, new GUICallback() {
            ...
            public void callback(GUIElement callingGuiElement, MouseEvent event) {
                if (event.pressedLeftMouse()) {
                    ...
                    PlayerGameOkCancelInput c = new PlayerGameOkCancelInput("CONFIRM", getState(), Lng.str("Cancel Order"),
                ...
                if (event.pressedLeftMouse()) {
                    final PlayerGameTextInput playerTextInput = new PlayerGameTextInput("RENAME_INVENTORY", getState(), 10, Lng.str("Rename %s",  inventory.getCustomName()), Lng.str("Enter a new name for this stash (10 characters max)")) {
                        ...
                        public boolean onInput(String entry) {
                            if (((ManagedSegmentController<?>) segmentController).getManagerContainer().getNamedInventoriesClient().size() < 16) {
                                ((ManagedSegmentController<?>) segmentController).getManagerContainer().getInventoryNetworkObject().getInventoryCustomNameModBuffer().add(
                                        new RemoteLongString(new LongStringPair(inventory.getParameter(), entry.trim()), false));
                            } else {
                    ...
                    GUITextButton removeName = new GUITextButton(getState(), 100, 20, ColorPalette.CANCEL, Lng.str("CLEAR NAME"), new GUICallback() {
                        ...
                        public void callback(GUIElement callingGuiElement, MouseEvent event) {
                            if (event.pressedLeftMouse()) {
                                ((ManagedSegmentController<?>) segmentController).getManagerContainer().getInventoryNetworkObject().getInventoryCustomNameModBuffer().add(
                                        new RemoteLongString(new LongStringPair(inventory.getParameter(), ""), false));
        ...
        GUITextOverlay l = new GUITextOverlay(10, 10, FontSize.MEDIUM, getState()){
