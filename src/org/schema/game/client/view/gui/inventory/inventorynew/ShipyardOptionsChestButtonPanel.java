...
/**
 * Modified version of ShipyardOptionsChestButtonPanel.java
 *
 * @author Schema, TheDerpGamer (modified)
 */
public class ShipyardOptionsChestButtonPanel extends GUIAncor implements InventoryToolInterface {
  ...
  public ShipyardOptionsChestButtonPanel(
      InputState state,
      SecondaryInventoryPanelNew panel,
      InventoryPanelNew mainPanel,
  ...
  public ShipyardCollectionManager getShipyard() {
    ...
    if (pointUnsave != null
        && segmentController instanceof ManagedSegmentController<?>
        && ((ManagedSegmentController<?>) segmentController).getManagerContainer()
            instanceof ShipyardManagerContainerInterface) {
      ShipyardManagerContainerInterface sm =
          (ShipyardManagerContainerInterface)
              ((ManagedSegmentController<?>) segmentController).getManagerContainer();
      ShipyardCollectionManager shipyardCollectionManager =
          sm.getShipyard().getCollectionManagersMap().get(pointUnsave.getAbsoluteIndex());
  ...
  public void onInit() {
    searchbar =
        new InventoryFilterBar(
            getState(),
            Lng.str("FILTER BY BLOCK NAME"),
            this,
            new TextCallback() {
              @Override
    ...
    segmentController =
        ((ManagerContainer<?>) inventory.getInventoryHolder()).getSegmentController();

    GUIInventoryOtherDropDown switchInvDrop =
        new GUIInventoryOtherDropDown(
            getState(),
            this,
            segmentController,
            new DropDownCallback() {

              @Override
              public void onSelectionChanged(GUIListElement element) {
                if (element.getContent().getUserPointer() != null
                    && element.getContent().getUserPointer() instanceof StashInventory) {
                  ...
                  getState()
                      ...
                      .setSecondInventory(v);
                  mainPanel.update(null, null);
                }
              }
            });
    ...
    ac.addButton(
        ...
        0,
        Lng.str("Place Order"),
        HButtonType.BUTTON_BLUE_MEDIUM,
        new GUICallback() {
          @Override
          ...
          public void callback(GUIElement callingGuiElement, MouseEvent event) {
            if (event.pressedLeftMouse()) {
              ...
              if (currentDocked != null
                  && currentDocked.railController.isDockedAndExecuted()
                  && !currentDocked.isVirtualBlueprint()) {
                PlayerGameOkCancelInput c =
                    new PlayerGameOkCancelInput(
                        "CONFIRM",
                        getState(),
                        Lng.str("Undock"),
                        Lng.str(
                            "Do you really want to undock\nthe current ship '%s' from shipyard?",
                            currentDocked.getRealName())) {
    ...
    ac.addButton(
        1,
        1,
        Lng.str("Cancel current Order"),
        HButtonType.BUTTON_RED_MEDIUM,
        new GUICallback() {
          @Override
          public boolean isOccluded() {
            return !panel.isActive();
            ...
            if (event.pressedLeftMouse()) {
              ...
              PlayerGameOkCancelInput c =
                  new PlayerGameOkCancelInput(
                      "CONFIRM",
                      getState(),
                      Lng.str("Cancel Order"),
                      Lng.str("Do you really want to cancel the current order?\n")
                          + (s == Constructing.class
                              ? Lng.str("Blocks collected for construction will be refunded.")
                              : "")) {
    ...
    GUIHorizontalProgressBar progress =
        new GUIHorizontalProgressBar(getState(), Lng.str("Pull Tick"), this) {
          @Override
          public float getValue() {
            if (inventoryActive) {
              text = Lng.str("Pull Tick");
              double serverRunningTime =
                  ShipyardOptionsChestButtonPanel.this
                          .getState()
                          .getController()
                          .getServerRunningTime()
    ...
    GUIHorizontalProgressBar progressOrder =
        new GUIHorizontalProgressBar(getState(), Lng.str("Current Order"), this) {
          ...
          public float getValue() {
            ...
            if (!yard.isPowered()
                || (wasUnpowered > 0 && System.currentTimeMillis() - wasUnpowered < 500)) {
              if (wasUnpowered == 0) {
    ...
    progressOrder.getPos().x = 1;
    progressOrder.getPos().y = 3 + 3 + 24 + 24 + 24;
    attach(progressOrder);

    GUIHorizontalProgressBar progressLoaded =
        new GUIHorizontalProgressBar(getState(), Lng.str("Loaded Entity"), this) {
          @Override
          public float getValue() {
    ...
    progressLoaded.getPos().x = 1;
    progressLoaded.getPos().y = 3 + 3 + 24 + 24 + 24 + 24;
        ...
        new Object() {
          ...
          public String toString() {
            ...
            return inventoryActive
                ? Lng.str("Pulling on Construction")
                : Lng.str("No Block Pulling");
          }
        },
        HButtonType.BUTTON_BLUE_MEDIUM,
        new GUICallback() {
          ...
          public void callback(GUIElement callingGuiElement, MouseEvent event) {
            if (event.pressedLeftMouse()) {
              pointUnsave.refresh();
              long index =
                  ElementCollection.getEncodeActivation(
                      pointUnsave, true, !pointUnsave.isActive(), false);
    ...
    pc.addButton(
        ...
        new GUICallback() {
          ...
          public void callback(GUIElement callingGuiElement, MouseEvent event) {
            if (event.pressedLeftMouse()) {
              final PlayerGameTextInput playerTextInput =
                  new PlayerGameTextInput(
                      "RENAME_INVENTORY",
                      getState(),
                      10,
                      Lng.str("Rename %s", inventory.getCustomName()),
                      Lng.str("Enter a new name for this stash (10 characters max)")) {
                    ...
                    public boolean onInput(String entry) {
                      if (((ManagedSegmentController<?>) segmentController)
                              .getManagerContainer()
                              .getNamedInventoriesClient()
                              .size()
                          < 16) {
                        ((ManagedSegmentController<?>) segmentController)
                            .getManagerContainer()
                            .getInventoryNetworkObject()
                            .getInventoryCustomNameModBuffer()
                            .add(
                                new RemoteLongString(
                                    new LongStringPair(inventory.getParameter(), entry.trim()),
                                    false));
                      } else {
                        getState()
                            .getController()
                            .popupAlertTextMessage(
                                Lng.str("Can only name up to 16\ninventories!"), 0);
                      }
                      return true;
                    }
                  };
              ...
              GUITextButton removeName =
                  new GUITextButton(
                        ...
                        public void callback(GUIElement callingGuiElement, MouseEvent event) {
                          if (event.pressedLeftMouse()) {
                            ((ManagedSegmentController<?>) segmentController)
                                ...
                                .add(
                                    new RemoteLongString(
                                        new LongStringPair(inventory.getParameter(), ""), false));
                    ...
                    public void draw() {
                      setPos(
                          playerTextInput.getInputPanel().getButtonCancel().getPos().x
                              + playerTextInput.getInputPanel().getButtonCancel().getWidth()
                              + 15,
                          playerTextInput.getInputPanel().getButtonCancel().getPos().y,
                          0);
                      super.draw();
                    }
                  };
    ...
    pc.addButton(
        ...
        new GUICallback() {
          ...
          public void callback(GUIElement callingGuiElement, MouseEvent event) {
            if (event.pressedLeftMouse()) {
              SimpleTransformableSendableObject cc =
                  getState().getPlayer().getFirstControlledTransformableWOExc();
              if (cc != null) {
                PlayerGameControlManager pp =
                    getState()
                        .getGlobalGameControlManager()
                        .getIngameControlManager()
                        .getPlayerGameControlManager();
                ...
                //						Vector3i compPos = getShipyard().getControllerElement().getAbsolutePos(new
                // Vector3i());
                Vector3i anc =
                    getShipyard()
                        .getCurrentDocked()
                ...
                if (cc instanceof PlayerCharacter) {
                  ...
                  if (cc != null
                      && cc instanceof PlayerCharacter
                      && getShipyard().getCurrentDocked() != null) {
                    ...
                    SegmentPiece p =
                        getShipyard()
                            .getCurrentDocked()
                            .getSegmentBuffer()
                            .getPointUnsave(Ship.core);
                    if (p != null && ElementKeyMap.isValidType(p.getType())) {
                      System.err.println(
                          "[CLIENT] Shipyard: Entering Design " + getShipyard().getCurrentDocked());
                      ...
                      if (enter) {
                        ...
                        getState()
                            .getController()
                            .popupAlertTextMessage(
                                Lng.str("Can't enter ship design. Core access denied."), 0);
                      }
                    } else {
                      getState()
                          .getController()
                          .popupAlertTextMessage(
                              Lng.str(
                                  "Can't enter ship design. No core found.\n"
                                      + "(Please report at help.star-made.org! This might be a"
                                      + " bug)"),
                              0);
                    }
                  }
                  ...
                  SegmentPiece p =
                      getShipyard().getCurrentDocked().getSegmentBuffer().getPointUnsave(Ship.core);
                  if (p != null) {
    ...
    pc.getPos().x = 1;
    pc.getPos().y = 2 + 3 + 3 + 24 + 24 + 24 + 24 + 24;
    attach(pc);
    ...
    progress.getPos().x = 1;
    progress.getPos().y = 2 + 3 + 3 + 24 + 24 + 24 + 24 + 24 + 24;
    attach(progress);

    attach(switchInvDrop);

    GUIHorizontalArea f =
        new GUIHorizontalArea(getState(), HButtonType.TEXT_FIELD, 10) {
    ...
    f.getPos().x = 1;
    f.getPos().y = 2 + 3 + 3 + 24 + 24 + 24 + 24 + 24 + 24 + 25;
    ...
    GUITextOverlay l =
        new GUITextOverlay(10, 10, FontSize.MEDIUM, getState()) {
  ...
  private void popupNewOrderDialog(ShipyardCollectionManager yard) {
    ...
    PlayerButtonTilesInput a =
        new PlayerButtonTilesInput(
            "SHIPYARDORDER", getState(), 800, 640, Lng.str("Order"), 260, 165) {
    ...
    for (ShipyardCommandType c : ShipyardCommandType.values()) {
      // INSERTED CODE
      if (!c.equals(ShipyardCommandType.TEST_DESIGN)) c.addTile(a, yard);
      //
    }
