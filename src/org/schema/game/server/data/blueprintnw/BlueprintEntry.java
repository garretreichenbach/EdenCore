...
public class BlueprintEntry implements BlueprintInterface {
  ...
  //	private static final byte THRUST_CONFIG = 8;
  //	private static final byte TRANSPORTER_CONFIG = 9;

  public final List<BBWirelessLogicMarker> wirelessToOwnRail =
      new ObjectArrayList<BBWirelessLogicMarker>();
  ...
  private BlueprintClassification classification = null;
  ...
  public static File importToFile(File toImport, BluePrintController bluePrintController)
      throws IOException {
    ...
    System.err.println(
        "[IMPORT][ZIP] TOTAL FILE SIZE EXTRACTED: "
            + toImport
            + ": "
            + ((bytes / 1024L) / 1024L)
            + " MB");
    ...
    long gbInBytes =
        1024L
            * 1024L
            * ((Integer) ServerConfig.ALLOWED_UNPACKED_FILE_UPLOAD_IN_MB.getCurrentState())
                .longValue();
    if (bytes > gbInBytes) {
      throw new IOException(
          "Extracted files too big (possible zip bomb through sparse files) for "
    ...
    if (topLevel.length != 1) {
      throw new IOException(
          "wrong file format to import. Must be exctly one dir, but found "
              + Arrays.toString(tmp.list()));
    }
    ...
    if (listFiles.length > 0) {
      for (int i = 0; i < listFiles.length; i++) {
        if (listFiles[i].getName().toLowerCase(Locale.ENGLISH).endsWith(".txt")) {
          ...
          bluePrintController.convert(
              "./bbtmp/" + catalogAndData.getName() + "/", false, listFiles[i].getName());
          found = true;
          break;
        }
      }
      ...
      throw new IOException(
          "wrong file format to import. found " + Arrays.toString(catalogAndData.list()));
    }
    if (!found) {
      String[] fList = catalogAndData.list();
      System.err.println(
          "failed to import old method. no Catalog.txt found in "
              + Arrays.toString(fList)
              + " trying new!");
      ...
      if (!found) {
        throw new IOException(
            "ERROR: No blueprint data found to import: " + Arrays.toString(fList));
      }
    }
    ...
    File toDir =
        new FileExt(bluePrintController.entityBluePrintPath + "/" + importFolders[0].getName());
    ...
    if (dataDir.exists() && dataDir.isDirectory()) {
      ...
      if (dataFiles.length > 0
          && dataFiles[0].getName().endsWith(".smd")
          && !dataFiles[0].getName().endsWith(".smd2")) {
        BluePrintController.migrateCatalogV00898(0, dPath);
      }
      //			if( dataFiles.length > 0 && dataFiles[0].getName().endsWith(".smd2") &&
      // !dataFiles[0].getName().endsWith(SegmentDataIO.BLOCK_FILE_EXT)){
      //				BluePrintController.migrateCatalogV00898(0, dPath);
      //			}
    }
  ...
  ///
  public static String importFile(File toImport, BluePrintController bluePrintController)
      throws IOException {
    File tmp = new FileExt("./bbtmp/");
    if (tmp.exists()) {
      FileUtil.deleteRecursive(tmp);
  ...
  private void copyDataFilesToBlueprint(final SegmentController controller) throws IOException {
        ...
        dbDataDir.listFiles(
            new FilenameFilter() {
              ...
              public boolean accept(File arg0, String arg1) {
                return arg1.startsWith(controller.getUniqueIdentifier() + ".");
              }
            });
    ...
    for (File rawFile : rawFiles) {
      ...
      String fName = "";
      if (parent == null) {
        rawBlockBlueprintFile =
            new FileExt(
                bbController.entityBluePrintPath + "/" + name + "/DATA/" + rawFile.getName());
      } else {
        fName = rawFile.getName();
        fName = name.substring(name.lastIndexOf("/")) + fName.substring(fName.indexOf("."));
        rawBlockBlueprintFile =
            new FileExt(bbController.entityBluePrintPath + "/" + name + "/DATA/" + fName);
      }
      ...
      System.err.println(
          "[BLUEPRINT] NAME: "
              ...
              + rawFile.getName()
              + " TO: "
              + rawBlockBlueprintFile.getAbsolutePath());
      FileUtil.copyFile(rawFile, rawBlockBlueprintFile);
    }
  }

  private void copyLocalDataFilesToBlueptint(final SegmentController controller)
      throws IOException {
    File dbDataDir = new FileExt(ClientStatics.SEGMENT_DATA_DATABASE_PATH);
    StringBuilder sb = new StringBuilder();
    SegmentDataFileUtils.convertUID(
        controller.getUniqueIdentifier(), controller.getObfuscationString(), sb);
    ...
    File[] rawFiles =
        dbDataDir.listFiles(
            new FilenameFilter() {
              @Override
              public boolean accept(File arg0, String arg1) {
                return arg1.startsWith(id + ".");
              }
            });
    if (rawFiles.length == 0) {
      System.err.println(
          "[BLUEPRINT] ERROR, NO DATA FOUND FOR: "
              + controller
              + "; UID: "
              + controller.getUniqueIdentifier()
              + "; Pattern: "
              + id);
    }
    for (File rawFile : rawFiles) {
      ...
      if (parent == null) {
        ...
        rawBlockBlueprintFile =
            new FileExt(
                bbController.entityBluePrintPath
                    + "/"
                    + name
                    + "/DATA/"
                    + (new FileExt(fName)).getName());
      } else {
  ...
  private void copyOldDataFilesToBlueprint(
      final SegmentControllerBluePrintEntryOld old, String entityBluePrintPath) throws IOException {
        ...
        dataPathF.listFiles(
            new FileFilter() {
              ...
              public boolean accept(File arg0) {
                return arg0.getName().startsWith(old.name + ".");
              }
            });
    for (File s : list) {
      if (s.getName().startsWith(old.name + ".")) {
        ...
        File outputFile =
            new FileExt(
                bbController.entityBluePrintPath + "/" + name + "/DATA/" + inputFile.getName());
    ...
    if (dataDir.exists() && dataDir.isDirectory()) {
      ...
      if (listFiles.length > 0
          && listFiles[0].getName().endsWith(".smd")
          && !listFiles[0].getName().endsWith(".smd2")) {
        BluePrintController.migrateCatalogV00898(0, dPath);
      }
    }
  }
  ...
  public File export() throws IOException {
    ...
    String tipDirName = bbController.entityBluePrintPath + File.separator + getName() + "/";
  ...
  public boolean isOkWithConfig(GameConfig g) {
    ...
    if (!ok) {
      ...
      for (BlueprintEntry e : getChilds()) {
  ...
  public void init(String name, BluePrintController bbController) {
    if (name.contains(".")) {
      System.err.println(
          "Exception: WARNING: Blueprint name contained irregular characters: \""
              + name
              + "\"! Removing characters...");
    }
    ...
    // INSERTED CODE
    this.modMappingsFile =
        new FileExt(bbController.entityBluePrintPath + "/" + name + "/modmappings.smbmm");
    ///
  ...
  public void read() throws IOException {
    ...
    readHeader(header);
    setTookHeaderRead(System.currentTimeMillis() - t);
    ...
    synchronized (getClass()) {
      ...
      if (needsChunkMigration) {
        ...
        DataOutputStream hOut =
            new DataOutputStream(new BufferedOutputStream(new FileOutputStream(header)));
    ...
    ///
    setTookMetaRead(System.currentTimeMillis() - t);
    t = System.currentTimeMillis();
    ...
    setTookStructureRead(System.currentTimeMillis() - t);
    t = System.currentTimeMillis();
  }
  ...
  public void readManagerTagOnly(File meta) throws IOException {
    DataInputStream stream =
        new DataInputStream(new BufferedInputStream(new FileInputStream(meta), 1024 * 64));
        ...
        case (DOCKING_BYTE):
          {
            ...
            for (int i = 0; i < size; i++) {
              ...
              child.dockingSize =
                  new Vector3f(stream.readFloat(), stream.readFloat(), stream.readFloat());
    ...
    while (!end && (dataType = stream.readByte()) != FINISH_BYTE) {
      ...
      switch (dataType) {
        ...
        case (DOCKING_BYTE):
          {
            if (childs == null) {
              childs = new ObjectArrayList<BlueprintEntry>();
            }
  ...
  public ControlElementMapper readStructure(File structure, boolean force) throws IOException {
    if (this.controlElementMap == null || force) {
      ...
      DataInputStream stream =
          new DataInputStream(new BufferedInputStream(new FileInputStream(structure), 1024 * 64));
      ...
      //			System.err.println("[BLUEPRINT] READ: "+controlElementMap.getAll().size()+" CONTROLLING
      // ELEMENS");
    }
  ...
  public void write(SegmentController controller, boolean local) throws IOException {
    ...
    DataOutputStream sOut =
        new DataOutputStream(new BufferedOutputStream(new FileOutputStream(structure)));
    ...
    if (local) {
      System.err.println(
          "[CLIENT][BLUEPRINT][LOCAL] " + controller + "; childs: " + childsToWrite.size());
  ...
  public void write(SegmentControllerBluePrintEntryOld old, String entityBluePrintPath)
      throws IOException {
    File fileOld = new FileExt(bbController.entityBluePrintPath + "/" + name + "Tmp/");
    header.getParentFile().renameTo(fileOld);
    if (!header.getParentFile().exists()) {
    ...
    try {
      ...
      DataOutputStream mOut =
          new DataOutputStream(new BufferedOutputStream(new FileOutputStream(meta)));
      writeMeta(old, mOut);
      mOut.close();
  ...
  private void readModMappings(FileExt file) throws IOException {
    ...
    if (file.exists()) {
      ...
      Scanner scanner = new Scanner(new InflaterInputStream(new FileInputStream(file)));
  ...
  private void writeModMappings() throws FileNotFoundException {
    ...
    for (Entry<String, Short> stringShortEntry :
        BlueprintModMappings.getCurrent().getReverseNamespacedBlocks().entrySet()) {
    ...
    DeflaterOutputStream output = new DeflaterOutputStream(new FileOutputStream(modMappingsFile));
    try {
  ...
  public static void writeHeader(
      ...
      Vector3f bbMax,
      BlueprintType t,
      EntityIndexScore score,
      BlueprintClassification classifi,
      ElementCountMap elementClassCountMap,
      DataOutputStream stream)
      throws IOException {
    stream.writeInt(dataVersion);
    String vString = Version.VERSION + "_" + Version.build;
  ...
  public void readHeader(File header) throws IOException {
    try {
      DataInputStream stream =
          new DataInputStream(new BufferedInputStream(new FileInputStream(header), 1024 * 64));
      ...
      System.err.println(
          "ERROR: " + headerVersion + "; " + getName() + "; " + blueprintSavedInGameVersion);
      throw e;
    }
  }

  public void writeHeader(SegmentController controller, DataOutputStream stream)
      throws IOException {
    ...
    if (controller instanceof ManagedSegmentController<?>) {
      ManagerContainer<?> managerContainer =
          ((ManagedSegmentController<?>) controller).getManagerContainer();
  ...
  private void writeHeader(SegmentControllerBluePrintEntryOld old, DataOutputStream stream)
      throws IOException {
    //		assert(false);
    stream.writeInt(dataVersion);
    stream.writeUTF(Version.VERSION + "_" + Version.build);
  ...
  public void writeMeta(
      ...
      DataOutputStream stream,
      FileOutputStream fileOutputStreamForSize)
      throws IOException {
    ...
    childsToWrite =
        new ObjectArrayList<
            BlueprintEntry>(); // BlueprintEntry[controller.getDockingController().getDockedOnThis().size()];
    int i = 0;
    ...
    if (cargo == null) {
      ...
      double totalCargo = 0;
      for (Entry<Long, CargoCollectionManager> e : cargo.getCollectionManagersMap().entrySet()) {
    ...
    if (controller.railController.isRoot()
        && controller.getPhysicsDataContainer() != null
        && controller.getPhysicsDataContainer().getShape() != null) {
    ...
    if (controller instanceof ManagedSegmentController<?>
      ...
      ActivationManagerInterface a =
          (ActivationManagerInterface)
              ((ManagedSegmentController<?>) controller).getManagerContainer();

      for (ActivationCollectionManager m : a.getActivation().getCollectionManagers()) {
        if (m.getDestination() != null) {
          SegmentController chainElementByUID =
              controller
                  .railController
                  .getRoot()
                  .railController
                  .getChainElementByUID(m.getDestination().marking);
    ...
    if (controller instanceof ManagedSegmentController<?>) {
      ...
      Tag t;
      if (((ManagedSegmentController<?>) controller).getManagerContainer()
      ...
      stream.flush();
      //			System.err.println("WRITTEN SC TAG: "+(fileOutputStreamForSize.getChannel().position() -
      // position));
  ...
  private void writeMeta(SegmentControllerBluePrintEntryOld old, DataOutputStream stream)
      throws IOException {
    stream.writeInt(metaVersion);
  ...
  public void writeStructure(SegmentController controller, DataOutputStream stream)
      throws IOException {
    stream.writeInt(structureVersion);
    //		System.err.println("[BLUEPRINT] serializing structure (ALL
    // "+controller.getControlElementMap().getControllingMap().getAll().size()+")");
    ControlElementMap.serializeForDisk(
        stream, controller.getControlElementMap().getControllingMap());
  }

  private void writeStructure(SegmentControllerBluePrintEntryOld old, DataOutputStream stream)
      throws IOException {
  ...
  public void canSpawn(StateInterface state, String newEntityName)
      throws EntityAlreadyExistsException {
    canSpawn(state, newEntityName, getEntityType(), false);
  }

  public static void canSpawn(
      StateInterface state, String newEntityName, BlueprintType type, boolean purge)
      throws EntityAlreadyExistsException {
    ...
    switch (type) {
      case SHIP:
        fileName = EntityRequest.convertShipEntityName(newEntityName);
        break;
      case ASTEROID:
        fileName = EntityRequest.convertAsteroidEntityName(newEntityName, false);
        break;
      case SHOP:
        fileName = EntityRequest.convertShopEntityName(newEntityName);
        break;
      case SPACE_STATION:
        fileName = EntityRequest.convertStationEntityName(newEntityName);
        break;
      case PLANET:
        fileName = EntityRequest.convertPlanetEntityName(newEntityName);
        break;
        ...
        if (purge) {
          ...
          long idForFullUID =
              ((GameServerState) state)
                  ...
                  .getTableManager()
                  .getEntityTable()
                  .getIdForFullUID(type.type.dbPrefix + newEntityName);
          if (idForFullUID >= 0) {
            ((GameServerState) state)
                .getDatabaseIndex()
                .getTableManager()
                .getEntityTable()
                .removeSegmentController(
                    type.type.dbPrefix + newEntityName, (GameServerState) state);
          }
        } else {
          if ((!newEntityName.startsWith("MOB_SIM")
              || (dbCount =
                      ((GameServerState) state)
                          .getDatabaseIndex()
                          .getTableManager()
                          .getEntityTable()
                          .getByUIDExact(newEntityName, 1)
                          .size())
                  > 0)) {
            if (dbCount > 0) {
            ...
            throw new EntityAlreadyExistsException(
                "FILE: " + fileName + "; Name-Only: " + newEntityName);
          } else {
            System.err.println(
                "[MOB] File exists but is not in database: Removing file " + fileName);
            (new FileExt(fileName)).delete();
          }
        }
