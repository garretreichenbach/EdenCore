...
public class PlayerState extends AbstractOwnerState implements Sendable, DiskWritable, FogOfWarReceiver, RuleEntityContainer {
    ...
    public PlayerState(StateInterface state) {
        ...
        setPersonalFactoryInventoryCapsule(new PersonalFactoryInventory(this,CAPSULE_INV, ElementKeyMap.FACTORY_CAPSULE_ASSEMBLER_ID));
        setPersonalFactoryInventoryMicro(new PersonalFactoryInventory(this, MICRO_INV, ElementKeyMap.FACTORY_MICRO_ASSEMBLER_ID));
        setPersonalFactoryInventoryMacroBlock(new PersonalFactoryInventory(this,  MACRO_BLOCK_INV, ElementKeyMap.FACTORY_BASIC_ID));
    ...
    public void announceKill(Damager killerEntity) {
        ...
        if (isOnServer()) {
            ...
            System.err.println(getState() + " " + this
                    + " Announcing kill: " + killerEntity + " killed " + this);
    ...
    public void checkIfDiedOnServer() {
        ...
        if (!dieList.isEmpty()) {
            ...
            System.err.println("[SERVER] PLAYER " + this
                    + " died, removing ALL control units");

            Sector sector = ((GameServerState) getState()).getUniverse().getSector(getCurrentSectorId());
            if (sector != null) {

                if(isSpawnProtected()) {
                    sendServerMessage(new ServerMessage(Lng.astr("No punishment: spawn protection"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                    ...
                    if (lastSpawnedThisSession == 0 || System.currentTimeMillis() - lastSpawnedThisSession > secsUntilPunish * 1000) {
                        if (cMult < 1f) {
                            ...
                            if (ServerConfig.PLAYER_DEATH_CREDIT_DROP.isOn()) {
                                sendServerMessage(new ServerMessage(Lng.astr("You dropped %s credits!",  lost), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                                dropCreditsIntoSpace(lost);
                            } else {
                                sendServerMessage(new ServerMessage(Lng.astr("You lost %s credits!",  lost), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                                modCreditsServer(-(lost));
                            }
                        }
                        if (ServerConfig.PLAYER_DEATH_BLOCK_PUNISHMENT.isOn()) {
                            sendServerMessage(new ServerMessage(Lng.astr("Your inventory dropped into space!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                            putAllInventoryInSpace();
                        }

                    } else {
                        long s = (getState().getUpdateTime() - lastSpawnedThisSession) / 1000;
                        sendServerMessage(new ServerMessage(Lng.astr("No punishment: respawn protection still on!\n%s / %s seconds",  s,  secsUntilPunish), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                    }
                } else {
                    sendServerMessage(new ServerMessage(Lng.astr("No punishment: Sector set to peace!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                }
            }
            ...
            synchronized (getState().getLocalAndRemoteObjectContainer()
                    .getLocalObjects()) {
                for (Sendable s : getState().getLocalAndRemoteObjectContainer()
        ...
        try {
            ...
            if (t != null) {
                ...
                if (t instanceof SegmentController) {
                    ...
                    ClosestRayResultCallback result = ((PhysicsExt) s.getPhysics()).testRayCollisionPoint(v,
                            v2, false, null, (SegmentController) t, false, true, false);
                    check = result.hasHit();
                } else {
                ...
                if (check) {
                    ...
                    if (item.getType() == FreeItem.CREDITS_TYPE) {
                        ...
                        sendServerMessage(new ServerMessage(Lng.astr("Picked up %s credits",  item.getCount()), ServerMessage.MESSAGE_TYPE_INFO, this.getId()));
                        item.setCount(0);
                    } else {
                        ...
                        if((countMax = getInventory().canPutInHowMuch(item.getType(), countMax, item.getMetaId())) > 0){
                            ...
                            if (item.getType() > 0) {
                                sendServerMessage(new ServerMessage(Lng.astr("Picked up %s\n%s",  countMax,  ElementKeyMap.getInfo(item.getType()).getName()), ServerMessage.MESSAGE_TYPE_INFO, this.getId()));
                            }else{
                                MetaObject object = ((MetaObjectState) state).getMetaObjectManager().getObject(item.getMetaId());
                                if(object != null){
                                    sendServerMessage(new ServerMessage(Lng.astr("Picked up %s", object.getName()), ServerMessage.MESSAGE_TYPE_INFO, this.getId()));
                                }
                            ...
                            if (System.currentTimeMillis() - lastInventoryFullMsgSent > 7000) {
                                sendServerMessage(new ServerMessage(Lng.astr("Inventory full!"), ServerMessage.MESSAGE_TYPE_WARNING, this.getId()));
    ...
    public void initFromNetworkObject(NetworkObject o) {
        ...
        if (!isOnServer()) {
            ...
            for(int i = 0; i < getNetworkObject().cargoInventoryChange.getReceiveBuffer().size(); i++){
                RemoteSegmentControllerBlock remoteSegmentControllerBlock = getNetworkObject().cargoInventoryChange.getReceiveBuffer().get(i);
            ...
            if (getClientId() == state.getId()) {
                ...
                s.setPlayer(this);
                System.out
                        .println("[PlayerState] Client successfully received player state "
    ...
    public void updateFromNetworkObject(NetworkObject o, int senderId) {
        ...
        if (!isClientOwnPlayer()) {
            ...
            if (senderId != 0 && senderId != clientId) {
                System.err.println(getState() + " " + this + " WARNING: Possible attempt to hack controls of another player. sender: " + senderId + " but was: " + clientId);
        ...
        for(int i = 0; i < getNetworkObject().resetFowBuffer.getReceiveBuffer().size(); i++){
            resetFowSystem.enqueue(getNetworkObject().resetFowBuffer.getReceiveBuffer().get(i).getVector());
        }
        ...
        if (!isOnServer()) {
            ...
            for (int i = 0; i < getNetworkObject().blockCountMapBuffer.getReceiveBuffer().size(); i++) {
                RemoteBlockCountMap remoteBlockCountMap = getNetworkObject().blockCountMapBuffer.getReceiveBuffer().get(i);
                if (isClientOwnPlayer()) {
...
//		getSpawnPoint().set(getNetworkObject().spawnPoint.getVector());
        ...
        if (isOnServer()) {
            ...
            for (int i = 0; i < getNetworkObject().creatureSpawnBuffer.getReceiveBuffer().size(); i++) {
                RemoteCreatureSpawnRequest r = getNetworkObject().creatureSpawnBuffer.getReceiveBuffer().get(i);
            ...
            if (getState().getLocalAndRemoteObjectContainer().getLocalObjects().containsKey(p.aquiredTargetId.get())) {
                aquiredTarget = (SimpleTransformableSendableObject) getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(p.aquiredTargetId.get());
                //INSERTED CODE
...
//			getNetworkObject().spawnPoint.set(getSpawnPoint());
            ...
            assert (!changedSit || (getNetworkObject().sittingState.hasChanged() && getNetworkObject().isChanged()));

            getNetworkObject().isAdminClient.set(((GameServerState) getState()).getController().isAdmin(getName()));
        ...
        if (isClientOwnPlayer()) {
            ...
            SimpleTransformableSendableObject selectedAITarget = ((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().getPlayerIntercationManager().getSelectedAITarget();

            getNetworkObject().selectedEntityId.set(selectedEntity == null ? -1 : selectedEntity.getId());
            getNetworkObject().selectedAITargetId.set(selectedAITarget == null ? -1 : selectedAITarget.getId());
    ...
    public Vector3f getRight(Vector3f out) {
        if (isOnServer() || !isClientOwnPlayer()) {
            ...
            if (Controller.getCamera() instanceof InShipCamera) {
                return ((InShipCamera) Controller.getCamera())
                        .getHelperCamera().getRight(out);
            } else {
    ...
    public Vector3f getForward(Vector3f out) {
        if (isOnServer() || !isClientOwnPlayer()) {
            ...
            if (Controller.getCamera() instanceof InShipCamera) {
                return ((InShipCamera) Controller.getCamera())
                        .getHelperCamera().getForward(out);
            } else {
                if (Controller.getCamera() != null && !(Controller.getCamera() instanceof ObjectViewerCamera)) {
...
//	}
    ...
    public void updateLocal(Timer timer) {
        ...
        if (firstControlledTransformable != null && firstControlledTransformable instanceof ShopperInterface) {
            getShopsInDistance().addAll(((ShopperInterface) firstControlledTransformable).getShopsInDistance());
        }
        ...
        while(!isOnServer() && ((GameClientState)getState()).getController().getClientChannel() != null && !resetFowSystem.isEmpty()){
            Vector3i dequeue = resetFowSystem.dequeue();
            System.err.println("[CLIENT] Received FOW reset for " + getName());
            ((GameClientState)getState()).getController().getClientChannel().getGalaxyManagerClient().resetClientVisibilitySystem(dequeue);
        }
        ...
        if (isOnServer()) {
            ...
            if(forcedEnterUID != null && getAssingedPlayerCharacter() != null && getFirstControlledTransformableWOExc() == getAssingedPlayerCharacter()) {
                    ...
                    if(pointUnsave != null) {
                        getControllerState()
                                .requestControlServerAndSend(
                                        ...
                                        (PlayerControllable) pointUnsave
                                                .getSegment()
        ...
        if (isClientOwnPlayer()) {
            ...
            if (tutorialCallClient != null) {
                if (tutorialCallClient.equals("TutorialBasics00Welcome")) {
                    if (((GameClientState) getState()).getController().getTutorialMode() != null && ((GameClientState) getState()).getController().getTutorialMode().getMachine().getFsm().getCurrentState() instanceof TeleportToTutorialSector) {
                ...
                while (!requestedreatureSpawns.isEmpty()) {
                    ...
                    if (isOnServer()) {
                        ...
                        if (s.isAdmin(getName())) {
                            try {
                                dequeue.execute(s);
                                sendServerMessage(new ServerMessage(Lng.astr("Spawing creature"), ServerMessage.MESSAGE_TYPE_INFO, getId()));
                            } catch (IOException e) {
                                e.printStackTrace();
                                sendServerMessage(new ServerMessage(Lng.astr("[SERVER] Error spawning\nPlease send a report"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                            }

                        } else {
                            sendServerMessage(new ServerMessage(Lng.astr("Cannot spawn!\nYou are not an admin!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                        }
                    }
                }
            }
        ...
        if (!queuedModifactions.isEmpty()) {
            synchronized (queuedModifactions) {
                while (!queuedModifactions.isEmpty()) {
                    ...
                    getNetworkObject().getInventoryMultModBuffer().add(new RemoteInventoryMultMod(dequeue, getNetworkObject()));
                }
            }
        }
        ...
        if (isOnServer()) {
            ...
            if (getHelmetSlot() >= 0 && getInventory().getType(getHelmetSlot()) != MetaObjectType.HELMET.type) {
                setHelmetSlot(-1);
            }
            ...
            if (s.getUniverse().existsSector(currentSectorId)) {
                ...
                System.err.println("[SERVER] Exception (CRITICAL) player " + this + " is in an unloaded sector (" + getCurrentSectorId() + ")" + " attempting reload of " + getCurrentSector());
                Sector sector;
                try {
            ...
            if (serverClient != null) {
                this.ping = (int) serverClient.getProcessor()
                        .getPingTime();
                if (serverClientFirst) {
                    ...
                    while (getHosts().size() > (Integer) ServerConfig.PLAYER_HISTORY_BACKLOG.getCurrentState()) {
                        getHosts().remove(0);
                    }
...
//					if(su.playerControllable == firstControlledTransformable && su.playerControllable instanceof SegmentController && su.getParameter() != null){
//						Vector3f p = new Vector3f(su.getParameter().x-8,su.getParameter().y-8,su.getParameter().z-8);
...
//			}
            ...
            try {
                ...
                e.printStackTrace();
                sendServerMessage(new ServerMessage(Lng.astr("Cannot spawn!\n(already exists)"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
            } catch (IOException e) {
                e.printStackTrace();
                throw new RuntimeException(e);
            }
        ...
        if (!basicTutorialStarted && isOnServer() && Sector.isTutorialSector(getCurrentSector()) && getAssingedPlayerCharacter() != null) {
        ...
        if (sectorChanged) {
            ...
            if (isOnServer() && !isInvisibilityMode() && !beforeSystem.equals(getCurrentSystem())) {
                try {
                    StellarSystem sSys = ((GameServerState) getState()).getUniverse().getStellarSystemFromStellarPos(getCurrentSystem());
                    if (sSys.getOwnerFaction() != 0 && sSys.getOwnerFaction() != getFactionId()) {
                        Faction faction = ((GameServerState) getState()).getFactionManager().getFaction(sSys.getOwnerFaction());
                        if (faction != null) {
                            //how is the owner's relation to this player
                            RType relation = ((GameServerState) getState()).getFactionManager().getRelation(faction.getIdFaction(), getFactionId());
                            ...
                            switch (relation) {
                                case ENEMY:
                                    ...
                                    faction.broadcastMessage(msg, ServerMessage.MESSAGE_TYPE_WARNING, ((GameServerState) getState()));
                                    ...
                                    break;
                                case FRIEND:
                                    faction.broadcastMessage(Lng.astr("Our scanners picked up a FRIENDLY\nsignature in our territory.\n\nOrigin System: %s %s.\nSend Scanner there to get exact\npositions!",  sSys.getName(),  sSys.getPos()), ServerMessage.MESSAGE_TYPE_INFO, ((GameServerState) getState()));
                                    break;
                                case NEUTRAL:
        ...
        if (isOnServer()) {
            try {
                ...
                currentSectorType = ((GameServerState) getState()).getUniverse().getStellarSystemFromStellarPos(currentSystem).getSectorType(currentSector);
...
//				System.err.println("SERVER CENTER TYPE: "+((GameServerState)getState()).getUniverse().getStellarSystemFromStellarPos(currentSystem).getCenterSectorType());
        ...
        try {
            ...
            e.printStackTrace();
            if (isOnServer()) {
                ((GameServerState) getState()).getController().broadcastMessage(Lng.astr("[UPLOAD][ERROR] Critical error while extracting upload file!"), ServerMessage.MESSAGE_TYPE_ERROR);
            }
        } catch (Exception e) {
            e.printStackTrace();
            if (!isOnServer()) {
...
//		System.err.println("CURRENT: "+currentSectorType);
    ...
    private void handleDragDrop(DragDrop g) {
        try {
            ...
            Sendable sendable = getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(invHolderId);
                ...
                if (s != null) {
                    ...
                    if (type == InventorySlot.MULTI_SLOT && typeToDrop == InventorySlot.MULTI_SLOT) {
                        ...
                        if(((MetaObjectState)getState()).getMetaObjectManager().getObject(meta) != null &&
                                ((MetaObjectState)getState()).getMetaObjectManager().getObject(meta).isInventoryLocked(inventory)){
        ...
        if(!createDockRequests.isEmpty()){
            synchronized(createDockRequests){
                while(!createDockRequests.isEmpty()){
                    ...
                    Sendable snd = getState().getLocalAndRemoteObjectContainer().getUidObjectMap().get(d.core.uniqueIdentifierSegmentController);
...
//						diffCoreDocker.mul(dMatInv);
                        ...
                        Vector3i pPrim = new Vector3i(Math.round(prim.x), Math.round(prim.y), Math.round(prim.z));
...
//								d.docker.setActive(i < SegmentData.MAX_ORIENT);
                        ...
                        Vector3i relToPos = new Vector3i(Math.round(diffF.x)+SegmentData.SEG_HALF, Math.round(diffF.y)+SegmentData.SEG_HALF, Math.round(diffF.z)+SegmentData.SEG_HALF);
                        ...
                        s.setSegmentData(new SegmentDataIntArray(newShip.getState() instanceof ClientStateInterface));
                        ...
                        try {
                            ...
                            if(!s.pos.equals(addSecPos)){
                                ...
                                sA.setSegmentData(new SegmentDataIntArray(newShip.getState() instanceof ClientStateInterface));
                                sA.getSegmentData().setSegment(sA);
                                sA.getSegmentData().setInfoElementUnsynched(local.x, local.y, local.z, ElementKeyMap.RAIL_BLOCK_DOCKER,
                                        d.docker.getOrientation(), d.docker.isActive() ? (byte) 1 : (byte)0, true,
                                        sA.getAbsoluteIndex(local.x, local.y, local.z), newShip.getState().getUpdateTime());
                                ...
                                s.getSegmentData().setInfoElementUnsynched(local.x, local.y, local.z, ElementKeyMap.RAIL_BLOCK_DOCKER,
                                        d.docker.getOrientation(), d.docker.isActive() ? (byte) 1 : (byte)0, true,
                                        s.getAbsoluteIndex(local.x, local.y, local.z), newShip.getState().getUpdateTime());
                            }
                        ...
                        ((GameServerState)getState()).getController().getSynchController().addNewSynchronizedObjectQueued(newShip);
                    }else{
    ...
    public String toString() {
        return "PlS[" + getName() + " " + (starmadeName != null ? "[" + starmadeName + "]" + (upgradedAccount ? "*" : "") : "") + "; id(" + getId() + ")(" + clientId + ")" + "f(" + getFactionId() + ")]";
    }
    ...
    public void updateToFullNetworkObject() {
        ...
        if(cargoInventoryBlock != null){
            getNetworkObject().cargoInventoryChange.add(new RemoteSegmentControllerBlock(cargoInventoryBlock, isOnServer()));
        }
    ...
    private void dieOnServer(Damager from) {
        ...
        if (isInTutorial() || isInTestSector()) {
            ...
            Sector defaultSec;
            try {
                defaultSec = ((GameServerState) getState()).getUniverse().getSector(
                        new Vector3i(
        ...
        Faction faction = ((GameServerState) getState()).getFactionManager().getFaction(getFactionController().getFactionId());
        if (faction != null) {
            if (!isFactionPointProtected()) {
                ...
                ((GameServerState) getState()).getController().broadcastMessage(Lng.astr("Player %s died\nbut didn't lose FP.\n(Admin FP loss protected)", getName()), ServerMessage.MESSAGE_TYPE_ERROR);
            }
        }
...
//	}
    ...
    public boolean spawnCrew() {
        if (getPlayerAiManager().getCrew().size() < 5) {
            CreatureSpawn s;
            try {
                s = new CreatureSpawn(new Vector3i(this.getCurrentSector()), new Transform(this
                        .getFirstControlledTransformable().getWorldTransform()), "NoName", CreatureType.CHARACTER) {
                    @Override
                    public void initAI(
    ...
    private void executeSimpleCommand(SimpleCommand<?> simpleCommand) {
        ...
        if (command >= SimplePlayerCommands.values().length || command < 0) {
            sendServerMessage(new ServerMessage(Lng.astr("Unknown command: %s",  command), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
        } else {
            ...
            switch (simplePlayerCommands) {
                    ...
                    if (isInTutorial()) {
                        ...
                        try {
                            if (spawnData.preSpecialSector == null) {
                                originalSec = ((GameServerState) getState()).getUniverse().getSector(
                                        new Vector3i(
                                                (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_X.getCurrentState(),
                                                (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Y.getCurrentState(),
                                                (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Z.getCurrentState()));
                            } else {
                                originalSec = ((GameServerState) getState()).getUniverse().getSector(spawnData.preSpecialSector);
                            }
                ...
                case DESTROY_TUTORIAL_ENTITY:
                    ...
                    if (isInTutorial()) {
                        ...
                        ((GameServerState) getState()).getController().broadcastMessage(Lng.astr("Possible hacking attempt\nby %s!",  this), ServerMessage.MESSAGE_TYPE_ERROR);
                    }
                ...
                case PUT_ON_HELMET:
                    ...
                    System.err.println("HELMETCOMMAND: BuildSlot: " + getSelectedBuildSlot() + "; cur: " + getHelmetSlot());
                    if (System.currentTimeMillis() - lastHelmetCommand > 1000) {
                        if (getSelectedBuildSlot() == getHelmetSlot() && getHelmetSlot() >= 0 && getInventory().getType(getHelmetSlot()) == MetaObjectType.HELMET.type) {
                ...
                case REPAIR_STATION: {
                    ...
                    Sendable s = getState().getLocalAndRemoteObjectContainer().getLocalUpdatableObjects().get(id);
                    ...
                    if (s != null && s instanceof SpaceStation) {
                        ...
                        if (getCredits() >= price) {
                            ...
                            sendServerMessage(new ServerMessage(Lng.astr("Not enough credits!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                        }
                ...
                case FAILED_TO_JOIN_CHAT_INVALLID_PASSWD: {
                    getPlayerChannelManager().handleFailedJoinInvalidPassword((String) simpleCommand.getArgs()[0]);
                    break;
                }
...
//				}
                ...
                case SPAWN_SHOPKEEP: {
                    ...
                    if (sendable != null && sendable instanceof ShopSpaceStation) {
                        ...
                        t.setIdentity();
                        t.origin.set(p);
                        CreatureSpawn s = new CreatureSpawn(new Vector3i(getCurrentSector()), t, "ShopKeep", CreatureType.CHARACTER) {
                            @Override
                            public void initAI(
                                    AIGameCreatureConfiguration<?, ?> aiConfiguration) {
                                try {
                ...
                case ADD_BLUEPRINT_META_ALL: {
                    ...
                    MetaObject metaObject = ((GameServerState) getState()).getMetaObjectManager().getObject(metaId);
...
//					System.err.println("Faction Verification for "+this+": "+cFid+"; "+getFactionId());
                ...
                case SET_FREE_WARP_TARGET: {
                    ...
                    Sendable sendable = getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(entityId);
                ...
                case SPAWN_BLUEPRINT_META: {
                    ...
                    Sendable sendable = getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(invHolder);
                ...
                case ADD_BLOCK_STORAGE_META_SINGLE: {
                    ...
                    Sendable s = getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(invHolderId);
                ...
                case GET_BLOCK_STORAGE_META_SINGLE: {
                    ...
                    if (metaObject != null && metaObject instanceof BlockStorageMetaItem) {
                        ...
                        if (count > 0 && inv.canPutIn(invType, count)) {
                            ...
                            sendServerMessage(new ServerMessage(Lng.astr("Requested items don't fit in inventory!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                        }
                    }
                        ...
                        for(short type : ElementKeyMap.keySet){
                            ...
                            if((count = bb.storage.get(type)) > 0){
                                ...
                                if (count > 0 && inv.canPutIn(invType, count)) {
                                    ...
                                    sendServerMessage(new ServerMessage(Lng.astr("Requested items don't fully fit in inventory!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                                    break;
                                }
                            }
                        }
                ...
                case REQUEST_BLUEPRINT_ITEM_LIST: {
                    ...
                    try {
                        ...
                        getNetworkObject().blockCountMapBuffer.add(new RemoteBlockCountMap(blueprint.getElementCountMapWithChilds(), isOnServer()));
...
//			cubatomInventory.fromTagStructure(struct[10]);
        ...
        if (struct.length >= 22 && struct[20].getType() != Type.FINISH) {
            ...
            Tag.listFromTagStructSP(scanHistory, struct[20], new ListSpawnObjectCallback<ScanData>() {
...
//			assert(false):scanHistory;
        ...
        if (struct.length >= 25 && struct[23].getType() != Type.FINISH) {
            ...
            Tag.listFromTagStructSPElimDouble(savedCoordinates, struct[23], new ListSpawnObjectCallback<SavedCoordinate>() {
...
//		}
    ...
    public Tag toTagStructure() {
        try {
            ((GameServerState)getState()).getDatabaseIndex().getTableManager().getPlayerTable().updateOrInsertPlayer(this);
...
//		System.err.println("["+(isOnServer()? "SERVER": "CLIENT")+"][TAG] LOGOUT SPAWNING POINT OF "+this+" WRITTEN: "+this.getLogoutSpawnPoint()+" SECTOR "+getLogoutSpawnSector());
        ...
        d[10] = (new Tag(Type.BYTE, null, isHasCreativeMode() ? (byte) 1 : (byte) 0));//cubatomInventory.toTagStructure();
        d[11] = new Tag(Type.STRING, null, getLastEnteredEntity() != null ? getLastEnteredEntity() : "none");
        ...
        d[21] = isFactionPointProtected() ? new Tag(Type.BYTE, null, (byte) 1) : new Tag(Type.BYTE, null, (byte) 0);
    ...
    public void callTutorialClient(String tutorialName) {
        ...
        if (((GameClientState) getState()).getController().getTutorialMode() != null) {
            ((GameClientState) getState()).getController().getTutorialMode().setCurrentMachine(tutorialName);
            assert (((GameClientState) getState()).getController().getTutorialMode().getMachine() != null) : tutorialName + " :::: " + ((GameClientState) getState()).getController().getTutorialMode().getMachineNames();
    ...
    private ShopInterface getClosestShopsInDistance() {
        ...
        for (ShopInterface shop : shopsInDistance) {
            ...
            pos.sub(((SimpleTransformableSendableObject) getControllerState().getUnits().iterator().next().playerControllable).getWorldTransform().origin);
        ...
        if (!killerIds.isEmpty()) {
            synchronized (killerIds) {
                while (!killerIds.isEmpty()) {
                    ...
                    Sendable sendableKiller = getState()
                            ...
                            if (getId() == sendableKiller.getId()) {
                                ...
                                if (sendableKiller instanceof Damager) {
                                    ...
                                    ((GameClientState) getState()).getController()
                                            .popupGameTextMessage(
                                                    Lng.str("%s\nkilled\n%s!",
                                                            ((Damager) sendableKiller).getName(),  getName()), 0);
                                } else {
            ...
            try {
                if (System.currentTimeMillis() - lastBlueprintSpawn > blueprintDelay) {
                    ...
                    if (e.metaItem >= 0) {
                        ...
                        MetaObject object = ((GameServerState) getState()).getMetaObjectManager().getObject(e.metaItem);
                        ...
                        try {
                            loadBluePrint = BluePrintController.active.loadBluePrint((GameServerState) getState(), e.catalogName,
                                    e.shipName, t, -1, e.factionId, getCurrentSector(), this.getUniqueIdentifier(), buffer, e.toDockOn,e.activeAI, new ChildStats(false));
                            loadBluePrint.checkOkName();

                            if(!((GameServerState)getState()).getGameConfig().isBBOk(loadBluePrint.en)){
                                sendServerMessagePlayerError(Lng.astr("Cannot spawn blueprint! Server doesn't allow ship dimension!\nAllowed: %s",  ((GameServerState) getState()).getGameConfig().toStringAllowedSize(loadBluePrint.en)));
                                continue;
                            }
                        ...
                        int stationsAllowed = (Integer) ServerConfig.ALLOWED_STATIONS_PER_SECTOR.getCurrentState();
                        ...
                        if (loadBluePrint != null && loadBluePrint.en.getType() == org.schema.game.server.data.blueprintnw.BlueprintType.SPACE_STATION) {
                            ...
                            for (Sendable s : state.getLocalAndRemoteObjectContainer().getLocalObjects().values()) {
                                if (s instanceof SpaceStation && ((SpaceStation) s).getSectorId() == getCurrentSectorId()) {
                                    stations++;
                                }
                            }
...
//								loadBluePrint.spawn(this.getCurrentSector(), true, new ChildStats(false));
                                ...
                                String msg = "[BLUEPRINT][BUY] " + this.getName() + " bought blueprint from metaItem: \"" + loadBluePrint.en.getName() + "\" as \"" + loadBluePrint.realName + "\"; Price: " + loadBluePrint.en.getPrice() + "; to sector: " + this.getCurrentSector() + " (loadTime: " + timeLoad + "ms, spawnTime: " + timeSpawn + "ms)";
                                ...
                                String msg = "[BLUEPRINT][BUY] " + this.getName() + " failed to buy blueprint from metaItem: \"" + e.catalogName + "\" as \"" + bbRname + "\"; to sector: " + this.getCurrentSector() + "; MetaItem goal for that blueprint differed";
                                System.err.println(msg);
                                LogUtil.log().fine(msg);
                            }
                        ...
                        SegmentControllerOutline loadBluePrint = BluePrintController.active.loadBluePrint((GameServerState) getState(), e.catalogName,
                                e.shipName, t, e.infiniteShop ? -1 : credits , e.factionId, getCurrentSector(), this.getUniqueIdentifier(), buffer, e.toDockOn, e.activeAI, new ChildStats(false));
...
//							loadBluePrint.spawn(loadBluePrint.spawnSectorId, loadBluePrint.checkProspectedBlockCount, new ChildStats(false));
                            ...
                            lastBlueprintSpawn = System.currentTimeMillis();
                            String msg = "[BLUEPRINT][BUY] " + this.getName() + " bought: \"" + loadBluePrint.en.getName() + "\" as \"" + loadBluePrint.realName + "\"; Price: " + loadBluePrint.en.getPrice() + "; to sector: " + this.getCurrentSector() + " (loadTime: " + timeLoad + "ms, spawnTime: " + timeSpawn + "ms)";
                    ...
                    sendServerMessage(new ServerMessage(Lng.astr("Cannot buy blueprint.\nSpam protection!\nWait %s seconds",  ((blueprintDelay / 1000) - (System.currentTimeMillis() - lastBlueprintSpawn) / 1000)), ServerMessage.MESSAGE_TYPE_ERROR, this.getId()));
                }
            } catch (NotEnoughCreditsException ex) {

                sendServerMessage(new ServerMessage(Lng.astr("Cannot buy blueprint.\nNot enough credits!"), ServerMessage.MESSAGE_TYPE_ERROR, this.getId()));
                ex.printStackTrace();
                ...
                if (isOnServer()) {
                    sendServerMessage(new ServerMessage(Lng.astr("Name already exists!"), ServerMessage.MESSAGE_TYPE_ERROR, this.getId()));
                }
                ex.printStackTrace();
            }
        }
        ...
        if (isOnServer()) {
            ...
            if (!p.recipeSellRequests.getReceiveBuffer().isEmpty()) {
                ...
                for (int slot : p.recipeSellRequests.getReceiveBuffer()) {
                    ...
                    MetaObject object = ((GameServerState) getState()).getMetaObjectManager().getObject(metaId);

                    if (object != null) {
                        boolean ok = false;
                        if (((Recipe) object).fixedPrice >= 0) {
                            int price = (int) (((Recipe) object).fixedPrice * ((Float) ServerConfig.RECIPE_REFUND_MULT.getCurrentState()).floatValue());
                            modCreditsServer(price);
                            ok = true;
                        } else {
                            int refund = (Integer) ServerConfig.RECIPE_BLOCK_COST.getCurrentState();

                            refund = (int) (refund * ((Float) ServerConfig.RECIPE_REFUND_MULT.getCurrentState()).floatValue());
                    ...
                    if (fixedRecipe.canAfford(this)) {
                        ...
                        if (fixedRecipe.costType == -1) {
                            ...
                            if (ElementKeyMap.exists(fixedRecipe.costType)) {
                                ...
                                sendServerMessage(new ServerMessage(Lng.astr("SERVER: recipe cost type unknown %s\n%s",  fixedRecipe.costType,  fixedRecipe.name), ServerMessage.MESSAGE_TYPE_ERROR, this.getId()));
                            }
                        ...
                        if (ok) {
                            ...
                            try {
                                ...
                                e.printStackTrace();
                                sendServerMessage(new ServerMessage(Lng.astr("SERVER: no slot free for recipe!"), ServerMessage.MESSAGE_TYPE_ERROR, this.getId()));
                            }
                        }
                        if (mod.size() > 0) {
            ...
            for (RemoteBlueprintPlayerRequest s : p.catalogPlayerHandleBuffer.getReceiveBuffer()) {
                ...
                if (bbR.save) {
                    ...
                    Sendable sendable = getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(toSaveShip);
                    if (sendable != null && sendable instanceof Ship || sendable instanceof SpaceStation) {
                        ...
                        if ((Integer) ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState() < 0 ||
                                getCatalog().getPersonalCatalog().size() < (Integer) ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState()) {

                            BluePrintWriteQueueElement e = new BluePrintWriteQueueElement(segmentController, entitySpawnName, bbR.classification, false);
                            bluePrintWriteQueue.add(e);
                        } else {
                            sendServerMessage(new ServerMessage(Lng.astr("Cannot save blueprint:\nout of slots: %s/%s!", getCatalog().getPersonalCatalog().size(),  ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState()), ServerMessage.MESSAGE_TYPE_ERROR, this.getId()));
                        }
    ...
    public void handleLocalKeyboardInput() {
        if (isClientOwnPlayer() && ((GameClientController)getState().getController()).isWindowActive()) {
            if (canClientPressKey()) {
                for (int i = 0; i < KeyboardMappings.remoteMappings.length; i++) {
                    KeyboardMappings.remoteMappings[i].setNTKeyDown(getNetworkObject().keyboardOfController, (ClientStateInterface) state);
                }
            }
        }
    }
    ...
    public void handleLocalMouseInput() {
        if (isClientOwnPlayer() && GameClientController.hasGraphics(state) && Display.isActive()) {
            ...
            if (Controller.getCamera() != null && Controller.getCamera().isStable()) {
                ...
                if(Controller.getCamera() instanceof InShipCamera && getState().getUpdateTime() - ((InShipCamera)Controller.getCamera()).getHelperCamera().lastInitialRecoil < Math.max(140, ((GameClientState)getState()).getPing())) {
...
//					System.err.println("SENDING FOR::: "+GlUtil.getForwardVector(new Vector3f(), t.basis));
    ...
    public void handleNormalShoppingFromNT(NetworkPlayer p) {
        ...
        if (p.buyBuffer.getReceiveBuffer().size() > 0 || p.sellBuffer.getReceiveBuffer().size() > 0) {
            ...
            if (closestShopsInDistance == null) {
                ServerMessage m = new ServerMessage(Lng.astr("Cannot buy!\nNo shop in range!"), ServerMessage.MESSAGE_TYPE_ERROR, this.getId());
            ...
            if (!closestShopsInDistance.getShoppingAddOn().hasPermission(this)) {
                ServerMessage m = new ServerMessage(Lng.astr("Server:\nNo permission to use shop!"), ServerMessage.MESSAGE_TYPE_ERROR, this.getId());
                sendServerMessage(m);
                return;
            }
            ...
            for (RemoteIntegerArray s : p.buyBuffer.getReceiveBuffer()) {
                ...
                if (closestShopsInDistance != null) {
                    ...
                    try {
                        ...
                        e1.printStackTrace();
                        ServerMessage m = new ServerMessage(Lng.astr("No space in inventory!"), ServerMessage.MESSAGE_TYPE_ERROR, this.getId());
                        sendServerMessage(m);
                    }
                } else {
            ...
            for (RemoteIntegerArray s : p.sellBuffer.getReceiveBuffer()) {
                ...
                if (!info.isShoppable()) {
                    ((GameServerState) getState()).getController().broadcastMessage(Lng.astr("WARNING\nPossible hacking:\nPlayer %s\ntried to sell forbidden item.", getName()) , ServerMessage.MESSAGE_TYPE_ERROR);
                }
                if (quantity <= 0) {
    ...
    private void handlePlayerInBlackHoleSystem() {
        ...
        if (sectorBlackHoleEffectStart < 0) {
            ...
            if (isClientOwnPlayer()) {
                ((GameClientState) getState()).getController().popupAlertTextMessage(Lng.str("WARNING\nSpace time disruption\ndetected!"), 0);
            }
        } else {
    ...
    public void handleRoundEndFromNT(NetworkPlayer pn) {
        if (!isOnServer()) {
            ...
            if (state.getPlayer() == this) {
                for (RemoteIntegerArray s : pn.roundEndBuffer
                        .getReceiveBuffer()) {
                    ...
                    if (lastKillId < 0) {
                        System.out
                                .println("[CLIENT][ROUNDEND] NOBODY HAD THE LAST KILL");
                    } else {
                        System.out.println("[CLIENT][ROUNDEND] ENTITY "
                                + lastKillId + " HAD THE LAST KILL");
                    }
                    ...
                    if (!contains) {
                        RoundEndMenu gameMenu = new RoundEndMenu(state, winner,
                                loser);
    ...
    public void handleServerHealthAndCheckAliveOnServer(float health, Damager from) {
        ...
        if (isSpawnProtected()) {
            ...
            if (System.currentTimeMillis() - lastSetProtectionMsg > 3000) {
                sendServerMessage(new ServerMessage(Lng.astr("You are not taking damage!\n(spawn protection %s / %s sec)",  (getSpawnProtectionTimeLeft()/1000),  getSpawnProtectionTimeSecsMax()) , ServerMessage.MESSAGE_TYPE_INFO, getId()));
            }
            return;
        }
...
//		System.err.println("[SERVER] " + this + ": NEW HEALTH IS " + health + "; from: " + from);
        if (health <= 0 && oldHealth > 0 && alive) {
            ...
            if (from != null) {
                ...
                if (r != null) {
                    responsible = Lng.str("Killer: %s (%s/%s HP left)",  fromName,  r.getHealth(),  r.getMaxHealth());
                }

            }
            LogUtil.log().fine("[DEATH] " + getName() + " has been killed by '" + responsible + "'; controllable: " + from);
            if (!suicide) {
                sendServerMessage(new ServerMessage(Lng.astr("%s has died!\n\n%s", this.getName(),  responsible), ServerMessage.MESSAGE_TYPE_WARNING));
            }
    ...
    public boolean isClientOwnPlayer() {
        ...
        return (((GameClientState) getState()).getPlayer() != null && ((GameClientState) getState())
                .getPlayer().getId() == this.getId());
    }
...
//	}
    ...
    public void updateNTJoystick() {
        if (isClientOwnPlayer()) {
            ...
            getNetworkObject().frontBackAxis.set((float) ((GameClientState) state).getController().getJoystickAxis(JoystickAxisMapping.FORWARD_BACK));

            getNetworkObject().upDownAxis.forceClientUpdates();
            getNetworkObject().upDownAxis.set((float) ((GameClientState) state).getController().getJoystickAxis(JoystickAxisMapping.UP_DOWN));

            getNetworkObject().rightLeftAxis.forceClientUpdates();
            getNetworkObject().rightLeftAxis.set((float) ((GameClientState) state).getController().getJoystickAxis(JoystickAxisMapping.RIGHT_LEFT));
        }
    }
    ...
    public void printControllerState() {
        ...
        for (ControllerStateUnit cu : getControllerState().getUnits()) {
            AbstractScene.infoList.add("+ " + i + " CONTR: " + getName()
                    + "): " + cu);
            i++;
        }
    }
    ...
    public void searchForLastEnteredEntity() {
        ...
        if (lastEnteredEntity == null || lastEnteredEntity.isEmpty() || !lastEnteredEntity.startsWith("ENTITY_")) {
            sendServerMessage(new ServerMessage(Lng.astr("Invalid last entered:\n", lastEnteredEntity == null ? "Not set" : lastEnteredEntity),
                    ServerMessage.MESSAGE_TYPE_ERROR, getId()));
            return;
        }
        ...
        try {
            ...
            if (lastEnteredEntity != null && !lastEnteredEntity.equals("none")) {
                try {
                    ...
                    if (byUID.isEmpty()) {
                        sendServerMessage(new ServerMessage(Lng.astr("Ship has not been found! (original name: %s)",  entity),
                                ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                    } else {
                        for (DatabaseEntry a : byUID) {
                            sendServerMessage(new ServerMessage(Lng.astr("Found ship in sector:\n(orig: %s)\n-> %s", entity,  a.sectorPos),
                                    ServerMessage.MESSAGE_TYPE_INFO, getId()));
                        }
                    }
                } catch (SQLException e) {
                    e.printStackTrace();
    ...
    public void sendSimpleCommand(SimplePlayerCommands command, Object... args) {
        getNetworkObject().simpleCommandQueue.add(new RemoteSimpelCommand(new SimplePlayerCommand(command, args), getNetworkObject()));
    }
    ...
    public void suicideOnClient() {
        ((GameClientState) getState()).getGlobalGameControlManager()
                ...
                .getPlayerIntercationManager().getInShipControlManager()
                .exitShip(true);
        ((GameClientState) getState()).getGlobalGameControlManager()
                .getIngameControlManager().getPlayerGameControlManager()
                .getPlayerIntercationManager().getInShipControlManager()
                ...
                if (ok || bypass) {
                    ...
                    if (((GameStateInterface) getState()).getGameState().allowedToSpawnBBShips(this, f)) {
                        try {
                            ...
                            if (bb.getType() != BlueprintType.SPACE_STATION || getCredits() >= ((GameStateInterface) getState()).getGameState().getStationCost() || inInfiniteShop) {

                                if (bb.getType() == BlueprintType.SPACE_STATION && !inInfiniteShop) {
                                    modCreditsServer(-((GameStateInterface) getState()).getGameState().getStationCost());
                                }

                                BlueprintMetaItem m = (BlueprintMetaItem) MetaObjectManager
...
//								}
                            ...
                            e.printStackTrace();
                            sendServerMessage(new ServerMessage(Lng.astr("Blueprint not found!\n%s",  giveMetaBlueprint), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                        } catch (NoSlotFreeException e) {
                            e.printStackTrace();
                            sendServerMessage(new ServerMessage(Lng.astr("Inventory is full!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
                        }
                    } else {
    ...
    private void onClientChannelCreatedOnServer() {
        for (ScanData d : scanHistory) {
            getClientChannel().getNetworkObject().scanDataUpdates.add(new RemoteScanData(d, isOnServer()));
        }
        for (SavedCoordinate d : savedCoordinates) {
            getClientChannel().getNetworkObject().savedCoordinates.add(new RemoteSavedCoordinate(d, isOnServer()));
        }

        ((GameServerState) getState()).getUniverse().getGalaxyManager().sendAllFtlDataTo(getClientChannel().getNetworkObject());

        ((GameServerState) getState()).getUniverse().getGalaxyManager().sendAllTradeStubsTo(getClientChannel().getNetworkObject());
                        ...
                        if (max == 0) {
                            ...
                            if (savedCoordinates.size() < max || dequeue.isRemoveFlag()) {
                                if (dequeue.isRemoveFlag()) {
                                    ...
                                    getClientChannel().getNetworkObject().savedCoordinates.add(new RemoteSavedCoordinate(dequeue, isOnServer()));
                                } else {
    ...
    public void requestCargoInventoryChange(SegmentPiece p) {
        if(p != null){
            getNetworkObject().cargoInventoryChange.add(new RemoteSegmentControllerBlock(new VoidUniqueSegmentPiece(p), getNetworkObject()));
        }else{
            VoidUniqueSegmentPiece v = new VoidUniqueSegmentPiece();
            v.uniqueIdentifierSegmentController = "NONE";
            getNetworkObject().cargoInventoryChange.add(new RemoteSegmentControllerBlock(v, getNetworkObject()));
        }
    }
    ...
    private Inventory getCargoInventoryIfActive() {
        if(isUseCargoInventory()){
            ...
            if(isInTutorial()){
                ...
                Sendable sendable = getState().getLocalAndRemoteObjectContainer().getUidObjectMap().get(cargoInventoryBlock.uniqueIdentifierSegmentController);
                ...
                if(sendable != null && sendable instanceof SegmentController){
                    ...
                    if(cargoInventoryBlock.getSegmentController().getSectorId() == getCurrentSectorId()){
                        ...
                        if(cargoInventoryBlock.getSegmentController() instanceof ManagedSegmentController<?> &&
                                (inventory = ((ManagedSegmentController<?>)cargoInventoryBlock.getSegmentController()).getManagerContainer().getInventory(cargoInventoryBlock.getAbsoluteIndex())) != null){
    ...
    public boolean isInventoryPersonalCargo(StashInventory inventory) {
        if(cargoInventoryBlock != null && inventory.getInventoryHolder() instanceof ManagerContainer<?>){
            ...
            return c.getSegmentController().getUniqueIdentifier().equals(cargoInventoryBlock.uniqueIdentifierSegmentController)
    ...
    public void handleReceivedBlockMsg(ManagerContainer<?> c, ServerMessage s) {
        assert(!isOnServer());
        GameClientState state = (GameClientState) getState();
        InventoryControllerManager inventoryControlManager = state.getGlobalGameControlManager().getIngameControlManager()
    ...
    public String toDetailedString() {
        return getName() + ", credits: " + getCredits()+"; Sector: "+getCurrentSector()+"; Invetory slots filled: "+inventory.getCountFilledSlots()+"; last login: "+getLastLogin()+"; last logout: "+getLastLogout()+"; Spawn data: "+spawnData;
    }
    ...
    public void readDatabase() throws SQLException {
        ...
        dbId = ((GameServerState)getState()).getDatabaseIndex().getTableManager().getPlayerTable().getPlayerId(this);
    }
    public void updateDatabase() throws SQLException {
        assert(isOnServer());
        ((GameServerState)getState()).getDatabaseIndex().getTableManager().getPlayerTable().getPlayerFactionAndPermission(this, offlinePermssion);
        ((GameServerState)getState()).getDatabaseIndex().getTableManager().getPlayerTable().updateOrInsertPlayer(this);
    }
    public long getDbId() {
        return dbId;
    ...
    public void onChangedFactionServer(FactionChange factionChange) {
        Faction from = ((GameServerState)getState()).getFactionManager().getFaction(factionChange.from);
