...
public class PlayerState extends AbstractOwnerState
    implements Sendable, DiskWritable, FogOfWarReceiver, RuleEntityContainer {
  ...
  public static Vector3i NO_WAYPOINT =
      new Vector3i(Integer.MIN_VALUE, Integer.MAX_VALUE, Integer.MIN_VALUE);
  ...
  private final ObjectArrayList<SavedCoordinate> savedCoordinates =
      new ObjectArrayList<SavedCoordinate>();
  private final ObjectArrayFIFOQueue<SavedCoordinate> savedCoordinatesToAdd =
      new ObjectArrayFIFOQueue<SavedCoordinate>();
  ...
  private ArrayList<BluePrintSpawnQueueElement> bluePrintSpawnQueue =
      new ArrayList<BluePrintSpawnQueueElement>();
  private ArrayList<BluePrintWriteQueueElement> bluePrintWriteQueue =
      new ArrayList<BluePrintWriteQueueElement>();
  ...
  private final ObjectArrayFIFOQueue<CreateDockRequest> createDockRequests =
      new ObjectArrayFIFOQueue<CreateDockRequest>();
  ...
  public PlayerState(StateInterface state) {
    ...
    setPersonalFactoryInventoryCapsule(
        new PersonalFactoryInventory(
            this, CAPSULE_INV, ElementKeyMap.FACTORY_CAPSULE_ASSEMBLER_ID));
    setPersonalFactoryInventoryMicro(
        new PersonalFactoryInventory(this, MICRO_INV, ElementKeyMap.FACTORY_MICRO_ASSEMBLER_ID));
    setPersonalFactoryInventoryMacroBlock(
        new PersonalFactoryInventory(this, MACRO_BLOCK_INV, ElementKeyMap.FACTORY_BASIC_ID));
  ...
  public Inventory getInventory() {
    ...
    } else if (DataUtils.isPlayerInAnyBuildSector(this)
        || (getFirstControlledTransformableWOExc() != null
            && getFirstControlledTransformableWOExc() instanceof SegmentController
            && ((SegmentController) getFirstControlledTransformableWOExc()).isVirtualBlueprint())) {
  ...
  public void announceKill(Damager killerEntity) {
    ...
    if (isOnServer()) {
      ...
      System.err.println(
          getState() + " " + this + " Announcing kill: " + killerEntity + " killed " + this);
      ...
      ((GameServerState) getState()).getGameState().announceKill(this, id);
  ...
  public void checkIfDiedOnServer() {
    ...
    if (!dieList.isEmpty()) {
      ...
      Sector sector = ((GameServerState) getState()).getUniverse().getSector(getCurrentSectorId());
      if (sector != null) {

        if (isSpawnProtected()) {
          sendServerMessage(
              new ServerMessage(
                  Lng.astr("No punishment: spawn protection"),
          ...
          int secsUntilPunish =
              (Integer) ServerConfig.PLAYER_DEATH_PUNISHMENT_TIME.getCurrentState();
          if (lastSpawnedThisSession == 0
              || System.currentTimeMillis() - lastSpawnedThisSession > secsUntilPunish * 1000) {
            if (cMult < 1f) {
              ...
              if (ServerConfig.PLAYER_DEATH_CREDIT_DROP.isOn()) {
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("You dropped %s credits!", lost),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
                ...
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("You lost %s credits!", lost),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
            ...
            if (ServerConfig.PLAYER_DEATH_BLOCK_PUNISHMENT.isOn()) {
              sendServerMessage(
                  new ServerMessage(
                      Lng.astr("Your inventory dropped into space!"),
                      ServerMessage.MESSAGE_TYPE_ERROR,
                      getId()));
            ...
            long s = (getState().getUpdateTime() - lastSpawnedThisSession) / 1000;
            sendServerMessage(
                new ServerMessage(
                    Lng.astr(
                        "No punishment: respawn protection still on!\n%s / %s seconds",
                        s, secsUntilPunish),
                    ServerMessage.MESSAGE_TYPE_ERROR,
                    getId()));
          }
          ...
          sendServerMessage(
              new ServerMessage(
                  Lng.astr("No punishment: Sector set to peace!"),
                  ServerMessage.MESSAGE_TYPE_ERROR,
                  getId()));
        }
      }
      lastDeathTime = getState().getUpdateTime();

      announceKill(from);

      synchronized (getState().getLocalAndRemoteObjectContainer().getLocalObjects()) {
        for (Sendable s :
            getState().getLocalAndRemoteObjectContainer().getLocalObjects().values()) {
          if (s instanceof AbstractCharacter<?>) {
            AbstractCharacter<?> p = (AbstractCharacter<?>) s;
            if (p.getOwnerState() == this) {
              //							if (p instanceof PlayerCharacter && ((PlayerCharacter)p).getClientOwnerId()
              // == this.clientId) {
              p.destroy(from);
              break;
  ...
  public boolean checkItemInReach(
      FreeItem item, Object2ObjectOpenHashMap<PlayerState, IntOpenHashSet> changedSlots) {
    assert (isOnServer());
    ...
    try {
      ...
      if (t != null) {
        boolean check = false;
        Sector s = ((GameServerState) state).getUniverse().getSector(getCurrentSectorId());
        if (t instanceof SegmentController) {
          ...
          ClosestRayResultCallback result =
              ((PhysicsExt) s.getPhysics())
                  .testRayCollisionPoint(
                      v, v2, false, null, (SegmentController) t, false, true, false);
          check = result.hasHit();
        } else {
        ...
        if (check) {
          // INSERTED CODE
          PlayerPickupFreeItemEvent event =
              new PlayerPickupFreeItemEvent(this, item, Event.Condition.PRE);
          ...
          if (item.getType() == FreeItem.CREDITS_TYPE) {
            ...
            sendServerMessage(
                new ServerMessage(
                    Lng.astr("Picked up %s credits", item.getCount()),
            ...
            if ((countMax =
                    getInventory().canPutInHowMuch(item.getType(), countMax, item.getMetaId()))
                > 0) {
              System.err.println(
                  "[SERVER][PLAYERSTATE] Picked up: Type "
                      + item.getType()
                      + "; Count "
                      + countMax
                      + "; Meta "
              ...
              int slot =
                  getInventory(null)
                      .incExistingOrNextFreeSlotWithoutException(
                          item.getType(), countMax, item.getMetaId());
              ...
              if (item.getType() > 0) {
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr(
                            "Picked up %s\n%s",
                            countMax, ElementKeyMap.getInfo(item.getType()).getName()),
                        ServerMessage.MESSAGE_TYPE_INFO,
                        this.getId()));
              } else {
                MetaObject object =
                    ((MetaObjectState) state).getMetaObjectManager().getObject(item.getMetaId());
                if (object != null) {
                  sendServerMessage(
                      new ServerMessage(
                          Lng.astr("Picked up %s", object.getName()),
                          ServerMessage.MESSAGE_TYPE_INFO,
                          this.getId()));
                }
              }
              ...
              if (System.currentTimeMillis() - lastInventoryFullMsgSent > 7000) {
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("Inventory full!"),
                        ServerMessage.MESSAGE_TYPE_WARNING,
                        this.getId()));
                lastInventoryFullMsgSent = System.currentTimeMillis();
              }
              // INSERTED CODE
              PlayerInventoryTooFullToPickupFreeItemEvent event2 =
                  new PlayerInventoryTooFullToPickupFreeItemEvent(this, item);
          ...
          if (item.getCount() > 0) {
            s.getRemoteSector()
                .addItem(item.getPos(), item.getType(), item.getMetaId(), item.getCount());
          }
          return true;
        }
      }
  ...
  public void cleanUpOnEntityDelete() {
    ...
    getControllerState().removeAllUnitsFromPlayer(this, true);
  ...
  public void destroyPersistent() {
    assert (isOnServer());
    String uName = "ENTITY_PLAYERSTATE_" + getName();
  ...
  public void initFromNetworkObject(NetworkObject o) {
    ...
    setName(p.playerName.get());

    personalSector = p.personalSector.getVector();
    testSector = p.testSector.getVector();
    dbId = getNetworkObject().dbId.getLong();
    setUpgradedAccount(getNetworkObject().upgradedAccount.get());
    ...
    ElementCollection.getPosFromIndex(
        getNetworkObject().sittingState.getLongArray()[1], sittingPos);
    ElementCollection.getPosFromIndex(
        getNetworkObject().sittingState.getLongArray()[2], sittingPosTo);
    ElementCollection.getPosFromIndex(
        getNetworkObject().sittingState.getLongArray()[3], sittingPosLegs);

    assert (getId() >= 0);
    assert (getClientId() >= 0);
    assert (getState().getId() >= 0);

    if (!isOnServer()) {
      lastSpawnedThisSession = getNetworkObject().lastSpawnedThisSession.getLong();
      for (int i = 0; i < getNetworkObject().cargoInventoryChange.getReceiveBuffer().size(); i++) {
        RemoteSegmentControllerBlock remoteSegmentControllerBlock =
            getNetworkObject().cargoInventoryChange.getReceiveBuffer().get(i);
        this.cargoInventoryBlock = remoteSegmentControllerBlock.get();
      }
      this.infiniteInventoryVolume = getNetworkObject().infiniteInventoryVolume.getBoolean();
      getNetworkObject().tint.getVector(getTint());
      setInvisibilityMode(p.invisibility.get());
      if (getClientId() == state.getId()) {
        GameClientState s = (GameClientState) state;

        s.setPlayer(this);
        System.out.println(
            "[PlayerState] Client successfully received player state "
                + getState()
                + ", owner: "
                + getClientId());
        //				clientRequestProxmitySectors();
      } else {
        // System.err.println("[PlayerState] Client received player state "+getState()+", owner:
        // "+getClientId());
      }
    }
    buildModePosition.initFromNetworkObject();
    // System.err.println("[PLAYER] "+getState()+" initializing data from network object
    // "+getId()+", credits "+getCredits());
  ...
  public void newNetworkObject() {
    setNetworkPlayerObject(new NetworkPlayer(getState(), this));
  }
  ...
  public void updateFromNetworkObject(NetworkObject o, int senderId) {
      ...
      if (senderId != 0 && senderId != clientId) {
        System.err.println(
            getState()
                + " "
                + this
                + " WARNING: Possible attempt to hack controls of another player. sender: "
                + senderId
                + " but was: "
                + clientId);
        System.err.println(getState() + " " + this + " debugging what was sent: " + o.lastDecoded);
      }
      getNetworkObject().camOrientation.getMatrix(remoteCam.basis);
    }
    for (int i = 0; i < getNetworkObject().cargoInventoryChange.getReceiveBuffer().size(); i++) {
      RemoteSegmentControllerBlock remoteSegmentControllerBlock =
          getNetworkObject().cargoInventoryChange.getReceiveBuffer().get(i);
      ...
      if (isOnServer()) {
        getNetworkObject().cargoInventoryChange.add(remoteSegmentControllerBlock);
      }
    }
    for (int i = 0; i < getNetworkObject().resetFowBuffer.getReceiveBuffer().size(); i++) {
      resetFowSystem.enqueue(
          getNetworkObject().resetFowBuffer.getReceiveBuffer().get(i).getVector());
    }
    ruleEntityManager.receive(getNetworkObject());
    if (!isOnServer()) {
      lastSpawnedThisSession = getNetworkObject().lastSpawnedThisSession.getLong();

      mineAutoArmSecs = getNetworkObject().mineArmTimer.getInt();

    } else {
      for (int i = 0; i < getNetworkObject().mineArmTimerRequests.getReceiveBuffer().size(); i++) {
        mineAutoArmSecs = getNetworkObject().mineArmTimerRequests.getReceiveBuffer().get(i);
      }
    }
    ...
    if (!isOnServer()) {
      this.infiniteInventoryVolume = getNetworkObject().infiniteInventoryVolume.getBoolean();
      this.setUseCargoInventory(getNetworkObject().useCargoMode.getBoolean());

      this.setHasCreativeMode(getNetworkObject().hasCreativeMode.getBoolean());
      if (!isClientOwnPlayer()) {
        this.setUseCreativeMode(getNetworkObject().useCreativeMode.getBoolean());
      }

      for (int i = 0; i < getNetworkObject().blockCountMapBuffer.getReceiveBuffer().size(); i++) {
        RemoteBlockCountMap remoteBlockCountMap =
            getNetworkObject().blockCountMapBuffer.getReceiveBuffer().get(i);
        if (isClientOwnPlayer()) {
          CatalogEntryPanel.currentRequestedBlockMap = remoteBlockCountMap.get();
        }
      }

      lastDeathNotSuicide = getNetworkObject().lastDeathNotSuicide.get();
      ...
      //						if(pc.isClientOwnObject()){
      //							pc.getAttachedPlayers().get(i).onVesselHit(this);
      //						}
      //					}
      //				}
      //			}
      this.health = p.health.getFloat();
      int old = getCurrentSectorId();
      if (p.sectorId.get() != currentSectorId) {
        //				System.err.println("[CLIENT] received playerstate sector change:
        // "+currentSectorId+"("+getCurrentSector()+") -> "+p.sectorId.get()+"
        // ("+p.sectorPos.getVector()+")");
        if (((GameClientState) getState()).getPlayer() == this) {
          ((GameClientState) getState()).flagWarped();
        }
      }
      setCurrentSector(p.sectorPos.getVector());
      setCurrentSectorId(p.sectorId.get());
      if (old != getCurrentSectorId() && isClientOwnPlayer()) {
        ((GameClientState) getState()).setFlagSectorChange(old);
      }
      ...
      for (int i = 0; i < p.messages.getReceiveBuffer().size(); i++) {
        ...
        ((GameClientState) getState()).getServerMessages().add(fM);
      }
    }

    getShipUploadController().handleUploadNT(senderId);

    getSkinUploadController().handleUploadNT(senderId);
    ...
    if (!isClientOwnPlayer()) {
      for (int i = 0; i < p.keyboardOfControllerBuffer.getReceiveBuffer().size(); i++) {
        keyboardEvents.add(p.keyboardOfControllerBuffer.getReceiveBuffer().getByte(i));
      }
    }
    if (isOnServer()) {
      for (int i = 0; i < p.mouseOfControllerBuffer.getReceiveBuffer().size(); i++) {
        // server has its seeds set by transmitted mouse input from client
        inputBasedSeed++;
        this.mouseEvents.add(p.mouseOfControllerBuffer.getReceiveBuffer().getByte(i));
      }
    }
    ...
    if (isOnServer()) {
      boolean befUse = this.isUseCreativeMode();
      this.setUseCreativeMode(getNetworkObject().useCreativeMode.getBoolean());
      if (befUse != this.isUseCreativeMode()) {
        flagSendUseCreative = true;
      }

      for (int i = 0; i < getNetworkObject().requestCargoMode.getReceiveBuffer().size(); i++) {
        int r = getNetworkObject().requestCargoMode.getReceiveBuffer().getInt(i);
        flagRequestCargoInv = r;
      }

      for (int i = 0; i < getNetworkObject().creatureSpawnBuffer.getReceiveBuffer().size(); i++) {
        RemoteCreatureSpawnRequest r =
            getNetworkObject().creatureSpawnBuffer.getReceiveBuffer().get(i);
        synchronized (requestedreatureSpawns) {
          requestedreatureSpawns.enqueue(r.get());
        }
      }

      for (int i = 0; i < getNetworkObject().simpleCommandQueue.getReceiveBuffer().size(); i++) {
        SimpleCommand<?> r = getNetworkObject().simpleCommandQueue.getReceiveBuffer().get(i).get();
        synchronized (simpleCommandQueue) {
          simpleCommandQueue.enqueue(r);
        }
      }

      for (int i = 0; i < getNetworkObject().creditsDropBuffer.getReceiveBuffer().size(); i++) {
        int credits = getNetworkObject().creditsDropBuffer.getReceiveBuffer().get(i);
        synchronized (toDrop) {
      ...
      if (getState()
          ...
          .getLocalObjects()
          .containsKey(p.aquiredTargetId.get())) {
        aquiredTarget =
            (SimpleTransformableSendableObject)
                getState()
                    .getLocalAndRemoteObjectContainer()
                    .getLocalObjects()
                    .get(p.aquiredTargetId.get());
        // INSERTED CODE
  ...
  public void updateToNetworkObject() {
    ...
    if (isOnServer()) {
      getNetworkObject().hasCreativeMode.set(isHasCreativeMode());
      if (flagSendUseCreative) {
        getNetworkObject().useCreativeMode.set(isUseCreativeMode(), true);
        flagSendUseCreative = false;
      }
      getNetworkObject().lastSpawnedThisSession.set(lastSpawnedThisSession);
      getNetworkObject().inputSeed.set(inputBasedSeed);
      getNetworkObject().useCargoMode.set(isUseCargoInventory());
      getNetworkObject().invisibility.set(invisibilityMode);
      getNetworkObject().id.set(getId());
      //			getNetworkObject().spawnPoint.set(getSpawnPoint());
      getNetworkObject().helmetSlot.set(getHelmetSlot());
      getNetworkObject().infiniteInventoryVolume.set(infiniteInventoryVolume);

      getNetworkObject().currentSectorType.set(currentSectorType.ordinal());
      getNetworkObject().credits.set(getCredits());
      getNetworkObject().tint.set(getTint());

      getNetworkObject().mineArmTimer.set(mineAutoArmSecs);

      getNetworkObject().health.set(getHealth());
      getNetworkObject().kills.set(kills);
      getNetworkObject().deaths.set(deaths);
      getNetworkObject().lastDeathNotSuicide.set(lastDeathNotSuicide);

      getNetworkObject().ping.set(ping);

      getNetworkObject().sectorId.set(currentSectorId);
      getNetworkObject().sectorPos.set(currentSector);
      ...
      assert (!changedSit
          || (getNetworkObject().sittingState.hasChanged() && getNetworkObject().isChanged()));

      getNetworkObject()
          .isAdminClient
          .set(((GameServerState) getState()).getController().isAdmin(getName()));
    }
    ...
    if (isClientOwnPlayer()) {
      getNetworkObject().useCreativeMode.set(isUseCreativeMode(), true);
      getNetworkObject().mouseSwitched.forceClientUpdates();
      getNetworkObject().mouseSwitched.set(EngineSettings.C_MOUSE_BUTTON_SWITCH.isOn());
      getNetworkObject().selectedEntityId.forceClientUpdates();
      getNetworkObject().selectedAITargetId.forceClientUpdates();
      getNetworkObject().aquiredTargetId.forceClientUpdates();
      SimpleTransformableSendableObject selectedEntity =
          ((GameClientState) getState())
              ...
              .getSelectedEntity();
      SimpleTransformableSendableObject selectedAITarget =
          ((GameClientState) getState())
              .getGlobalGameControlManager()
              .getIngameControlManager()
              .getPlayerGameControlManager()
      ...
      getNetworkObject().selectedEntityId.set(selectedEntity == null ? -1 : selectedEntity.getId());
      getNetworkObject()
          .selectedAITargetId
          .set(selectedAITarget == null ? -1 : selectedAITarget.getId());
      getNetworkObject().aquiredTargetId.set(aquiredTarget == null ? -1 : aquiredTarget.getId());
    }
  }
  ...
  public boolean controls(PlayerControllable controllable, Vector3i parameter) {
    return getControllerState().isOwnerControlling(controllable, parameter);
  }
  ...
  public void damage(float damage, Destroyable destroyable, Damager from) {
    ...
    if (!isGodMode() && isVulnerable() && isDamageable(from)) {
      if (damage > 0) {
        from.sendHitConfirm(Damager.CHARACTER);
      }
      handleServerHealthAndCheckAliveOnServer((Math.max(0, getHealth() - damage)), from);
    } else {
      if (from != null && getState().getUpdateTime() > lastSentHitDeniedMessage + 2000) {

        if (isGodMode()) {
          from.sendServerMessage(Lng.astr("Target in god mode"), ServerMessage.MESSAGE_TYPE_ERROR);
          lastSentHitDeniedMessage = getState().getUpdateTime();
        } else if (isSpawnProtected()) {
          from.sendServerMessage(
              Lng.astr("Target in spawn protection"), ServerMessage.MESSAGE_TYPE_ERROR);
          lastSentHitDeniedMessage = getState().getUpdateTime();
        }
      }
    }
  ...
  public void heal(float heal, Destroyable destroyable, Damager from) {
    if (!isGodMode() && isVulnerable()) {
      handleServerHealthAndCheckAliveOnServer((Math.min(getMaxHealth(), getHealth() + heal)), from);
    }
  }
    ...
    if (isOnServer() || !isClientOwnPlayer()) {
      ...
      if (Controller.getCamera() instanceof InShipCamera) {
        ...
        if (Controller.getCamera() != null
            && !(Controller.getCamera() instanceof ObjectViewerCamera)) {
  ...
  public boolean isHarvestingButton() {
    ...
    // FIXME: this will be wrong on server if mouse buttons are switched
    return isMouseButtonDown(EngineSettings.C_MOUSE_BUTTON_SWITCH.isOn() ? 0 : 1);
  }
  ...
  protected void onNoSlotFree(short type, int amount) {
    if (getState().getUpdateTime() - lastMessage > 1000) {
      sendServerMessage(
          new ServerMessage(
              Lng.astr("No empty slots\nin inventory!"),
              ServerMessage.MESSAGE_TYPE_ERROR,
              getId()));
      lastMessage = getState().getUpdateTime();
    }
  }
  ...
  public void sendHitConfirm(byte damageType) {
    if (isOnServer()) {
      getNetworkObject().hitNotifications.set(damageType);
      lastHitConfirm = getState().getUpdateTime();
    }
  }
  ...
  public boolean isVulnerable() {
    return !isInTutorial() && !isGodMode() && !isSpawnProtected();
  }
  ...
  public void updateLocal(Timer timer) {
    ...
    getShopsInDistance().clear();
    SimpleTransformableSendableObject firstControlledTransformable =
        getFirstControlledTransformableWOExc();
    if (firstControlledTransformable != null
        && firstControlledTransformable instanceof ShopperInterface) {
      getShopsInDistance()
          .addAll(((ShopperInterface) firstControlledTransformable).getShopsInDistance());
    }
    ...
    while (!isOnServer()
        && ((GameClientState) getState()).getController().getClientChannel() != null
        && !resetFowSystem.isEmpty()) {
      Vector3i dequeue = resetFowSystem.dequeue();
      System.err.println("[CLIENT] Received FOW reset for " + getName());
      ((GameClientState) getState())
          .getController()
          .getClientChannel()
          .getGalaxyManagerClient()
          .resetClientVisibilitySystem(dequeue);
    }

    if (!isClientOwnPlayer()) {
      if (Controller.getCamera() instanceof InShipCamera) {
        getNetworkObject().adjustMode.forceClientUpdates();
        getNetworkObject().adjustMode.set(((InShipCamera) Controller.getCamera()).isInAdjustMode());
      }
    }
    ...
    if (!isOnServer() && slotChangeUpdateDelay > 0.0F) {
      ...
      if (slotChangeUpdateDelay <= 0.0F) {
        getNetworkObject().shipControllerSlot.set(shipControllerSlotClient, true);
      }
    }
    ...
    if (isOnServer()) {
      if (forcedEnterSector != null) {
        if (getFirstControlledTransformableWOExc() == getAssingedPlayerCharacter()) {
          if (getSectorId() != forcedEnterSector.getSectorId()) {
            try {
              SectorSwitch s =
                  new SectorSwitch(
                      getFirstControlledTransformable(),
                      forcedEnterSector.pos,
                      SectorSwitch.TRANS_JUMP);
              s.execute((GameServerState) getState());
            } catch (PlayerControlledTransformableNotFound e) {
              e.printStackTrace();
            } catch (IOException e) {
              e.printStackTrace();
      ...
      if (forcedEnterUID != null
          && getAssingedPlayerCharacter() != null
          && getFirstControlledTransformableWOExc() == getAssingedPlayerCharacter()) {
        GameServerState state = ((GameServerState) getState());
        Sendable sendable =
            state.getLocalAndRemoteObjectContainer().getUidObjectMap().get(forcedEnterUID);
        if (sendable instanceof Ship && ((Ship) sendable).getSectorId() == getSectorId()) {
          Ship s = (Ship) sendable;
          SegmentPiece pointUnsave = s.getSegmentBuffer().getPointUnsave(Ship.core);

          if (pointUnsave != null) {
            getControllerState()
                .requestControlServerAndSend(
                    getAssingedPlayerCharacter(),
                    (PlayerControllable) pointUnsave.getSegment().getSegmentController(),
                    new Vector3i(),
                    pointUnsave.getAbsolutePos(new Vector3i()),
                    true);
            forcedEnterUID = null;
          }
        }
      ...
      if (timer.currentTime - lastHitConfirm > 370) {
        ...
        getNetworkObject().hitNotifications.set((byte) 0);
      }
      ...
      this.setClientHitNotifaction(getNetworkObject().hitNotifications.getByte());
    }
    if (flagRequestCargoInv >= 0) {
      if (flagRequestCargoInv == 1) {
        setUseCargoInventory(true);
        getCargoInventoryIfActive(); // will instantly turn it off if not valid
      } else {
        setUseCargoInventory(false);
      }
      flagRequestCargoInv = -1;
    }
    spawnData.updateLocal(timer);

    if (getClientChannel() != null && getClientChannel().isConnectionReady()) {
    ...
    if (isOnServer() && checkForOvercap && getAssingedPlayerCharacter() != null) {
      if (getPersonalInventory().isOverCapacity()) {
        BlockStorageMetaItem storage =
            (BlockStorageMetaItem)
                MetaObjectManager.instantiate(MetaObjectType.BLOCK_STORAGE, (short) -1, true);
        ...
        System.err.println(
            "[SERVER][PLAYERSTATE] "
                + this
                + " WAS OVER CAPACITY. ADDED INVENTORY ALL TO META ITEM");
        sendServerMessage(
            new ServerMessage(
                Lng.astr(
                    "Your inventory was over capacity.\n"
                        + "A block storage item has been created\n"
                        + "which contains all of your blocks.\n\n"
                        + "This is done to ensure the over capacity penalty wouldn't hit a player"
                        + " out of the blue.\n\n"
                        + "You can pull out blocks from there by right clicking on the item."),
                ServerMessage.MESSAGE_TYPE_DIALOG,
                getId()));
      }
    ...
    if (isClientOwnPlayer()) {
      if (Controller.getCamera() != null) {
        getNetworkObject().canRotate.forceClientUpdates();
        getNetworkObject().canRotate.set(Controller.getCamera().isStable());
      }
      if (tutorialCallClient != null) {
        if (tutorialCallClient.equals("TutorialBasics00Welcome")) {
          if (((GameClientState) getState()).getController().getTutorialMode() != null
              && ((GameClientState) getState())
                      .getController()
                      .getTutorialMode()
                      .getMachine()
                      .getFsm()
                      .getCurrentState()
                  instanceof TeleportToTutorialSector) {
      ...
      getNetworkObject().waypoint.forceClientUpdates();
      if (waypoint != null) {
        getNetworkObject().waypoint.set(waypoint);
      } else {
        getNetworkObject().waypoint.set(NO_WAYPOINT);
      }
    }
    ...
    if (isOnServer()) {
      ...
      if (getNetworkObject().isAdminClient.get()) {
    ...
    if (!requestedreatureSpawns.isEmpty()) {
      synchronized (requestedreatureSpawns) {
        while (!requestedreatureSpawns.isEmpty()) {
          ...
          if (isOnServer()) {
            GameServerState s = (GameServerState) getState();
            if (s.isAdmin(getName())) {
              try {
                dequeue.execute(s);
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("Spawing creature"), ServerMessage.MESSAGE_TYPE_INFO, getId()));
              } catch (IOException e) {
                e.printStackTrace();
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("[SERVER] Error spawning\nPlease send a report"),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
              }
    ...
    if (!queuedModifactions.isEmpty()) {
      synchronized (queuedModifactions) {
        while (!queuedModifactions.isEmpty()) {
          ...
          getNetworkObject()
              .getInventoryMultModBuffer()
              .add(new RemoteInventoryMultMod(dequeue, getNetworkObject()));
        }
      }
    }
    ...
    if (isOnServer()) {
      ...
      if (getHelmetSlot() >= 0
          && getInventory().getType(getHelmetSlot()) != MetaObjectType.HELMET.type) {
        setHelmetSlot(-1);
      }
      ...
      if (s.getUniverse().existsSector(currentSectorId)) {
        ...
        System.err.println(
            "[SERVER] Exception (CRITICAL) player "
                + this
                + " is in an unloaded sector ("
                + getCurrentSectorId()
                + ")"
                + " attempting reload of "
                + getCurrentSector());
        Sector sector;
        try {
          sector = s.getUniverse().getSector(getCurrentSector());

          if (sector != null) {
            System.err.println(
                "[SERVER] reloaded sector successfully. overwriting sector ids to "
                    + sector.getId());
            for (PlayerState p : s.getPlayerStatesByName().values()) {
              int oldInvalid = getCurrentSectorId();
              if (p.getCurrentSectorId() == oldInvalid) {
                System.err.println(
                    "[SERVER] reloaded sector successfully. overwriting sector id for " + p);
                p.setCurrentSectorId(sector.getId());
              }
            }
          } else {
      ...
      if (serverClient != null) {
        this.ping = (int) serverClient.getProcessor().getPingTime();
        if (serverClientFirst) {
          this.ip =
              serverClient
                  .getProcessor()
                  .getIp()
                  .substring(0, serverClient.getProcessor().getIp().indexOf(":"));
          long time = System.currentTimeMillis();
          this.getHosts().add(new PlayerInfoHistory(time, ip, serverClient.getStarmadeName()));

          while (getHosts().size()
              > (Integer) ServerConfig.PLAYER_HISTORY_BACKLOG.getCurrentState()) {
            getHosts().remove(0);
          }
      ...
      //			if(healthChanged){
      //				handleServerHealthAndCheckAlive(getHealth(), null);
      //				healthChanged = false;
      //			}
      //			getLogoutSpawnSector().set(currentSector);
      //			if(firstControlledTransformable != null){
      //
      //				getLogoutSpawnPoint().set(firstControlledTransformable.getWorldTransform().origin);
      //				for(ControllerStateUnit su : getControllerState().getUnits()){
      //					if(su.playerControllable == firstControlledTransformable && su.playerControllable
      // instanceof SegmentController && su.getParameter() != null){
      //						Vector3f p = new
      // Vector3f(su.getParameter().x-8,su.getParameter().y-8,su.getParameter().z-8);
      //						firstControlledTransformable.getWorldTransform().transform(p);
      //						getLogoutSpawnPoint().set(p);
      //					}
      //				}
      //			}
    ...
    if (!basicTutorialStarted
        && isOnServer()
        && Sector.isTutorialSector(getCurrentSector())
        && getAssingedPlayerCharacter() != null) {
    ...
    if (sectorChanged) {
      ...
      Vector3i beforeSystem = new Vector3i(getCurrentSystem());
      StellarSystem.getPosFromSector(getCurrentSector(), getCurrentSystem());

      if (isOnServer() && !isInvisibilityMode() && !beforeSystem.equals(getCurrentSystem())) {
        try {
          StellarSystem sSys =
              ((GameServerState) getState())
                  .getUniverse()
                  .getStellarSystemFromStellarPos(getCurrentSystem());
          if (sSys.getOwnerFaction() != 0 && sSys.getOwnerFaction() != getFactionId()) {
            Faction faction =
                ((GameServerState) getState())
                    .getFactionManager()
                    .getFaction(sSys.getOwnerFaction());
            if (faction != null) {
              // how is the owner's relation to this player
              RType relation =
                  ((GameServerState) getState())
                      .getFactionManager()
                      .getRelation(faction.getIdFaction(), getFactionId());

              switch (relation) {
                case ENEMY:
                  String msgs =
                      Lng.str(
                          "Our scanners picked up a HOSTILE\n"
                              + "signature in our territory.\n\n"
                              + "Origin System: %s %s.\n"
                              + "Send Scanner there to get exact\n"
                              + "positions!",
                          sSys.getName(), sSys.getPos());
                  Object[] msg =
                      Lng.astr(
                          "Our scanners picked up a HOSTILE\n"
                              + "signature in our territory.\n\n"
                  ...
                  faction.broadcastMessage(
                      msg, ServerMessage.MESSAGE_TYPE_WARNING, ((GameServerState) getState()));

                  FactionNewsPost o = new FactionNewsPost();
                  o.set(
                      faction.getIdFaction(),
                      Lng.str("Faction Auto Scanner"),
                      System.currentTimeMillis(),
                      Lng.str("Hostile signature in territory"),
                      msgs,
                      0);
                  ((GameServerState) getState()).getFactionManager().addNewsPostServer(o);
                  break;
                case FRIEND:
                  faction.broadcastMessage(
                      Lng.astr(
                          "Our scanners picked up a FRIENDLY\n"
                              + "signature in our territory.\n\n"
                              + "Origin System: %s %s.\n"
                              + "Send Scanner there to get exact\n"
                              + "positions!",
                ...
                case NEUTRAL:
                  faction.broadcastMessage(
                      Lng.astr(
                          "Our scanners picked up a NEUTRAL\n"
                              + "signature in our territory.\n\n"
                              + "Origin System: %s %s.\n"
                              + "Send Scanner there to get exact\n"
                              + "positions!",
    ...
    if (isOnServer()) {
      try {
        ...
        currentSectorType =
            ((GameServerState) getState())
                .getUniverse()
                .getStellarSystemFromStellarPos(currentSystem)
                .getSectorType(currentSector);

        assert (((GameServerState) getState())
                .getUniverse()
                .getStellarSystemFromStellarPos(currentSystem)
            == ((GameServerState) getState())
                .getUniverse()
                .getStellarSystemFromSecPos(currentSector));

        //				System.err.println("SERVER CENTER TYPE:
        // "+((GameServerState)getState()).getUniverse().getStellarSystemFromStellarPos(currentSystem).getCenterSectorType());
      } catch (IOException e) {
        e.printStackTrace();
      }
    }

    handleReceivedInputEvents();

    try {

      getSkinUploadController().updateLocal();

      getShipUploadController().updateLocal();

      getControllerState().update(timer);
      ...
      if (isOnServer()) {
        ((GameServerState) getState())
            .getController()
            .broadcastMessage(
                Lng.astr("[UPLOAD][ERROR] Critical error while extracting upload file!"),
                ServerMessage.MESSAGE_TYPE_ERROR);
      }
    } catch (Exception e) {
      e.printStackTrace();
    ...
    if (isOnServer()) {
      ...
      while (!serverToSendMessages.isEmpty()) {
        ...
        getNetworkObject().messages.add(new RemoteServerMessage(m, getNetworkObject()));
      }
      if (!creditModifications.isEmpty()) {
        ...
        if (newP.compareTo(BigInteger.valueOf(Long.MAX_VALUE))
            > 0) { // Detect overflow // if (newP > Integer.MAX_VALUE) {
          sendServerMessage(
              new ServerMessage(
                  Lng.astr("WARNING:\nCannot hold more credits!"),
                  ServerMessage.MESSAGE_TYPE_ERROR,
                  getId()));
    ...
    if (oldSectorId != getCurrentSectorId()) {
      ...
      // INSERTED CODE
      PlayerChangeSectorEvent event =
          new PlayerChangeSectorEvent(this, oldSectorId, getCurrentSectorId());
      StarLoader.fireEvent(event, isOnServer());
      ///
    ...
    for (int i = 0; i < wasMouseDown.length; i++) {
      wasMouseDown[i] = isMouseButtonDown(i);
    }
  ...
  private void handleDragDrop(DragDrop g) {
    try {
      ...
      Sendable sendable =
          getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(invHolderId);
        ...
        if (s != null) {
          ...
          if (type == InventorySlot.MULTI_SLOT && typeToDrop == InventorySlot.MULTI_SLOT) {
            ...
            inventory
                .getSlot(slot)
                .setMulti(
                    typeToDrop, Math.max(0, inventory.getCount(slot, typeToDrop) - countToDrop));
            ...
            int count =
                countToDrop > 0
                    ? Math.min(inventory.getCount(slot, (short) 0), countToDrop)
                    : inventory.getCount(slot, (short) 0);
            int meta = inventory.getMeta(slot);

            if (((MetaObjectState) getState()).getMetaObjectManager().getObject(meta) != null
                && ((MetaObjectState) getState())
                    .getMetaObjectManager()
                    .getObject(meta)
                    .isInventoryLocked(inventory)) {
              sendServerMessagePlayerError(Lng.astr("Can't drop active item!"));
              return;
            }
            System.err.println(
                "[SERVER][PLAYER] DROPPING NORMAL SLOT "
                    + ElementKeyMap.toString(type)
                    + ": "
                    + count);
    ...
    if (!createDockRequests.isEmpty()) {
      synchronized (createDockRequests) {
        while (!createDockRequests.isEmpty()) {
          ...
          if (((GameServerState) getState()).existsEntity(EntityType.SHIP, d.name)) {
            sendServerMessagePlayerError(
                Lng.astr("Cannot create dock! Name already exists! Choose a different name."));
            continue;
          }
          if (isInTestSector()) {
            sendServerMessagePlayerError(
                Lng.astr("Cannot create dock!\nAction not allowed in test sector."));
            continue;
          }
          ...
          Sendable snd =
              getState()
                  .getLocalAndRemoteObjectContainer()
                  .getUidObjectMap()
                  .get(d.core.uniqueIdentifierSegmentController);
          ...
          if (snd != null && snd instanceof SegmentController) {
            ...
            for (RailRelation r : dockOn.railController.next) {
              if (r.rail
                  .getAbsolutePos(new Vector3i())
                  .equals(d.rail.getAbsolutePos(new Vector3i()))) {
                sendServerMessagePlayerError(
            ...
            Ship newShip =
                EntityRequest.getNewShip(
                    getState(),
                    EntityType.SHIP.dbPrefix + d.name,
                    dockOn.getSectorId(),
                    d.name,
                    mat,
                    -2,
            ...
            Vector3f prim =
                new Vector3f(
                    Element.DIRECTIONSf[
                        Element.switchLeftRight(dAlgo.getOrientCubePrimaryOrientation())]);
            Vector3f sec =
                new Vector3f(
                    Element.DIRECTIONSf[
                        Element.switchLeftRight(dAlgo.getOrientCubeSecondaryOrientation())]);

            cMatInv.transform(prim);
            cMatInv.transform(sec);

            Vector3i pPrim =
                new Vector3i(Math.round(prim.x), Math.round(prim.y), Math.round(prim.z));
            Vector3i pSec = new Vector3i(Math.round(sec.x), Math.round(sec.y), Math.round(sec.z));
            ...
            System.err.println(
                "NEW DIR: "
                    + Element.getSideString(newPrim)
                    + "; "
                    + Element.getSideString(newSec));

            Oriencube nOrientCube = Oriencube.getOrientcube(newPrim, newSec);

            boolean found = false;
            for (int i = 0; i < SegmentData.FULL_ORIENT; i++) {
              if (BlockShapeAlgorithm.algorithms[6 - 1][i]
                  .getClass()
                  .equals(nOrientCube.getClass())) {
                ...
                assert (d.docker.getAlgorithm().getClass().equals(nOrientCube.getClass()))
                    : "Is "
                        + d.docker.getAlgorithm().getClass().getSimpleName()
                        + "; "
                        + nOrientCube.getClass().getSimpleName();
            ...
            Vector3i relToPos =
                new Vector3i(
                    Math.round(diffF.x) + SegmentData.SEG_HALF,
                    Math.round(diffF.y) + SegmentData.SEG_HALF,
                    Math.round(diffF.z) + SegmentData.SEG_HALF);
            ...
            Segment.getSegmentIndexFromSegmentElement(
                relToPos.x, relToPos.y, relToPos.z, addSecPos);
            ...
            s.setSegmentData(
                new SegmentDataIntArray(newShip.getState() instanceof ClientStateInterface));
            s.getSegmentData().setSegment(s);
            try {
              s.getSegmentData()
                  .setInfoElementUnsynched(
                      (byte) Ship.core.x,
                      (byte) Ship.core.y,
                      (byte) Ship.core.z,
                      ElementKeyMap.CORE_ID,
                      true,
                      s.getAbsoluteIndex((byte) 8, (byte) 8, (byte) 8),
                      newShip.getState().getUpdateTime());

              s.setLastChanged(System.currentTimeMillis());

              if (!s.pos.equals(addSecPos)) {
                System.err.println(
                    "[SERVER][CREATEDOCKING] adding additional segment for docker("
                        + relToPos
                        + "): "
                        + addSecPos);
                ...
                sA.getSegmentData()
                    .setInfoElementUnsynched(
                        ...
                        d.docker.getOrientation(),
                        d.docker.isActive() ? (byte) 1 : (byte) 0,
                        true,
                        sA.getAbsoluteIndex(local.x, local.y, local.z),
                        newShip.getState().getUpdateTime());
                ...
                s.getSegmentData()
                    .setInfoElementUnsynched(
                        ...
                        d.docker.isActive() ? (byte) 1 : (byte) 0,
                        true,
                        s.getAbsoluteIndex(local.x, local.y, local.z),
                        newShip.getState().getUpdateTime());
              }
            ...
            assert (d.docker.getType() == ElementKeyMap.RAIL_BLOCK_DOCKER
                && newShip.getSegmentBuffer().getPointUnsave(d.docker.getAbsoluteIndex()).getType()
                    == d.docker.getType());

            assert (d.rail.uniqueIdentifierSegmentController.equals(dockOn.getUniqueIdentifier()));
            assert (d.rail.getType() != 0
                && dockOn.getSegmentBuffer().getPointUnsave(d.rail.getAbsoluteIndex()).getType()
                    == d.rail.getType());

            RailRequest railRequest =
                newShip.railController.getRailRequest(
                    d.docker, d.rail, railContact, null, DockingPermission.PUBLIC);
            ...
            ((GameServerState) getState())
                .getController()
                .getSynchController()
                .addNewSynchronizedObjectQueued(newShip);
          } else {
  ...
  public String toString() {
    return "PlS["
        + getName()
        + " "
        + (starmadeName != null ? "[" + starmadeName + "]" + (upgradedAccount ? "*" : "") : "")
        + "; id("
        + getId()
        + ")("
        + clientId
  ...
  public AbstractCharacter<? extends AbstractOwnerState> getAbstractCharacterObject() {
    return getAssingedPlayerCharacter();
  }
  ...
  public void onFiredWeapon(Weapon object) {
    if (isSpawnProtected()) {
      sendServerMessage(
          new ServerMessage(
              Lng.astr("Spawn protection ended!\n(weapon fired)"),
              ServerMessage.MESSAGE_TYPE_ERROR,
              getId()));
  ...
  public void updateToFullNetworkObject() {
    ...
    getNetworkObject().id.set(getId());
    getNetworkObject().clientId.set(getClientId());
    getNetworkObject().playerName.set(name);
    getNetworkObject().credits.set(getCredits());
    getNetworkObject().health.set(getHealth());
    getNetworkObject().kills.set(kills);
    getNetworkObject().deaths.set(deaths);
    ...
    getNetworkObject().sectorId.set(currentSectorId);
    getNetworkObject().sectorPos.set(currentSector);
    getNetworkObject().upgradedAccount.set(isUpgradedAccount());
    getNetworkObject().tint.set(getTint());
    getNetworkObject().dbId.set(dbId);
    getNetworkObject().infiniteInventoryVolume.set(infiniteInventoryVolume);
    getNetworkObject().personalSector.set(personalSector);
    getNetworkObject().testSector.set(testSector);
    getNetworkObject().lastSpawnedThisSession.set(lastSpawnedThisSession);
    boolean changedSit = getNetworkObject().sittingState.getLongArray()[0] != sittingOnId;
    getNetworkObject().sittingState.set(0, sittingOnId);
    getNetworkObject().sittingState.set(1, ElementCollection.getIndex(sittingPos));
    ...
    if (cargoInventoryBlock != null) {
      getNetworkObject()
          .cargoInventoryChange
          .add(new RemoteSegmentControllerBlock(cargoInventoryBlock, isOnServer()));
    }
  ...
  private void dieOnServer(Damager from) {
      ...
      try {
        defaultSec =
            ((GameServerState) getState())
                ...
                .getSector(
                    new Vector3i(
                        (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_X.getCurrentState(),
                        (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Y.getCurrentState(),
                        (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Z.getCurrentState()));

        SectorSwitch s =
            new SectorSwitch(
                getFirstControlledTransformable(), defaultSec.pos, SectorSwitch.TRANS_JUMP);

        ((GameServerState) getState()).getSectorSwitches().add(s);
      } catch (IOException e) {
        e.printStackTrace();
      } catch (PlayerControlledTransformableNotFound e) {
        e.printStackTrace();
    ...
    Faction faction =
        ((GameServerState) getState())
            .getFactionManager()
            .getFaction(getFactionController().getFactionId());
    if (faction != null) {
      if (!isFactionPointProtected()) {

        try {
          Sector sector = ((GameServerState) getState()).getUniverse().getSector(currentSector);
          if (sector.isNoFPLoss()) {
            sendServerMessage(
                new ServerMessage(
                    Lng.astr(
                        "No Faction points lost!\n"
                            + "Sector set not to cost\n"
                            + "faction points on death."),
                    ServerMessage.MESSAGE_TYPE_INFO,
                    getId()));
          } else {
        ...
        System.err.println(
            "[SERVER] Player "
                + this
                + " didn't lose faction points. He is protected against faction point loss");
        ((GameServerState) getState())
            .getController()
            .broadcastMessage(
                Lng.astr(
                    "Player %s died\nbut didn't lose FP.\n(Admin FP loss protected)", getName()),
                ServerMessage.MESSAGE_TYPE_ERROR);
      }
    }

    if (from != this) {
      if (System.currentTimeMillis() - lastDeathNotSuicide
          > FactionPointsGeneralConfig.FACTION_POINT_DEATH_PROTECTION_MIN * 60 * 1000) {
        System.err.println(
            "[PLAYER][DEATH] last faction point death not suicide "
                + lastDeathNotSuicide
                + " set to "
                + System.currentTimeMillis());
  ...
  //			if(isOnServer()){
  //				this.requestedSectorProximityOnServer = new
  // Vector4i(a.get(0).get(),a.get(1).get(),a.get(2).get(),a.get(3).get());
  //			}else{
  //				this.proxmityReceived.add(new
  // int[]{a.get(0).get(),a.get(1).get(),a.get(2).get(),a.get(3).get(), a.get(4).get()});
  //			}
  //		}
  //	}
  ...
  public void dropCreditsIntoSpace(int amount) {
    ...
    if (amount > 0) {
      try {
        ...
        System.err.println(
            "[SERVER][PLAYERSTATE] CANNOT DROP CREDITS: no transformable for player found");
        e.printStackTrace();
      }
    }
  }
  ...
  public boolean spawnCrew() {
    if (getPlayerAiManager().getCrew().size() < 5) {
      CreatureSpawn s;
      try {
        s =
            new CreatureSpawn(
                new Vector3i(this.getCurrentSector()),
                new Transform(this.getFirstControlledTransformable().getWorldTransform()),
                "NoName",
              ...
              public void initAI(AIGameCreatureConfiguration<?, ?> aiConfiguration) {
                try {
                  ...
                  aiConfiguration
                      .get(Types.ORIGIN_X)
                      .switchSetting(String.valueOf(Integer.MIN_VALUE), false);
                  aiConfiguration
                      .get(Types.ORIGIN_Y)
                      .switchSetting(String.valueOf(Integer.MIN_VALUE), false);
                  aiConfiguration
                  ...
                  aiConfiguration
                      .get(Types.ORDER)
                      .switchSetting(AIGameCreatureConfiguration.BEHAVIOR_IDLING, false);
                ...
                getPlayerAiManager()
                    .addAI(new UnloadedAiContainer(aiConfiguration.getAiEntityState().getEntity()));
  ...
  public void invBackUpfromTag(Tag tag) {
    if (tag.getType() == Type.STRUCT) {
      ...
      System.err.println(
          "[SERVER] Player: "
              + this
              + " had backup inventory (used for instances) still. restored the backup!");
    }
  }
  ...
  public Tag invBackUpToTag() {
    if (mainInventoryBackup != null) {
      return new Tag(
          ...
          new Tag[] {
            mainInventoryBackup,
            capsuleRefinerInventoryBackup,
            microInventoryBackup,
            factoryInventoryBackup,
            FinishTag.INST
          });
    } else {
  ...
  private void executeSimpleCommand(SimpleCommand<?> simpleCommand) {
    ...
    if (command >= SimplePlayerCommands.values().length || command < 0) {
      sendServerMessage(
          new ServerMessage(
              Lng.astr("Unknown command: %s", command), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
    } else {
      ...
      switch (simplePlayerCommands) {
            ...
            try {
              if (spawnData.preSpecialSector == null) {
                ...
                originalSec =
                    ((GameServerState) getState())
                        .getUniverse()
                        .getSector(spawnData.preSpecialSector);
              }

              System.err.println(
                  "[SERVER] Ending tutorial. Respawning in sector: "
                      + originalSec
                      + ". IsDefault: "
                      + (spawnData.preSpecialSector == null));

              SectorSwitch s =
                  new SectorSwitch(
                      getFirstControlledTransformable(), originalSec.pos, SectorSwitch.TRANS_JUMP);
        ...
        case END_SHIPYARD_TEST:
          if (isInTestSector()) {
            ...
            try {
              ...
              System.err.println(
                  "[SERVER] Ending shipyard Test. Respawning in sector: "
                      + originalSec
                      + ". IsDefault: "
                      + (spawnData.preSpecialSector == null));
          ...
          if (isInTutorial()) {
            for (Sendable s :
              ...
              if (s instanceof SimpleTransformableSendableObject
                  && ((SimpleTransformableSendableObject) s).getUniqueIdentifier() != null
                  && ((SimpleTransformableSendableObject) s).getUniqueIdentifier().startsWith(uid)
                  && ((SimpleTransformableSendableObject) s).getSectorId()
                      == getCurrentSectorId()) {
            ...
            try {
              throw new IllegalArgumentException(
                  "Player " + this + " tried to destroy an entity while not in tutorial");
            } catch (Exception e) {
              e.printStackTrace();
            }
            ((GameServerState) getState())
                .getController()
                .broadcastMessage(
                    Lng.astr("Possible hacking attempt\nby %s!", this),
                    ServerMessage.MESSAGE_TYPE_ERROR);
          }
          ...
          for (int z = 1; z < VoidSystem.SYSTEM_SIZE; z += 2) {
            for (int y = 1; y < VoidSystem.SYSTEM_SIZE; y += 2) {
              for (int x = 1; x < VoidSystem.SYSTEM_SIZE; x += 2) {
                ...
                Sector sector =
                    ((GameServerState) getState()).getUniverse().getSectorWithoutLoading(sec);
                if (sector == null) {
                  // this sector is not loaded. that means it is ok to do tutorial there
                  try {
                    SectorUtil.importSectorFullDir(
                        "./data/prefabSectors",
                        "tutorial-v2.0.smsec",
                        sec,
                    ...
                    SectorSwitch sw =
                        new SectorSwitch(
                            getAssingedPlayerCharacter(), sec, SectorSwitch.TRANS_JUMP);
        ...
        case SIT_DOWN:
          {
            ...
            if (sittingOnId < 0) {
              ...
              System.err.println(
                  "[SERVER] " + this + " sitting down " + sittingPos + " " + sittingPosTo);
            }
            break;
          }
        case PUT_ON_HELMET:
          System.err.println(
              "HELMETCOMMAND: BuildSlot: " + getSelectedBuildSlot() + "; cur: " + getHelmetSlot());
          if (System.currentTimeMillis() - lastHelmetCommand > 1000) {
            if (getSelectedBuildSlot() == getHelmetSlot()
                && getHelmetSlot() >= 0
                && getInventory().getType(getHelmetSlot()) == MetaObjectType.HELMET.type) {
              ...
              setHelmetSlot(getSelectedBuildSlot());
            }
          }
        ...
        case REPAIR_STATION:
          {
            ...
            Sendable s =
                getState().getLocalAndRemoteObjectContainer().getLocalUpdatableObjects().get(id);
        ...
        case FAILED_TO_JOIN_CHAT_INVALLID_PASSWD:
          {
            getPlayerChannelManager()
                .handleFailedJoinInvalidPassword((String) simpleCommand.getArgs()[0]);
            break;
          }
        ...
        case REBOOT_STRUCTURE:
          {
            ...
            Sendable sendable =
                getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(id);
            if (sendable != null && sendable instanceof SegmentController) {

              if (fast) {
                long shopRebootCost =
                    ((SegmentController) sendable).getHpController().getShopRebootCost();
          ...
          //					if (sendable != null && sendable instanceof SegmentController) {
          //						if (fast) {
          //							long shopRebootCost = ((SegmentController)
          // sendable).getHpController().getShopArmorRepairCost();
          //							if (shopRebootCost <= getCredits()) {
          //								setCredits((int) (getCredits() - shopRebootCost));
          //							} else {
          //								sendServerMessagePlayerError(Lng.astr("Cannot afford repairs!"));
        ...
        case REBOOT_STRUCTURE_REQUEST_TIME:
          {
            ...
            Sendable sendable =
                getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(id);
            if (sendable != null && sendable instanceof SegmentController) {
              ((SegmentController) sendable)
                  .getHpController()
                  .setRebootTimeServerForced(
                      (((SegmentController) sendable).getHpController().getRebootTimeMS()));
            }
        ...
        case SPAWN_SHOPKEEP:
          {
            ...
            Sendable sendable =
                getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(id);
            if (sendable != null && sendable instanceof ShopSpaceStation) {
              ShopSpaceStation c = (ShopSpaceStation) sendable;
              for (AICreature<? extends AIPlayer> a : c.getAttachedAffinity()) {
                if (a.getFactionId() == FactionManager.TRAIDING_GUILD_ID) {
                  sendServerMessagePlayerError(Lng.astr("Shop already has a\nShopkeep!"));
                  return;
                }
              }
              Vector3f p = new Vector3f(0, -7, 0);
              ...
              CreatureSpawn s =
                  new CreatureSpawn(
                      new Vector3i(getCurrentSector()), t, "ShopKeep", CreatureType.CHARACTER) {
                    @Override
                    public void initAI(AIGameCreatureConfiguration<?, ?> aiConfiguration) {
                      try {
                        assert (aiConfiguration != null);
                        aiConfiguration.get(Types.ORIGIN_X).switchSetting("0", false);
                        aiConfiguration.get(Types.ORIGIN_Y).switchSetting("-7", false);
                        aiConfiguration.get(Types.ORIGIN_Z).switchSetting("0", false);
                        ...
                        aiConfiguration
                            .get(Types.ORDER)
                            .switchSetting(AIGameCreatureConfiguration.BEHAVIOR_ROAMING, false);

                        aiConfiguration
                            .getAiEntityState()
                            .getEntity()
                            .setFactionId(FactionManager.TRAIDING_GUILD_ID);
                      } catch (StateParameterNotFoundException e) {
                        e.printStackTrace();
                      }
                    }
        ...
        case SCAN:
          {
            ...
            if (s != null
                && s instanceof ManagedSegmentController<?>
                && ((ManagedSegmentController<?>) s).getManagerContainer()
                    instanceof ScannerManagerInterface) {

              ScannerManagerInterface scanInterface =
                  (ScannerManagerInterface) ((ManagedSegmentController<?>) s).getManagerContainer();

              scanInterface.getScanner().getElementManager().executeScanOnServer(this);
        ...
        case ADD_BLUEPRINT_META_ALL:
          {
            ...
            MetaObject metaObject =
                ((GameServerState) getState()).getMetaObjectManager().getObject(metaId);
            ...
            if (getInventory().isLockedInventory()) {
              ...
              try {
                ...
                if (bbl.getType() == BlueprintType.SPACE_STATION
                    && !ServerConfig.BLUEPRINT_SPAWNABLE_STATIONS.isOn()) {
                  sendServerMessagePlayerError(
                      Lng.astr("Server doesn't allow blueprint station spawning!"));
                } else if (bbl.getType() == BlueprintType.SHIP
                    && !ServerConfig.BLUEPRINT_SPAWNABLE_SHIPS.isOn()) {
                  sendServerMessagePlayerError(
                      Lng.astr(
                          "Server doesn't allow blueprint ship spawning!\nPlease use a shipyard!"));
                } else {
                ...
                sendServerMessagePlayerError(
                    Lng.astr("The blueprint of this item doesn't exist in the catalog!"));
              }
            }
            break;
          }
        case SEND_ALL_DESTINATIONS_OF_ENTITY:
          {
            ...
            if (s != null
                && s instanceof ManagedSegmentController<?>
                && ((ManagedSegmentController<?>) s).getManagerContainer()
                    instanceof ActivationManagerInterface) {
              ((SegmentController) s)
                  .railController
                  .getRoot()
                  .railController
                  .sendAdditionalBlueprintInfoToClient();
            }
            break;
          }
        case SET_SPAWN:
          {
            ...
            if (s != null && s instanceof SegmentController) {
              spawnData.setDeathSpawnTo(
                  (SegmentController) s, ElementCollection.getPosFromIndex(pos, new Vector3i()));
            }
        ...
        case VERIFY_FACTION_ID:
          {
            ...
            //					System.err.println("Faction Verification for "+this+": "+cFid+";
            // "+getFactionId());
            if (cFid != getFactionId()) {

              getNetworkObject().factionId.set(getFactionId(), true);
              getNetworkObject().factionId.setChanged(true);
              getNetworkObject().setChanged(true);
            }
            break;
          }
        case SET_FREE_WARP_TARGET:
          {
            ...
            Sendable sendable =
                getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(entityId);
            if (sendable != null
                && sendable instanceof ManagedSegmentController<?>
                && ((ManagedSegmentController<?>) sendable).getManagerContainer()
                    instanceof StationaryManagerContainer<?>) {
              StationaryManagerContainer<?> m =
                  (StationaryManagerContainer<?>)
                      ((ManagedSegmentController<?>) sendable).getManagerContainer();
              WarpgateCollectionManager c =
                  m.getWarpgate().getCollectionManagersMap().get(fromIndex);
              if (c != null) {
                String directUID =
                    FTLTable.DIRECT_PREFIX
                        + toX
                        + "_"
                        + toY
                        + "_"
                        ...
                        + DatabaseEntry.removePrefixWOException(
                            m.getSegmentController().getUniqueIdentifier());
                c.setDestination(directUID, new Vector3i(toX, toY, toZ));
                sendServerMessagePlayerInfo(
                    Lng.astr(
                        "Successfully set warp gate target to %s!", toX + ", " + toY + ", " + toZ));
              } else {
        ...
        case SPAWN_BLUEPRINT_META:
          {
            ...
            Vector3i invParam =
                new Vector3i(
                    (Integer) simpleCommand.getArgs()[3],
                    (Integer) simpleCommand.getArgs()[4],
                    (Integer) simpleCommand.getArgs()[5]);
            ...
            if (spawnOnId > 0) {
              ...
              Sendable sendable =
                  s.getLocalAndRemoteObjectContainer().getLocalObjects().get(spawnOnId);

              if (!(sendable instanceof SegmentController)) {
                sendServerMessagePlayerError(
                    Lng.astr("Cannot spawn on rail: selected object invalid"));
                break;
              }
              ...
              if (pointUnsave == null
                  || !pointUnsave.isValid()
                  || !pointUnsave.getInfo().isRailDockable()) {
                sendServerMessagePlayerError(
                    Lng.astr(
                        "Cannot spawn on rail: selected block is not a rail block to dock on"));
                break;
              }
              spawnOnRail = pointUnsave;
            }
            Sendable sendable =
                getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(invHolder);

            if (metaObject != null
                && metaObject instanceof BlueprintMetaItem
                && sendable != null
                && sendable instanceof InventoryHolder
                && ((InventoryHolder) sendable).getInventory(ElementCollection.getIndex(invParam))
                    != null) {
              ...
              if (!inv.conatainsMetaItem(metaObject)) {
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("ERROR: Inventory doesn't\ncontain blueprint!\n(Possible Hack)"),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
              } else {
                BlueprintMetaItem bb = (BlueprintMetaItem) metaObject;
                if (bb.metGoal()) {
                  ...
                  BluePrintSpawnQueueElement bluePrintSpawnQueueElement =
                      new BluePrintSpawnQueueElement(
                          bb.blueprintName, name, fid, true, false, false, spawnOnRail);
        ...
        case ADD_BLOCK_STORAGE_META_SINGLE:
          {
            ...
            if (metaObject != null && metaObject instanceof BlockStorageMetaItem) {
              ...
              short invType = type;
              if (ElementKeyMap.isValidType(type)
                  && ElementKeyMap.getInfoFast(type).getSlab() > 0) {
                invType = (short) ElementKeyMap.getInfoFast(type).getSourceReference();
              }
        ...
        case GET_BLOCK_STORAGE_META_SINGLE:
          {
            ...
            if (metaObject != null && metaObject instanceof BlockStorageMetaItem) {
              ...
              if (count > 0 && inv.canPutIn(invType, count)) {
                ...
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("Requested items don't fit in inventory!"),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
              }
            }
              ...
              for (short type : ElementKeyMap.keySet) {
                ...
                if ((count = bb.storage.get(type)) > 0) {
                  ...
                  if (count > 0 && inv.canPutIn(invType, count)) {
                    ...
                    sendServerMessage(
                        new ServerMessage(
                            Lng.astr("Requested items don't fully fit in inventory!"),
                            ServerMessage.MESSAGE_TYPE_ERROR,
                            getId()));
                    break;
                  }
        ...
        case REQUEST_BLUEPRINT_ITEM_LIST:
          {
            ...
            try {
              ...
              getNetworkObject()
                  .blockCountMapBuffer
                  .add(
                      new RemoteBlockCountMap(
                          blueprint.getElementCountMapWithChilds(), isOnServer()));
  ...
  public boolean isAdmin() {
    if (isOnServer()) {
      return ((GameServerState) getState()).isAdmin(getName());
    }
    return getNetworkObject().isAdminClient.getBoolean();
  }
  ...
  public boolean isInTutorial() {
    return Sector.isTutorialSector(getCurrentSector());
  }

  public boolean isInTestSector() {
    return getCurrentSector().equals(testSector);
  }

  public boolean isInPersonalSector() {
    return getCurrentSector().equals(personalSector);
  }
  ...
  public void fromTagStructure(Tag tag) {
    ...
    //		System.err.println("["+(isOnServer()? "SERVER": "CLIENT")+"][TAG] SPAWNING POINT OF "+this+"
    // READ: "+this.getSpawnPoint());
    ...
    if (struct.length >= 6
        && struct[4].getType() == Type.VECTOR3f
        && "lspawn".equals(struct[4].getName())) {
      this.spawnData.logoutSpawn.localPos.set((Vector3f) struct[4].getValue());
      //			System.err.println("[SERVER][PLAYER][TAG] read logout spawn point:
      // "+this.getLogoutSpawnPoint());
    } else {
      //			this.getLogoutSpawnPoint().set(getSpawnPoint());
    }

    if (struct.length >= 6
        && struct[5].getType() == Type.VECTOR3i
        && "lsector".equals(struct[5].getName())) {
      this.getCurrentSector().set((Vector3i) struct[5].getValue());
      this.spawnData.logoutSpawn.absoluteSector.set((Vector3i) struct[5].getValue());
      //			System.err.println("[SERVER][PLAYER][TAG] read logout spawn sector:
      // "+this.getLogoutSpawnSector());
    } else {
    ...
    if (struct.length >= 11) {
      ...
      if ("ips".equals(struct[9].getName())) {
        ...
        try {
          Tag.listFromTagStruct(
              PlayerInfoHistory.class.getConstructor(), getHosts(), (Tag[]) struct[9].getValue());
    ...
    if (struct.length > 10 && struct[10].getType() == Type.BYTE) {
      setHasCreativeMode(((Byte) struct[10].getValue()) != 0);
      //			cubatomInventory.fromTagStructure(struct[10]);
    }
    if (struct.length >= 13) {
      setLastEnteredEntity((String) struct[11].getValue());
    }
    ...
    if (struct.length >= 15 && struct[13].getType() == Type.INT) {
      setHelmetSlot((Integer) struct[13].getValue());
    }
    ...
    if (struct.length >= 17
        && struct[15].getType() != Type.FINISH
        && struct[15].getType() != Type.BYTE) {
    ...
    if (struct.length >= 22 && struct[20].getType() != Type.FINISH) {
      ...
      Tag.listFromTagStructSP(
          scanHistory,
          struct[20],
          new ListSpawnObjectCallback<ScanData>() {

            @Override
            public ScanData get(Object fromValue) {
    ...
    if (struct.length >= 23 && struct[21].getType() != Type.FINISH) {
      setFactionPointProtected(((Byte) struct[21].getValue()).byteValue() == (byte) 1);
    }
    ...
    if (struct.length >= 25 && struct[23].getType() != Type.FINISH) {
      ...
      Tag.listFromTagStructSPElimDouble(
          savedCoordinates,
          struct[23],
          new ListSpawnObjectCallback<SavedCoordinate>() {
  ...
  public boolean isControllingCore(ManagedUsableSegmentController<?> c) {
    if (c instanceof Ship) {
      return getControllerState().isControlling(this, c, Ship.core);
    } else {
      return false;
    }
  }
  ...
  public Tag toTagStructure() {
    ...
    //		System.err.println("["+(isOnServer()? "SERVER": "CLIENT")+"][TAG] SPAWNING POINT OF "+this+"
    // WRITTEN: "+this.getSpawnPoint());
    d[1] = spawnData.toTagStructure();
    d[2] = getPersonalInventory().toTagStructure();
    d[3] =
        (new Tag(
            Type.BYTE, null,
            (byte) 0)); // removed (spawnpoint meta). can be used for somethign else

    //		if(getLogoutSpawnPoint().lengthSquared() == 0){
    //			((GameServerState)getState()).getController().setLogoutSpawnToPlayerPos(this);
    //		}
    //		System.err.println("["+(isOnServer()? "SERVER": "CLIENT")+"][TAG] LOGOUT SPAWNING POINT OF
    // "+this+" WRITTEN: "+this.getLogoutSpawnPoint()+" SECTOR "+getLogoutSpawnSector());
    d[4] =
        (new Tag(
            Type.BYTE, null,
            (byte) 0)); // removed (spawnpoint meta). can be used for somethign else
    ...
    d[10] =
        (new Tag(
            Type.BYTE,
            null,
            isHasCreativeMode() ? (byte) 1 : (byte) 0)); // cubatomInventory.toTagStructure();
    d[11] =
        new Tag(
            Type.STRING, null, getLastEnteredEntity() != null ? getLastEnteredEntity() : "none");
    ...
    d[12] =
        testSector == null
            ...
            : new Tag(
                Type.VECTOR3i, null, testSector); // playerKeyConfigurationManager.toTagStructure();

    d[13] = new Tag(Type.INT, null, getHelmetSlot());
    d[14] = playerAiManager.toTagStructure();

    d[15] =
        personalSector == null
            ? new Tag(Type.BYTE, null, (byte) 0)
            : new Tag(Type.VECTOR3i, null, personalSector);
    ...
    d[21] =
        isFactionPointProtected()
            ? new Tag(Type.BYTE, null, (byte) 1)
            : new Tag(Type.BYTE, null, (byte) 0);
    ...
    d[24] =
        new Tag(
            Type.BYTE, null,
            (byte)
                0); // preTutorialTransform == null ? new Tag(Type.BYTE, null, (byte) 0) : new
                    // Tag(Type.VECTOR3f, "pretutpoint", preTutorialTransform.origin);
    d[25] =
        new Tag(
            Type.BYTE, null,
            (byte)
                0); // preTutorialSector == null ? new Tag(Type.BYTE, null, (byte) 0) : new
    ...
    d[28] =
        cargoInventoryBlock != null
            ? cargoInventoryBlock.getUniqueTag()
            : new Tag(Type.BYTE, null, (byte) 0);
  ...
  public void callTutorialServer(String tutorialName) {
    assert (isOnServer());
    getNetworkObject().tutorialCalls.add(new RemoteString(tutorialName, true));
  }
  ...
  public void callTutorialClient(String tutorialName) {
    ...
    if (((GameClientState) getState()).getController().getTutorialMode() != null) {
      ((GameClientState) getState())
          .getController()
          .getTutorialMode()
          .setCurrentMachine(tutorialName);
      assert (((GameClientState) getState()).getController().getTutorialMode().getMachine() != null)
          : tutorialName
              + " :::: "
              + ((GameClientState) getState()).getController().getTutorialMode().getMachineNames();
      ((GameClientState) getState()).getController().getTutorialMode().getMachine().reset();
  ...
  private ShopInterface getClosestShopsInDistance() {
    ...
    for (ShopInterface shop : shopsInDistance) {
      ...
      pos.sub(
          ((SimpleTransformableSendableObject)
                  getControllerState().getUnits().iterator().next().playerControllable)
              .getWorldTransform()
              .origin);
  ...
  public int getCurrentShipControllerSlot() {
    ...
    return getNetworkObject().shipControllerSlot.get();
  }
  ...
  public void setCurrentShipControllerSlot(byte i, float delay) {
    ...
    if (delay == 0) {
      getNetworkObject().shipControllerSlot.set(i, true);
    }
  ...
  public SimpleTransformableSendableObject getFirstControlledTransformable()
      throws PlayerControlledTransformableNotFound {
    for (ControllerStateUnit c : getControllerState().getUnits()) {
      if (c.playerState.equals(this)
          && c.playerControllable instanceof SimpleTransformableSendableObject) {
        SimpleTransformableSendableObject obj =
            (SimpleTransformableSendableObject) c.playerControllable;
        return obj;
      }
    }
    ...
    if (!killerIds.isEmpty()) {
      synchronized (killerIds) {
        while (!killerIds.isEmpty()) {
          ...
          Sendable sendableKiller =
              getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(killerId);
          ...
          if (!isOnServer()) {
            ...
            if (isClientOwnPlayer()) {
              ((GameClientState) getState()).setCurrentPlayerObject(null);
              //							if(getAbstractCharacterObject() != null){
              //								getAbstractCharacterObject().neverSpawnedBefore = true;
              //							}
            }
            if (sendableKiller != null) {
              if (getId() == sendableKiller.getId()) {
                message = Lng.str("Cause of last Death:\nYou committed suicide.");
                ((GameClientState) getState())
                    .getController()
                    .popupGameTextMessage(Lng.str("%s\ncommitted suicide.", getName()), 0);

              } else {
                if (sendableKiller instanceof Damager) {
                  message =
                      "Cause of last Death:\n"
                          + ((Damager) sendableKiller).getName()
                          + " killed you";
                  ((GameClientState) getState())
                      .getController()
                      .popupGameTextMessage(
                          Lng.str(
                              "%s\nkilled\n%s!", ((Damager) sendableKiller).getName(), getName()),
                          0);
                } else {
              ...
              ((GameClientState) getState())
                  .getController()
                  .popupGameTextMessage(
                      Lng.str("Unkown Object(%s) killed\n%s!", killerId, getName()), 0);
            }
            ...
            System.err.println(
                "[SERVER][PLAYER] "
                    + this
                    + " received suicide request: killer: "
                    + sendableKiller);
  ...
  public void enqueueClientBlueprintToWrite(
      SegmentController ship, String name, BlueprintClassification classification) {
    BluePrintWriteQueueElement e = new BluePrintWriteQueueElement(ship, name, classification, true);
    bluePrintWriteQueue.add(e);
  }

  public void handleBluePrintQueuesClient()
      throws EntityNotFountException, IOException, EntityAlreadyExistsException {
    for (int i = 0; i < bluePrintWriteQueue.size(); i++) {
      ...
      if (!e.requestedAdditionalBlueprintData) {
        ((SendableSegmentController) e.segmentController)
            .getNetworkObject()
            .additionalBlueprintData
            .set(false, true);
        e.requestedAdditionalBlueprintData = true;
        sendSimpleCommand(
            SimplePlayerCommands.SEND_ALL_DESTINATIONS_OF_ENTITY, e.segmentController.getId());
        System.err.println(
            "[CLIENT][PLAYER][BLUEPRINT] "
                + this
                + " REQUESTED EXTRA INFO FOR LOCAL BLUEPRINT "
                + e.segmentController);
      } else if (((SendableSegmentController) e.segmentController)
          .railController
          .getRoot()
          .railController
          .isAllAdditionalBlueprintInfoReceived()) {
  ...
  public void handleBluePrintQueuesServer()
      throws EntityNotFountException, IOException, EntityAlreadyExistsException {
    while (!bluePrintWriteQueue.isEmpty()) {
      BluePrintWriteQueueElement e = bluePrintWriteQueue.remove(0);
      try {
        ((CatalogState) getState()).getCatalogManager().writeEntryServer(e, this.getName());
        System.err.println(
            "[SERVER][PLAYER][BLUEPRINT] " + this + " SAVED BLUEPRINT " + e.segmentController);
        LogUtil.log().fine("[BLUEPRINT][SAVE] " + this.getName() + " saved: \"" + e.name + "\"");
      } catch (IOException ex) {
        ex.printStackTrace();
      }
    }
      ...
      try {
        if (System.currentTimeMillis() - lastBlueprintSpawn > blueprintDelay) {
          ...
          if (e.metaItem >= 0) {
            ...
            MetaObject object =
                ((GameServerState) getState()).getMetaObjectManager().getObject(e.metaItem);

            if (object == null) {
              System.err.println(
                  "[SERVER][BLUEPRINT][BUY] Exception: Meta Object Blueptinz not found: "
                      + e.metaItem);
              continue;
            }
            if (!(object instanceof BlueprintMetaItem)) {
              System.err.println(
                  "[SERVER][BLUEPRINT][BUY] Exception: Metaitem not a blueprint: " + object);
              continue;
            }
            ...
            try {
              loadBluePrint =
                  BluePrintController.active.loadBluePrint(
                      (GameServerState) getState(),
                      e.catalogName,
                      e.shipName,
                      t,
              ...
              if (!((GameServerState) getState()).getGameConfig().isBBOk(loadBluePrint.en)) {
                sendServerMessagePlayerError(
                    Lng.astr(
                        "Cannot spawn blueprint! Server doesn't allow ship dimension!\nAllowed: %s",
                        ((GameServerState) getState())
                            .getGameConfig()
                            .toStringAllowedSize(loadBluePrint.en)));
                continue;
              }
            ...
            int stationsAllowed =
                (Integer) ServerConfig.ALLOWED_STATIONS_PER_SECTOR.getCurrentState();
            boolean sectorOk = true;
            boolean spawningAllowed = true;

            int stations = 0;
            if (loadBluePrint != null
                && loadBluePrint.en.getType()
                    == org.schema.game.server.data.blueprintnw.BlueprintType.SPACE_STATION) {
              if (!ServerConfig.BLUEPRINT_SPAWNABLE_STATIONS.isOn()) {
                spawningAllowed = false;
              }
              for (Sendable s :
                  state.getLocalAndRemoteObjectContainer().getLocalObjects().values()) {
                if (s instanceof SpaceStation
                    && ((SpaceStation) s).getSectorId() == getCurrentSectorId()) {
                  stations++;
                }
              }
            ...
            if (loadBluePrint != null
                && loadBluePrint.en.getType()
                    == org.schema.game.server.data.blueprintnw.BlueprintType.SHIP) {
            ...
            if (!spawningAllowed) {
              ...
              if (loadBluePrint != null
                  && loadBluePrint.en.getElementCountMapWithChilds().equals(bb.goal)) {
                ...
                synchronized (((GameServerState) getState()).getBluePrintsToSpawn()) {
                  ((GameServerState) getState()).getBluePrintsToSpawn().add(loadBluePrint);
                }
                ...
                String msg =
                    "[BLUEPRINT][BUY] "
                        + this.getName()
                        + " bought blueprint from metaItem: \""
                        + loadBluePrint.en.getName()
                        + "\" as \""
                        + loadBluePrint.realName
                        + "\"; Price: "
                        + loadBluePrint.en.getPrice()
                        + "; to sector: "
                ...
                String msg =
                    "[BLUEPRINT][BUY] "
                        + this.getName()
                        + " failed to buy blueprint from metaItem: \""
                        + e.catalogName
                        + "\" as \""
                        + bbRname
                        + "\"; to sector: "
                        + this.getCurrentSector()
                        + "; MetaItem goal for that blueprint differed";
                System.err.println(msg);
                LogUtil.log().fine(msg);
              }
            ...
            if (loadBluePrint.en.getType()
              ...
              //							loadBluePrint.spawn(loadBluePrint.spawnSectorId,
              // loadBluePrint.checkProspectedBlockCount, new ChildStats(false));
          ...
          sendServerMessage(
              new ServerMessage(
                  Lng.astr(
                      "Cannot buy blueprint.\nSpam protection!\nWait %s seconds",
                      ((blueprintDelay / 1000)
                          - (System.currentTimeMillis() - lastBlueprintSpawn) / 1000)),
                  ServerMessage.MESSAGE_TYPE_ERROR,
                  this.getId()));
        }
    ...
    if (isOnServer()) {
        ...
        for (int slot : p.recipeSellRequests.getReceiveBuffer()) {
          ...
          if (object != null) {
            ...
            if (((Recipe) object).fixedPrice >= 0) {
              int price =
                  (int)
                      (((Recipe) object).fixedPrice
                          * ((Float) ServerConfig.RECIPE_REFUND_MULT.getCurrentState())
                              .floatValue());
              modCreditsServer(price);
              ok = true;
            } else {
              int refund = (Integer) ServerConfig.RECIPE_BLOCK_COST.getCurrentState();
      ...
      for (int index : p.fixedRecipeBuyRequests.getReceiveBuffer()) {
        if (index >= 0 && index < ElementKeyMap.fixedRecipes.recipes.size()) {
          ...
          if (fixedRecipe.canAfford(this)) {
            ...
            if (fixedRecipe.costType == -1) {
              ...
              if (ElementKeyMap.exists(fixedRecipe.costType)) {
                ...
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr(
                            "SERVER: recipe cost type unknown %s\n%s",
                            fixedRecipe.costType, fixedRecipe.name),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        this.getId()));
              }
            }
            if (ok) {
              ...
              try {
                ...
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("SERVER: no slot free for recipe!"),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        this.getId()));
              }
            }
            ...
            sendServerMessage(
                new ServerMessage(
                    Lng.astr("SERVER: Cannot afford recipe \n%s\n%s", index, fixedRecipe.name),
                    ServerMessage.MESSAGE_TYPE_ERROR,
                    this.getId()));
          }
        } else {
          sendServerMessage(
              new ServerMessage(
                  Lng.astr("SERVER: Invalid recipe request \n%s", index),
                  ServerMessage.MESSAGE_TYPE_ERROR,
                  this.getId()));
        }
      }
      for (int intType : p.recipeRequests.getReceiveBuffer()) {
        sendServerMessage(
            new ServerMessage(
                Lng.astr("Can no longer buy recipes...\nPlease use the crafting system!"),
                ServerMessage.MESSAGE_TYPE_ERROR,
                this.getId()));
      }
      ...
      for (RemoteBlueprintPlayerRequest s : p.catalogPlayerHandleBuffer.getReceiveBuffer()) {
        ...
        System.err.println(
            "[SERVER][PLAYER] RECEIVED SAVE BUY: " + catalogName + " " + entitySpawnName);
        if (bbR.save) {

          // SAVE

          Sendable sendable =
              getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(toSaveShip);
          if (sendable != null && sendable instanceof Ship || sendable instanceof SpaceStation) {
            ...
            if ((Integer) ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState() < 0
                || getCatalog().getPersonalCatalog().size()
                    < (Integer) ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState()) {

              BluePrintWriteQueueElement e =
                  new BluePrintWriteQueueElement(
                      segmentController, entitySpawnName, bbR.classification, false);
              bluePrintWriteQueue.add(e);
            } else {
              sendServerMessage(
                  new ServerMessage(
                      Lng.astr(
                          ...
                          getCatalog().getPersonalCatalog().size(),
                          ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState()),
                      ServerMessage.MESSAGE_TYPE_ERROR,
                      this.getId()));
            }
          ...
          if (bbR.directBuy) {
            ...
            if (o != null && o instanceof ShopperInterface) {
              ...
              if (!sh.getShopsInDistance().isEmpty()) {
                ...
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("Not near shop,\nOVERWRITTEN BY ADMINRIGHTS!"),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
              } else {
                sendServerMessage(
                    new ServerMessage(
                        Lng.astr("You are not\nnear a shop!"),
                        ServerMessage.MESSAGE_TYPE_ERROR,
                        getId()));
              }
              if (ok) {
                Faction f =
                    ((FactionState) getState()).getFactionManager().getFaction(getFactionId());

                if (((GameStateInterface) getState())
                    .getGameState()
                    .allowedToSpawnBBShips(this, f)) {

                  SegmentPiece spawnOnRail = bbR.getToSpawnOnRail(this, (GameServerState) state);

                  int fid = bbR.setOwnFaction ? getFactionId() : 0;
                  // LOAD
                  BluePrintSpawnQueueElement e =
                      new BluePrintSpawnQueueElement(
                          catalogName,
                          entitySpawnName,
                          fid,
  ...
  public boolean canClientPressKey() {
    ...
    if (lastCanKeyPressCheck != getState().getNumberOfUpdate()) {
      lastCanKeyPressCheckResult =
          !((GameClientState) getState())
                  .getGlobalGameControlManager()
                  .getIngameControlManager()
                  .getChatControlManager()
                  .isActive()
              && ((GameClientState) getState())
                  .getGlobalGameControlManager()
                  .getIngameControlManager()
  ...
  public void handleLocalKeyboardInput() {
    if (isClientOwnPlayer()
        && ((GameClientController) getState().getController()).isWindowActive()) {
      if (canClientPressKey()) {
        for (int i = 0; i < KeyboardMappings.remoteMappings.length; i++) {
          KeyboardMappings.remoteMappings[i].setNTKeyDown(
              getNetworkObject().keyboardOfController, (ClientStateInterface) state);
        }
      }
    }
  }
  ...
  public void handleLocalMouseInput() {
    if (isClientOwnPlayer() && GameClientController.hasGraphics(state) && Display.isActive()) {
      ...
      if (Controller.getCamera() != null && Controller.getCamera().isStable()) {
        ...
        if (Controller.getCamera() instanceof InShipCamera
            && getState().getUpdateTime()
                    - ((InShipCamera) Controller.getCamera()).getHelperCamera().lastInitialRecoil
                < Math.max(140, ((GameClientState) getState()).getPing())) {
        ...
        if (!getNetworkObject().camOrientation.equalsMatrix(t.basis)) {
          //					System.err.println("SENDING FOR::: "+GlUtil.getForwardVector(new Vector3f(),
          // t.basis));
          getNetworkObject().camOrientation.set(t.basis, true);
        }
      }
    }
  }
  ...
  public void handleNormalShoppingFromNT(NetworkPlayer p) {
    ...
    if (p.buyBuffer.getReceiveBuffer().size() > 0 || p.sellBuffer.getReceiveBuffer().size() > 0) {
      ...
      Object2ObjectOpenHashMap<ShopInterface, IntOpenHashSet> slotsShop =
          new Object2ObjectOpenHashMap<ShopInterface, IntOpenHashSet>();
      ...
      if (closestShopsInDistance == null) {
        ServerMessage m =
            new ServerMessage(
                ...
                ServerMessage.MESSAGE_TYPE_ERROR,
                this.getId());
        System.err.println("[SERVER] " + this + " No Shops in distance: " + getShopsInDistance());
        sendServerMessage(m);
        return;
      }
      ...
      for (RemoteIntegerArray s : p.buyBuffer.getReceiveBuffer()) {
        ...
        if (quantity <= 0) {
          System.err.println(
              "[SERVER] ERROR: invalid quantity Shopping Buy: "
                  + quantity
                  + " of "
                  + info.getName()
                  + " for "
        ...
        if (closestShopsInDistance != null) {
          //				totalPrice = closestShopsInDistance.getPriceBasedOfQuantity(info).canAfford(this,
          // quantity);
          ...
          try {
            closestShopsInDistance
                .getShoppingAddOn()
                .buy(this, type, quantity, closestShopsInDistance, slots, shopHash);
          } catch (NoSlotFreeException e1) {
            e1.printStackTrace();
            ServerMessage m =
                new ServerMessage(
                    Lng.astr("No space in inventory!"),
                    ServerMessage.MESSAGE_TYPE_ERROR,
                    this.getId());
            sendServerMessage(m);
          }
      ...
      for (RemoteIntegerArray s : p.sellBuffer.getReceiveBuffer()) {
        ...
        if (!info.isShoppable()) {
          ((GameServerState) getState())
              ...
              .broadcastMessage(
                  Lng.astr(
                      "WARNING\nPossible hacking:\nPlayer %s\ntried to sell forbidden item.",
                      getName()),
                  ServerMessage.MESSAGE_TYPE_ERROR);
        }
        if (quantity <= 0) {
          System.err.println(
              "[SERVER] ERROR: invalid quantity Shopping Sell: "
                  + quantity
                  + " of "
                  + info.getName()
                  + " for "
        ...
        System.err.println(
            "[SERVER] Executing Shopping Sell: "
                + quantity
                + " of "
                + info.getName()
                + " for "
        ...
        if (closestShopsInDistance != null) {
          ...
          try {
            closestShopsInDistance
                .getShoppingAddOn()
                .sell(this, (short) elementId, quantity, closestShopsInDistance, slots, shopHash);
          } catch (NoSlotFreeException e1) {
            e1.printStackTrace();
            ServerMessage m =
                new ServerMessage(
  ...
  private void handleReceivedInputEvents() {
    ...
    for (int i = 0; i < keyboardEvents.size(); i++) {
      byte b = keyboardEvents.getByte(i);

      boolean pressed = b > 0;
      int ordinal = Math.abs(b) - 1;
      KeyboardMappings m = KeyboardMappings.values()[ordinal];

      for (int k = 0; k < KeyboardMappings.remoteMappings.length; k++) {

        if (KeyboardMappings.remoteMappings[k] == m) {
          wasKeyDown[k] = pressed;
          break;
        }
      }

      getControllerState().handleKeyEvent(m);
    }

    keyboardEvents.clear();
    for (byte button : mouseEvents) {
      MouseEvent e = new MouseEvent();
      e.button = button;
      e.state = true;
      getControllerState().handleMouseEvent(e);
    }
    mouseEvents.clear();
  }
  ...
  public void handleServerHealthAndCheckAliveOnServer(float health, Damager from) {
    ...
    if (isSpawnProtected()) {
      ((GameStateInterface) getState()).getGameState().getSpawnProtectionSec();
      System.err.println(
          "[SERVER] player "
              + this
              + " was hit but is invulnerable from previous death "
              + getSpawnProtectionTimeLeft() / 1000
              + " / "
              + getSpawnProtectionTimeSecsMax()
              + " sec");
      if (System.currentTimeMillis() - lastSetProtectionMsg > 3000) {
        sendServerMessage(
            new ServerMessage(
                Lng.astr(
                    "You are not taking damage!\n(spawn protection %s / %s sec)",
                    (getSpawnProtectionTimeLeft() / 1000), getSpawnProtectionTimeSecsMax()),
                ServerMessage.MESSAGE_TYPE_INFO,
                getId()));
      }
      return;
    ...
    if (health <= 0 && oldHealth > 0 && alive) {
      ...
      if (from != null) {
        ...
        if (r != null) {
          responsible =
              Lng.str("Killer: %s (%s/%s HP left)", fromName, r.getHealth(), r.getMaxHealth());
        }
      }
      LogUtil.log()
          .fine(
  ...
  public boolean equals(Object o) {
    if (o != null && o instanceof PlayerState) {
      return getId() == ((PlayerState) o).getId();
    }
    return false;
  }
  ...
  private void hurtClientAnimation(Sendable vessel) {
    if (isClientOwnPlayer()) {
      ((GameClientState) getState()).getController().onHurt(this, vessel);
    }
  }
  ...
  public boolean isClientOwnPlayer() {
    ...
    return (((GameClientState) getState()).getPlayer() != null
        && ((GameClientState) getState()).getPlayer().getId() == this.getId());
  }
  ...
  //		//		System.err.println("["+(isOnServer()? "SERVER": "CLIENT")+"] SET PAWNING POINT OF "+this+"
  // TO: "+this.getSpawnPoint());
  //	}
    ...
    if (sector != null && sector.isProtected()) {
      ...
      for (int i = 0; i < attachedPlayers.size(); i++) {
        ...
        if (System.currentTimeMillis() - ps.lastSectorProtectedMsgSent > 5000) {
          ...
          ps.sendServerMessage(
              new ServerMessage(
                  Lng.astr("This Sector is Protected!"),
                  ServerMessage.MESSAGE_TYPE_WARNING,
                  ps.getId()));
        }
      }
  ...
  public boolean isKeyDownOrSticky(KeyboardMappings key) {
    if (isOnServer() || !isClientOwnPlayer()) {

      boolean controllerKeyDown = key.isNTKeyDown(getNetworkObject().keyboardOfController.get());

      boolean wKeyDown = wasKeyDown(key);

      boolean pressed = controllerKeyDown || wKeyDown;

      return pressed;
    } else {
      if (((GameClientController) getState().getController()).isWindowActive()) {
        if (canClientPressKey()) {
          return key.isDownOrSticky(getState());
        } else {
          return false;
        }
      }
  ...
  public void handleJoystickDir(Vector3f dir, Vector3f forward, Vector3f right, Vector3f up) {
    if (!isOnServer()) {
          ...
          float joystickAxis =
              (float)
                  ((GameClientState) state)
                      .getController()
                      .getJoystickAxis(JoystickAxisMapping.FORWARD_BACK);
        ...
        }
        {
          float joystickAxis =
              (float)
                  ((GameClientState) state)
                      .getController()
                      .getJoystickAxis(JoystickAxisMapping.RIGHT_LEFT);
        ...
        }
        {
          float joystickAxis =
              (float)
                  ((GameClientState) state)
                      .getController()
                      .getJoystickAxis(JoystickAxisMapping.UP_DOWN);
      ...
      {
        float joystickAxis = getNetworkObject().frontBackAxis.getFloat();
        Vector3f jForw = new Vector3f(forward);
        jForw.scale(joystickAxis);
        dir.add(jForw);
      }
      {
        float joystickAxis = getNetworkObject().rightLeftAxis.getFloat();
  ...
  public void updateNTJoystick() {
    if (isClientOwnPlayer()) {
      getNetworkObject().frontBackAxis.forceClientUpdates();
      getNetworkObject()
          .frontBackAxis
          .set(
              (float)
      ...
      getNetworkObject().upDownAxis.forceClientUpdates();
      getNetworkObject()
          .upDownAxis
          .set(
              (float)
      ...
      getNetworkObject().rightLeftAxis.forceClientUpdates();
      getNetworkObject()
          .rightLeftAxis
          .set(
              (float)
  ...
  public boolean wasMouseButtonDown(int i) {
    return (i >= 0 && i < wasMouseDown.length && wasMouseDown[i]);
  }

  public boolean wasMouseEventServer(int i) {
    return (i >= 0 && i < wasMouseEvent.length && wasMouseEvent[i]);
  }

  public boolean isMouseButtonDown(int i) {
    if (isOnServer() || !isClientOwnPlayer()) {
      boolean mouse = getNetworkObject().isMouseDown(i);
      return mouse || wasMouseEventServer(i);
    } else {
      if (((GameClientController) getState().getController()).isWindowActiveOutOfMenu()) {
        return ((GameClientController) state.getController()).isMouseButtonDown(i);
      }
    }
    return false;
  }
  ...
  public boolean isMouseSwitched() {
    if (isOnServer() || !isClientOwnPlayer()) {
      return getNetworkObject().mouseSwitched.get();
    } else {
      if (((GameClientController) getState().getController()).isWindowActiveOutOfMenu()) {
        //				System.err.println("OUT OF MENU "+outOfMenu());
        return EngineSettings.C_MOUSE_BUTTON_SWITCH.isOn();
      }
    }
    return false;
  }
  ...
  public void notifyRoundEnded(int winner, int loser, Damager lastKill) {
    ...
    RemoteIntegerArray a = new RemoteIntegerArray(3, getNetworkObject());
    a.set(0, winner);
    a.set(1, loser);
    if (lastKill != null) {
      a.set(2, -1);
    ...
    getNetworkObject().roundEndBuffer.add(a);
  }
  ...
  public void printControllerState() {
    int i = 0;
    for (ControllerStateUnit cu : getControllerState().getUnits()) {
      AbstractScene.infoList.add("+ " + i + " CONTR: " + getName() + "): " + cu);
      i++;
    }
  }
  ...
  public void searchForLastEnteredEntity() {
    ...
    if (lastEnteredEntity == null
        || lastEnteredEntity.isEmpty()
        || !lastEnteredEntity.startsWith("ENTITY_")) {
      sendServerMessage(
          new ServerMessage(
              Lng.astr(
                  "Invalid last entered:\n",
                  lastEnteredEntity == null ? "Not set" : lastEnteredEntity),
              ServerMessage.MESSAGE_TYPE_ERROR,
              getId()));
      return;
    }
    ...
    try {
      ...
      if (lastEnteredEntity != null && !lastEnteredEntity.equals("none")) {
        try {
          List<DatabaseEntry> byUID =
              state
                  .getDatabaseIndex()
                  .getTableManager()
                  .getEntityTable()
                  .getByUIDExact(DatabaseIndex.escape(entity), 20);

          if (byUID.isEmpty()) {
            sendServerMessage(
                new ServerMessage(
                    Lng.astr("Ship has not been found! (original name: %s)", entity),
                    ServerMessage.MESSAGE_TYPE_ERROR,
                    getId()));
          } else {
            for (DatabaseEntry a : byUID) {
              sendServerMessage(
                  new ServerMessage(
                      Lng.astr("Found ship in sector:\n(orig: %s)\n-> %s", entity, a.sectorPos),
                      ServerMessage.MESSAGE_TYPE_INFO,
                      getId()));
            }
          }
  ...
  private void sendKill(int killerId) {
    getNetworkObject().killedBuffer.add(killerId);
  }
  ...
  public void sendServerMessagePlayerError(Object[] msg) {
    serverToSendMessages.add(new ServerMessage(msg, ServerMessage.MESSAGE_TYPE_ERROR, getId()));
  }

  public void sendServerMessagePlayerInfo(Object[] msg) {
    serverToSendMessages.add(new ServerMessage(msg, ServerMessage.MESSAGE_TYPE_INFO, getId()));
  }

  public void sendServerMessagePlayerSimple(Object[] msg) {
    serverToSendMessages.add(new ServerMessage(msg, ServerMessage.MESSAGE_TYPE_SIMPLE, getId()));
  }

  public void sendServerMessagePlayerWarning(Object[] msg) {
    serverToSendMessages.add(new ServerMessage(msg, ServerMessage.MESSAGE_TYPE_WARNING, getId()));
  }

  public void sendSimpleCommand(SimplePlayerCommands command, Object... args) {
    getNetworkObject()
        .simpleCommandQueue
        .add(new RemoteSimpelCommand(new SimplePlayerCommand(command, args), getNetworkObject()));
  }
  ...
  public void setLastEnteredShip(UniqueInterface controllable) {
    setLastEnteredEntity(controllable.getUniqueIdentifier());
  }
  ...
  //			if(send){
  //				System.err.println("SENDING NEW TEAM
  // "+((Teamable)getState()).getTeamOf(this).getClass().getSimpleName()+
  //						" OF "+this+" TO CLIENTS");
  ...
  private void handleMetaBlueprintGive() {
    if (giveMetaBlueprint != null) {
      ...
      if (o != null && o instanceof ShopperInterface) {
        ...
        // INSERTED CODE
        PlayerRequestMetaBlueprintEvent event =
            new PlayerRequestMetaBlueprintEvent(this, ok, inInfiniteShop, giveMetaBlueprint);
        StarLoader.fireEvent(event, true);
        ok = event.isAllowed();
        ...
        if (ok || bypass) {
          ...
          if (((GameStateInterface) getState()).getGameState().allowedToSpawnBBShips(this, f)) {
            try {
              ...
              if (bb.getType() != BlueprintType.SPACE_STATION
                  || getCredits()
                      >= ((GameStateInterface) getState()).getGameState().getStationCost()
                  || inInfiniteShop) {
                ...
                BlueprintMetaItem m =
                    (BlueprintMetaItem)
                        MetaObjectManager.instantiate(MetaObjectType.BLUEPRINT, (short) -1, true);
                ...
                sendServerMessagePlayerError(
                    Lng.astr("You don't have enough\ncredits to buy a\nstation blueprint!"));
              }
              ...
              sendServerMessage(
                  new ServerMessage(
                      Lng.astr("Inventory is full!"), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
            }
          } else {
            sendServerMessagePlayerError(
                Lng.astr("You are currently\nnot allowed to buy\nblueprints!"));
          }
        }
      }
  ...
  private void onClientChannelCreatedOnServer() {
    for (ScanData d : scanHistory) {
      getClientChannel()
          .getNetworkObject()
          .scanDataUpdates
          .add(new RemoteScanData(d, isOnServer()));
    }
    for (SavedCoordinate d : savedCoordinates) {
      getClientChannel()
          .getNetworkObject()
          .savedCoordinates
          .add(new RemoteSavedCoordinate(d, isOnServer()));
    }

    ((GameServerState) getState())
        .getUniverse()
        .getGalaxyManager()
        .sendAllFtlDataTo(getClientChannel().getNetworkObject());

    ((GameServerState) getState())
        .getUniverse()
        .getGalaxyManager()
        .sendAllTradeStubsTo(getClientChannel().getNetworkObject());

    ((GameServerState) state).getChannelRouter().onLogin(getClientChannel());
  }
    ...
    if (!savedCoordinatesToAdd.isEmpty()) {
      synchronized (savedCoordinatesToAdd) {
        while (!savedCoordinatesToAdd.isEmpty()) {
          ...
          if (isOnServer()) {
            int max = (Integer) ServerConfig.MAX_COORDINATE_BOOKMARKS.getCurrentState();
            if (max == 0) {
              sendServerMessagePlayerError(Lng.astr("Server doesn't allow bookmarks!"));
            } else {
              // #RM1788 changed <= to < and allow deletion regardless of the total number of saved
              // waypoints
              if (savedCoordinates.size() < max || dequeue.isRemoveFlag()) {
                if (dequeue.isRemoveFlag()) {
                  ...
                  getClientChannel()
                      .getNetworkObject()
                      .savedCoordinates
                      .add(new RemoteSavedCoordinate(dequeue, isOnServer()));
                } else {
  ...
  private boolean wasKeyDown(KeyboardMappings key) {

    for (int i = 0; i < wasKeyDown.length; i++) {
      if (key == KeyboardMappings.remoteMappings[i] && wasKeyDown[i]) {
        return true;
      }
    }
  ...
  public Vector3i getUniqueSpawningSector(int uniquePlayerId) {
    return new Vector3i(
        Integer.MAX_VALUE - 32, uniquePlayerId * VoidSystem.SYSTEM_SIZE, Integer.MAX_VALUE - 32);
  }

  public Vector3i getUniqueTestingSector(int uniquePlayerId) {
    return new Vector3i(
        Integer.MAX_VALUE - 32,
        uniquePlayerId * VoidSystem.SYSTEM_SIZE + VoidSystem.SYSTEM_SIZE_HALF,
        Integer.MAX_VALUE - 32);
  }
  ...
  public boolean canRotate() {
    ...
    return getNetworkObject().canRotate.get();
  }
  ...
  public boolean isIgnored(PlayerState playerState) {
    return isIgnored(playerState.getName());
  }
  ...
  public void requestCargoInventoryChange(SegmentPiece p) {
    if (p != null) {
      getNetworkObject()
          .cargoInventoryChange
          .add(new RemoteSegmentControllerBlock(new VoidUniqueSegmentPiece(p), getNetworkObject()));
    } else {
      VoidUniqueSegmentPiece v = new VoidUniqueSegmentPiece();
      v.uniqueIdentifierSegmentController = "NONE";
      getNetworkObject()
          .cargoInventoryChange
          .add(new RemoteSegmentControllerBlock(v, getNetworkObject()));
    }
  }

  public void requestUseCargoInventory(boolean b) {
    getNetworkObject().requestCargoMode.add(b ? 1 : 0);
  }
      ...
      if (isInTutorial()) {
        ...
        Sendable sendable =
            getState()
                .getLocalAndRemoteObjectContainer()
                .getUidObjectMap()
                .get(cargoInventoryBlock.uniqueIdentifierSegmentController);

        if (sendable != null && sendable instanceof SegmentController) {
          cargoInventoryBlock.setSegmentController((SegmentController) sendable);
          if (!hasCargoBlockMigratedChunk16
              && cargoInventoryBlock.getSegmentController().isLoadedFromChunk16()) {
            cargoInventoryBlock.voidPos.add(Chunk16SegmentData.SHIFT);
          }
          hasCargoBlockMigratedChunk16 = true;
          if (cargoInventoryBlock.getSegmentController().getSectorId() == getCurrentSectorId()) {
            Inventory inventory;
            if (cargoInventoryBlock.getSegmentController() instanceof ManagedSegmentController<?>
                && (inventory =
                        ((ManagedSegmentController<?>) cargoInventoryBlock.getSegmentController())
                            .getManagerContainer()
                            .getInventory(cargoInventoryBlock.getAbsoluteIndex()))
                    != null) {
              ...
              if (isOnServer()) {
                sendServerMessagePlayerError(
                    Lng.astr("Selected personal cargo doesn't point to any storage block!"));
                setUseCargoInventory(false);
              }
            }
          } else {
            if (isOnServer()) {
              sendServerMessagePlayerError(
                  Lng.astr(
                      "Selected personal cargo out of range!\n"
  ...
  public boolean isInventoryPersonalCargo(StashInventory inventory) {
    if (cargoInventoryBlock != null
        && inventory.getInventoryHolder() instanceof ManagerContainer<?>) {
      ManagerContainer<?> c = (ManagerContainer<?>) inventory.getInventoryHolder();
      return c.getSegmentController()
              .getUniqueIdentifier()
              .equals(cargoInventoryBlock.uniqueIdentifierSegmentController)
          && c.getInventory(cargoInventoryBlock.getAbsoluteIndex()) == inventory;
    }
    return false;
  }
  ...
  public void handleReceivedBlockMsg(ManagerContainer<?> c, ServerMessage s) {
    assert (!isOnServer());
    GameClientState state = (GameClientState) getState();
    InventoryControllerManager inventoryControlManager =
        state
            .getGlobalGameControlManager()
            .getIngameControlManager()
            .getPlayerGameControlManager()
            .getInventoryControlManager();
    ...
    t.origin.set(
        p.x - SegmentData.SEG_HALF, p.y - SegmentData.SEG_HALF, p.z - SegmentData.SEG_HALF);
    c.getSegmentController().getWorldTransform().transform(t.origin);
    RaisingIndication raisingIndication =
        new RaisingIndication(
            t, StringTools.getFormatedMessage(s.getMessage()), 1f, 0.3f, 0.3f, 1f);
    ...
    if (secondInventory != null
        && inventoryControlManager.isTreeActive()
        && secondInventory.getInventoryHolder() == c
        && c.getInventory(s.block) == secondInventory) {
      s.type = ServerMessage.MESSAGE_TYPE_ERROR;
      ((GameClientState) getState()).getServerMessages().add(s);
    }
  }
  ...
  public String toDetailedString() {
    return getName()
        ...
        + "; Sector: "
        + getCurrentSector()
        + "; Invetory slots filled: "
        + inventory.getCountFilledSlots()
        + "; last login: "
        + getLastLogin()
        + "; last logout: "
        + getLastLogout()
  ...
  public void updateDatabase() throws SQLException {
    ...
    ((GameServerState) getState())
        ...
        .getTableManager()
        .getPlayerTable()
        .getPlayerFactionAndPermission(this, offlinePermssion);
    ((GameServerState) getState())
        .getDatabaseIndex()
        .getTableManager()
        .getPlayerTable()
  ...
  public void onChangedFactionServer(FactionChange factionChange) {
    Faction from =
        ((GameServerState) getState()).getFactionManager().getFaction(factionChange.from);
    Faction to = ((GameServerState) getState()).getFactionManager().getFaction(factionChange.to);

    if (from != null) {
      if ((factionChange.previousPermission & FactionPermission.PermType.FOG_OF_WAR_SHARE.value)
          == FactionPermission.PermType.FOG_OF_WAR_SHARE.value) {
    ...
    if ((from == null && factionChange.from != 0)
        || (from != null && from.getMembersUID().isEmpty())) {
      ...
      try {
        ((GameServerState) state)
            ...
            .getTableManager()
            .getVisibilityTable()
            .clearVisibility(factionChange.from);
      } catch (SQLException e) {
        e.printStackTrace();
      }
  ...
  public void sendFowResetToClient(Vector3i sysTo) {
    assert (isOnServer());
    getNetworkObject().resetFowBuffer.add(new RemoteVector3i(sysTo, true));
  }

  /**
   * this is an incrementing seed based on mouse inputs. This is consistent on both client and
   * server
   *
   * @return seed
   */
  ...
  public void requestMineArmTimerChange(int secs) {
    getNetworkObject().mineArmTimerRequests.add(secs);
  }
  ...
  private long getSpawnProtectionTimeLeft() {
    return (lastSpawnedThisSession + getSpawnProtectionTimeSecsMax() * 1000)
        - getState().getUpdateTime();
  }
