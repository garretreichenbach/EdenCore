...
public class LogUtils {
    ...
    private static final String logPath = EdenCore.instance.getSkeleton().getResourcesFolder().getPath() + "/logs";
        ...
        if(!logsFolder.exists()) logsFolder.mkdirs();
            ...
            if(logsFolder.listFiles() != null && logsFolder.listFiles().length > 0) {
                for(File logFile : logsFolder.listFiles()) {
                    ...
                    int logNumber = Integer.parseInt(fileName.split("log")[1]);
                    if(logNumber < EdenCore.instance.maxWorldLogs - 1) logFile.renameTo(new File("log" + logNumber + 1));
                    else logFile.delete();
                }
            }
        }
