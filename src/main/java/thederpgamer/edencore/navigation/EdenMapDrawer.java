...
public class EdenMapDrawer implements GameMapDrawListener {
	...
	public void createMarkerMenu(MapMarker selected, MousePressEvent event) {
		...
		if(selected != null && (selected.isCustomizable() || GameClient.getClientPlayerState().isAdmin())) {
			...
			try {
				...
				field.setAccessible(true);
				MapToolsPanel mapPanel = (MapToolsPanel) field.get(playerPanel);
				mapPanel.attach(markerMenu);
				markerMenu.setPos(mapPanel.getPos());
			} catch(Exception exception) {
				exception.printStackTrace();
			}
		}
	...
	public static class MarkerCustomizeMenu extends GUIAncor {
		...
		public void draw() {
			super.draw();
			if(buttonPane != null) buttonPane.draw();
		}

		@Override
		public void onInit() {
			super.onInit();
			buttonPane = new GUIHorizontalButtonTablePane(getState(), 3, 2, this);
			buttonPane.onInit();
