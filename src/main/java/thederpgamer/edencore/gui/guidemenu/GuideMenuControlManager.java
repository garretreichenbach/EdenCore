...
public class GuideMenuControlManager extends GUIControlManager {
    ...
    private void loadGuides() {
        //Load Guide entries from file
        String guidesPath = DataUtils.getWorldDataPath() + "/guides";
        try {
            File guidesFolder = new File(guidesPath);
            ...
            for(File file : guidesFolder.listFiles()) {
				try {
					...
					} else { //Put under misc category
						for(File subFile : file.listFiles()) {
							if(subFile.isFile()) {
								GuideEntryData entry = GuideEntryData.loadFromFile(subFile, subFile.getName().split(".txt")[0], GuideEntryCategory.MISC);
								guideEntries.add(entry);
							}
						}
					}
