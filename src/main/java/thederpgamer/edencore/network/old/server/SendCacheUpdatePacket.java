...
public class SendCacheUpdatePacket extends Packet {
	//Exchange
	private final ArrayList<BlueprintExchangeItemOld> blueprintExchangeItemOlds = new ArrayList<>();
	...
	public void readPacketData(PacketReadBuffer packetReadBuffer) throws IOException {
		...
		if(bpSize > 0) for(int i = 0; i < bpSize; i++) blueprintExchangeItemOlds.add(new BlueprintExchangeItemOld(packetReadBuffer));
	...
	public void writePacketData(PacketWriteBuffer packetWriteBuffer) throws IOException {
		getBlueprintItems();
		packetWriteBuffer.writeInt(blueprintExchangeItemOlds.size());
		if(blueprintExchangeItemOlds.size() > 0) for(BlueprintExchangeItemOld exchangeItem : blueprintExchangeItemOlds) exchangeItem.serialize(packetWriteBuffer);
		getResourceItems();
	...
	public void processPacketOnClient() {
		...
		try { //Exchange
			ClientCacheManager.BLUEPRINT_EXCHANGE_ITEM_OLDS.clear();
			ClientCacheManager.BLUEPRINT_EXCHANGE_ITEM_OLDS.addAll(blueprintExchangeItemOlds);
	...
	private void getBlueprintItems() {
		ArrayList<BlueprintExchangeItemOld> exchangeItems = new ArrayList<>();
		for(Object obj : PersistentObjectUtil.getObjects(EdenCore.getInstance().getSkeleton(), BlueprintExchangeItemOld.class)) {
			exchangeItems.add((BlueprintExchangeItemOld) obj);
		}
		blueprintExchangeItemOlds.addAll(exchangeItems);
	}
