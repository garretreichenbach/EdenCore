...
public class BuildSectorCommand implements CommandInterface {
	...
	public static final long MAX_WAIT_TIME = 10000; // How long to wait for the the AI toggling method to finish before killing it due to
	// stall
					...
					case "permissions":
						if(args.length == 2) {
							...
							if(target != null) {
								...
								for(Map.Entry<String, Boolean> entry : sectorData.getPermissions(target.getName()).entrySet())
									builder.append(entry.getKey()).append(" : ").append(entry.getValue().toString().toUpperCase()).append("\n");
								PlayerUtils.sendMessage(sender, builder.toString().trim());
							} else PlayerUtils.sendMessage(sender, "Player \"" + args[1] + "\" doesn't exist.");
						} else return false;
						return true;
