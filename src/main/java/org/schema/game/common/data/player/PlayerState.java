...
public class PlayerState extends AbstractOwnerState implements Sendable, DiskWritable, FogOfWarReceiver, RuleEntityContainer {
		...
		if(isOnServer()) {
			...
			if(forcedEnterUID != null && getAssingedPlayerCharacter() != null && getFirstControlledTransformableWOExc() == getAssingedPlayerCharacter()) {
				...
				if(sendable instanceof Ship && ((Ship) sendable).getSectorId() == getSectorId()) {
					...
					if(pointUnsave != null) {
						getControllerState().requestControlServerAndSend(getAssingedPlayerCharacter(), (PlayerControllable) pointUnsave.getSegment().getSegmentController(), new Vector3i(), pointUnsave.getAbsolutePos(new Vector3i()), true);
						forcedEnterUID = null;
					}
				}
			}
	...
	public boolean isClientOwnPlayer() {
		...
		return (((GameClientState) getState()).getPlayer() != null && ((GameClientState) getState()).getPlayer().getId() == this.getId());
	}
	...
	public void announceKill(Damager killerEntity) {
		if(isOnServer()) {
			...
			System.err.println(getState() + " " + this + " Announcing kill: " + killerEntity + " killed " + this);
			int id = -1;
	...
	public void initFromNetworkObject(NetworkObject o) {
		...
		if(!isOnServer()) {
			...
			if(getClientId() == state.getId()) {
				...
				s.setPlayer(this);
				System.out.println("[PlayerState] Client successfully received player state " + getState() + ", owner: " + getClientId());
			...
			if(type != Element.TYPE_NONE) {
				...
				if(s != null) {
					...
					if(type == InventorySlot.MULTI_SLOT && typeToDrop == InventorySlot.MULTI_SLOT) {
						...
						if(((MetaObjectState) getState()).getMetaObjectManager().getObject(meta) != null && ((MetaObjectState) getState()).getMetaObjectManager().getObject(meta).isInventoryLocked(inventory)) {
			...
			synchronized(createDockRequests) {
				while(!createDockRequests.isEmpty()) {
					...
					if(snd != null && snd instanceof SegmentController) {
						...
						Ship newShip = EntityRequest.getNewShip(getState(), EntityType.SHIP.dbPrefix + d.name, dockOn.getSectorId(), d.name, mat, -2, -2, -2, 2, 2, 2, getUniqueIdentifier(), false);
	...
	private void dieOnServer(Damager from) {
		...
		if(isInTutorial() || isInTestSector()) {
			...
			Sector defaultSec;
			try {
				defaultSec = ((GameServerState) getState()).getUniverse().getSector(new Vector3i((Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_X.getCurrentState(), (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Y.getCurrentState(), (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Z.getCurrentState()));
	...
	public boolean spawnCrew() {
		if(getPlayerAiManager().getCrew().size() < 5) {
			CreatureSpawn s;
			try {
				s = new CreatureSpawn(new Vector3i(this.getCurrentSector()), new Transform(this.getFirstControlledTransformable().getWorldTransform()), "NoName", CreatureType.CHARACTER) {
					@Override
		...
		if(command >= SimplePlayerCommands.values().length || command < 0) {
			...
			switch(simplePlayerCommands) {
					...
					if(isInTutorial()) {
						...
						try {
							if(spawnData.preSpecialSector == null) {
								originalSec = ((GameServerState) getState()).getUniverse().getSector(new Vector3i((Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_X.getCurrentState(), (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Y.getCurrentState(), (Integer) ServerConfig.DEFAULT_SPAWN_SECTOR_Z.getCurrentState()));
							} else {
				...
				case DESTROY_TUTORIAL_ENTITY:
					...
					if(isInTutorial()) {
						for(Sendable s : getState().getLocalAndRemoteObjectContainer().getLocalObjects().values()) {
							if(s instanceof SimpleTransformableSendableObject && ((SimpleTransformableSendableObject) s).getUniqueIdentifier() != null && ((SimpleTransformableSendableObject) s).getUniqueIdentifier().startsWith(uid) && ((SimpleTransformableSendableObject) s).getSectorId() == getCurrentSectorId()) {
		...
		if(!killerIds.isEmpty()) {
			synchronized(killerIds) {
				while(!killerIds.isEmpty()) {
					...
					Sendable sendableKiller = getState().getLocalAndRemoteObjectContainer().getLocalObjects().get(killerId);
					if(!isOnServer()) {
						String message = "";
						getControllerState().removeAllUnitsFromPlayer(this, true);
						if(isClientOwnPlayer()) {
						...
						if(sendableKiller != null) {
							...
							((GameClientState) getState()).getController().popupGameTextMessage(Lng.str("Unkown Object(%s) killed\n%s!", killerId, getName()), 0);
						}
			...
			try {
				if(System.currentTimeMillis() - lastBlueprintSpawn > blueprintDelay) {
					...
					if(e.metaItem >= 0) {
						...
						try {
							loadBluePrint = BluePrintController.active.loadBluePrint((GameServerState) getState(), e.catalogName, e.shipName, t, -1, e.factionId, getCurrentSector(), this.getUniqueIdentifier(), buffer, e.toDockOn, e.activeAI, new ChildStats(false));
						...
						SegmentControllerOutline loadBluePrint = BluePrintController.active.loadBluePrint((GameServerState) getState(), e.catalogName, e.shipName, t, e.infiniteShop ? -1 : credits, e.factionId, getCurrentSector(), this.getUniqueIdentifier(), buffer, e.toDockOn, e.activeAI, new ChildStats(false));
			...
			for(RemoteBlueprintPlayerRequest s : p.catalogPlayerHandleBuffer.getReceiveBuffer()) {
				...
				if(bbR.save) {
					...
					if(sendable != null && sendable instanceof Ship || sendable instanceof SpaceStation) {
						...
						if((Integer) ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState() < 0 || getCatalog().getPersonalCatalog().size() < (Integer) ServerConfig.CATALOG_SLOTS_PER_PLAYER.getCurrentState()) {
	...
	public boolean canClientPressKey() {
		...
		if(lastCanKeyPressCheck != getState().getNumberOfUpdate()) {
			lastCanKeyPressCheckResult = !((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().getChatControlManager().isActive() && ((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().isActive() && !((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().isAnyMenuActive() && ((GameClientState) getState()).getPlayerInputs().isEmpty();
	...
	public void searchForLastEnteredEntity() {
		...
		if(lastEnteredEntity == null || lastEnteredEntity.isEmpty() || !lastEnteredEntity.startsWith("ENTITY_")) {
			sendServerMessage(new ServerMessage(Lng.astr("Invalid last entered:\n", lastEnteredEntity == null ? "Not set" : lastEnteredEntity), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
			return;
		}
		...
		try {
			...
			if(lastEnteredEntity != null && !lastEnteredEntity.equals("none")) {
				try {
					...
					if(byUID.isEmpty()) {
						sendServerMessage(new ServerMessage(Lng.astr("Ship has not been found! (original name: %s)", entity), ServerMessage.MESSAGE_TYPE_ERROR, getId()));
					} else {
						for(DatabaseEntry a : byUID) {
							sendServerMessage(new ServerMessage(Lng.astr("Found ship in sector:\n(orig: %s)\n-> %s", entity, a.sectorPos), ServerMessage.MESSAGE_TYPE_INFO, getId()));
						}
					}
				} catch(SQLException e) {
					e.printStackTrace();
	...
	public void suicideOnClient() {
		((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().getPlayerIntercationManager().getInShipControlManager().exitShip(true);
		((GameClientState) getState()).getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().getPlayerIntercationManager().getInShipControlManager().setActive(false);
		sendKill(this.getId());
	}
