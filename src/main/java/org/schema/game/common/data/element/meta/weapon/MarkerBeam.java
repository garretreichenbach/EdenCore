...
public class MarkerBeam extends Weapon {
	...
	public void fire(AbstractCharacter<?> playerCharacter, AbstractOwnerState state, Vector3f dir, boolean addButton, boolean removeButton, Timer timer) {
		...
		if(segmentPiece != null) {
			if(DataUtils.isBuildSector(playerCharacter.getSector(new Vector3i()))) {
				for(short id : EdenCore.disabledBlocks) {
					if(id == segmentPiece.getType()) {
						System.err.println("Cannot mark disabled block!");
						return;
					}
				}
			}
