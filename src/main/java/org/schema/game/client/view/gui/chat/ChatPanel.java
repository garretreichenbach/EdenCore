...
public class ChatPanel extends GUIElement implements GUIActiveInterface {
	...
	public final ObjectArrayList<ChatWindow> chatWindows = new ObjectArrayList<ChatWindow>();
	public static GUIDialogWindow chatPanel;
	...
	public ChatPanel(InputState state) {
		...
		visibileChatDefault = new GUITextOverlayTable(10, 10, getClientState());
		visibileChatDefault.onInit();
		visibileChatDefault.setText((List<Object>) ((List<? extends Object>) getClientState().getChannelRouter().getDefaultVisibleChatLog()));
		visibileChatDefault.setBeginTextAtLast(true);

		GUIAncor defaultChatDependent = new GUIAncor(state, 400, 300);
	...
	public static void createChatPane(GUIContentPane pane, final GameClientState state, final ChatCallback chatCallback, final boolean scrollLockOnBar, boolean extraTBMemberListBottom, final ChatPanel mainPanel) {
		...
		GUITextOverlayTable text = new GUITextOverlayTable(10, 10, state) {
	...
	public void draw() {
		...
		for(int i = 0; i < chatWindows.size(); i++) {
			if(!chatWindows.get(i).ident.getMemberPlayerStates().contains(getClientState().getPlayer())) {
		...
		for(int i = chatWindows.size() - 1; i >= 0; i--) {
			if(!found) {
				...
				if(chatWindows.get(i).anchorInside()) {
					for(MouseEvent e : getClientState().getController().getInputController().getMouseEvents()) {
		...
		if(ChatPanel.flagActivate) {
			boolean fd = false;
			// System.err.println("############### FLAG ACTIVATE "+getClientState().lastSelectedInput);
			if(getClientState().getController().getInputController().getLastSelectedInput() != null) {
				for(int i = 0; i < chatWindows.size(); i++) {
					ChatWindow chatWindow = chatWindows.get(i);
					if(chatWindow.ident.getChatBar().getTextArea() == getClientState().getController().getInputController().getLastSelectedInput()) {
	...
	public void onInit() {
		...
		chatPanel = new GUIDialogWindow(getClientState(), 750, 550, "ChatPanelMain") {
		...
		chatPanel.setCloseCallback(new GUICallback() {
			@Override
			public boolean isOccluded() {
				return !getClientState().getController().getPlayerInputs().isEmpty();
			}

			@Override
			public void callback(GUIElement callingGuiElement, MouseEvent event) {
				if(event.pressedLeftMouse()) {
					getClientState().getGlobalGameControlManager().getIngameControlManager().getChatControlManager().setActive(false);
				}
			}
		});
	...
	public void recreateTabs() {
		...
		c = new GUITabbedContent(getClientState(), chatPanel.getMainContentPane().getContent(0));
		c.onInit();
		...
		cb = new ChatCallback() {
			...
			public List<Object> getChatLog() {
				return getClientState().getGeneralChatLog();
			}

			@Override
			public List<? extends Object> getVisibleChatLog() {
				return getClientState().getVisibleChatLog();
			}

			@Override
			public boolean localChatOnClient(String text) {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					return getClientState().getChannelRouter().getAllChannel().localChatOnClient(text);
				}
				return false;
			}
			...
			public void chat(String text) {
				...
				m.sender = getClientState().getPlayerName();
				m.receiver = "all";
				m.text = text;
				m.receiverType = ChatMessageType.CHANNEL;
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					getClientState().getChannelRouter().getAllChannel().send(m);
				}
				// getClientState().getChat().addToVisibleChat(text, "", true);
			}
			...
			public Collection<PlayerState> getMemberPlayerStates() {
				return getClientState().getOnlinePlayersLowerCaseMap().values();
			}
			...
			public boolean hasChannelMuteList() {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					return getClientState().getChannelRouter().getAllChannel().hasChannelMuteList();
				}
				return false;
			}
			...
			public boolean isMuted(PlayerState f) {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					return getClientState().getChannelRouter().getAllChannel().isMuted(f);
				}
				return false;
			}
			...
			public void requestMuteUnmuteOnClient(String f, boolean b) {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					getClientState().getChannelRouter().getAllChannel().requestMuteUnmuteOnClient(f, b);
					;
				}
			}
			...
			public String[] getMuted() {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					return getClientState().getChannelRouter().getAllChannel().getMuted();
				}
				return null;
			}
			...
			public void requestIgnoreUnignoreOnClient(String f, boolean b) {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					getClientState().getChannelRouter().getAllChannel().requestIgnoreUnignoreOnClient(f, b);
				}
			}
			...
			public void markRead() {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					getClientState().getChannelRouter().getAllChannel().markRead();
				}
			}
			...
			public int getUnread() {
				if(getClientState().getChannelRouter().getAllChannel() != null) {
					return getClientState().getChannelRouter().getAllChannel().getUnread();
				} else {
					return 0;
				}
			}
		...
		createChatPane(chatTab, getClientState(), cb, true, true, this);
		GUICheckBoxTextPair cb = new GUICheckBoxTextPair(getClientState(), Lng.str("Hide on ret."), 65, FontLibrary.getBlenderProMedium13(), 10) {
		...
		GUIScrollablePanel lr = new GUIScrollablePanel(1, 1, chatTab.getTextboxes(1).get(1), getClientState());
		lr.setScrollable(0);
		...
		ChatChannelsScrollableListNew channelsSc = new ChatChannelsScrollableListNew(getClientState(), channelsTab.getContent(0), this);
		channelsSc.onInit();
		...
		channelsTab.setTextBoxHeightLast(100);
		channelsTab.addNewTextBox(27);

		channelsTab.setListDetailMode(channelsTab.getTextboxes().get(0));

		GUIHorizontalButtonTablePane buttons = new GUIHorizontalButtonTablePane(getClientState(), 2, 1, channelsTab.getContent(1));
		buttons.onInit();
		...
		buttons.addButton(0, 0, Lng.str("Create Channel"), HButtonType.BUTTON_BLUE_MEDIUM, new GUICallback() {
			...
			public void callback(GUIElement callingGuiElement, MouseEvent event) {
				if(event.pressedLeftMouse()) {
					PlayerGameTextInput playerTextInput = new PlayerGameTextInput("ccChatPanel", getClientState(), 32, Lng.str("Create New Channel"), "") {
					...
					GUICheckBoxTextPair cb = new GUICheckBoxTextPair(getClientState(), Lng.str("Permanent"), 65, FontLibrary.getBlenderProMedium13(), 10) {
	...
	public void openChat(ChatCallback cb) {
		...
		if(chatWindow == null) {
			PlayerChatInput p = new PlayerChatInput(cb.getName(), getClientState(), cb.getTitle(), this, this, cb);
	...
	public ShopControllerManager getShopControlManager() {
		return getClientState().getGlobalGameControlManager().getIngameControlManager().getPlayerGameControlManager().getShopControlManager();
	}

	public PlayerState getOwnPlayer() {
		return ChatPanel.this.getClientState().getPlayer();
	}

	public Faction getOwnFaction() {
		return ChatPanel.this.getClientState().getFactionManager().getFaction(getOwnPlayer().getFactionId());
	}
	...
	public boolean isActive() {
		// System.err.println("CUDRA: "+currentlyDrawing+"; "+getClientState().getController().getPlayerInputs().size());
		return getClientState().getController().getPlayerInputs().isEmpty() && (currentlyDrawing == null || (currentlyDrawing.anchorInside() || chatWindows.get(chatWindows.size() - 1) == currentlyDrawing));
	}
	...
	public static class ChatWindow implements GUICallback {

		public ChatCallback ident;
		...
		public ChatWindow(GUIElement drawable, ChatCallback chatCallback) {
			...
			this.windowAnchor = new GUIAncor(getClientState());
			this.windowAnchor.setMouseUpdateEnabled(true);
			this.windowAnchor.setCallback(this);
			this.windowAnchorClose = new GUIAncor(getClientState());
			...
			visibileChatOnSticky = new GUITextOverlayTable(10, 10, getClientState());
			...
			if(drawable instanceof GUIInputPanel) {
				...
				chatLogPanel = new GUIChatLogPanel(400, 150, fontSize, ((GUIDialogWindow) ((GUIInputPanel) drawable).getBackground()).getMainContentPane().getContent(0, 0), getClientState());
			} else {
				visibileChatOnSticky.autoWrapOn = ((GUIDialogWindow) drawable).getMainContentPane().getContent(0, 0);
				chatLogPanel = new GUIChatLogPanel(400, 150, fontSize, ((GUIDialogWindow) drawable).getMainContentPane().getContent(0, 0), getClientState());
			}
