...
public class BlueprintEntry implements BlueprintInterface {
	...
	public void init(String name, BluePrintController bbController) {
		...
		//INSERTED CODE
		modMappingsFile = new FileExt(bbController.entityBluePrintPath + "/" + name + "/modmappings.smbmm");
		///
		rawBlockDir = new FileExt(bbController.entityBluePrintPath + "/" + name + "/DATA/");
	}

	public static File importToFile(File toImport, BluePrintController bluePrintController) throws IOException {
		...
		long gbInBytes = 1024L * 1024L * ((Integer) ServerConfig.ALLOWED_UNPACKED_FILE_UPLOAD_IN_MB.getCurrentState()).longValue();
		if(bytes > gbInBytes) {
		...
		if(!found) {
			...
			for(int i = 0; i < fList.length; i++) {
				if(!"header.smbph".equals(fList[i])) {
					found = true;
					break;
				}
			}
	...
	private void copyDataFilesToBlueprint(final SegmentController controller) throws IOException {
		...
		File[] rawFiles = dbDataDir.listFiles(new FilenameFilter() {
			@Override
	...
	private void copyLocalDataFilesToBlueptint(SegmentController controller) throws IOException {
	...
	public ElementCountMap getElementCountMapWithChilds() {
		if(countWithChilds == null) {
			countWithChilds = new ElementCountMap(elementMap);
			if(childs != null) {
				for(BlueprintEntry c : childs) {
	...
	public ControlElementMapper readStructure(File structure, boolean force) throws IOException {
		if(controlElementMap == null || force) {
		...
		while(!end && (dataType = stream.readByte()) != FINISH_BYTE) {
			...
			switch(dataType) {
				case (SEG_MANAGER_BYTE): {
					managerTag = Tag.readFrom(stream, false, false);
					end = true;
					break;
				}
				case (DOCKING_BYTE): {
	...
	public void read() throws IOException {
		...
		//INSERTED CODE
		readModMappings(modMappingsFile);
		///
	...
	public void write(SegmentController controller, boolean local) throws IOException {
		...
		if(classification == null) {
			...
			classification = controller.getType().getDefaultClassification();
		}
		DataOutputStream hOut = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(header)));
	...
	private void copyOldDataFilesToBlueprint(final SegmentControllerBluePrintEntryOld old, String entityBluePrintPath) throws IOException {
		...
		File[] list = dataPathF.listFiles(new FileFilter() {
			@Override
	...
	private void writeHeader(SegmentControllerBluePrintEntryOld old, DataOutputStream stream) throws IOException {
		//		assert(false);
		stream.writeInt(dataVersion);
	...
	private void writeStructure(SegmentControllerBluePrintEntryOld old, DataOutputStream stream) throws IOException {
		stream.writeInt(structureVersion);
		ControlElementMap.serializeForDisk(stream, old.getControllingMap());
	}

	private void writeMeta(SegmentControllerBluePrintEntryOld old, DataOutputStream stream) throws IOException {
	...
	public void readHeader(File header) throws IOException {
		try {
			...
			if(headerVersion == 0) {
				...
				if(hasScore) {
					score = new EntityIndexScore();
					score.deserialize(stream, 0, true);
				}
			}
			stream.close();
	...
	public void writeHeader(SegmentController controller, DataOutputStream stream) throws IOException {
		...
		if(controller instanceof ManagedSegmentController<?>) {
			...
			if(managerContainer.getStatisticsManager() != null) {
				try {
					score = managerContainer.getStatisticsManager().calculateIndex();
				} catch(Exception e) {
					e.printStackTrace();
				}
			}
		...
		writeHeader(controller.getBoundingBox().min, controller.getBoundingBox().max, BlueprintType.getType(controller.getClass()), score, classification, controller.getElementClassCountMap(), stream);
	}
		...
		if(fileName == null || EntityRequest.existsIdentifierWOExc(state, fileName)) {
			try {
				...
				if(purge) {
					...
					if(idForFullUID >= 0) {
						((GameServerState) state).getDatabaseIndex().getTableManager().getEntityTable().removeSegmentController(type.type.dbPrefix + newEntityName, (GameServerState) state);
					}
				} else {
